...
> **Date**: 2026-02-18
> **Root cause**: Secondary SH-2 callback crashes due to incorrect data state (init functions not yet investigated)
...
mechanism** between the primary and secondary SH-2 processors. In production, both CPUs
synchronize via MINIT/SINIT writes. In the reimpl, the secondary's callback crashes due
to incorrect data state — the specific init functions responsible have not yet been
identified. The NOP bypass lets the primary continue without the secondary's response.
...
```
Primary SH-2 (FUN_0600C010)         Secondary SH-2 (FUN_06034F08)
============================        ============================
1. Write to MINIT (0x01000000)  ──> Sets secondary's ICF
...
|-----------|---------|----------|
| Primary MINIT write | 0x0600C0F0 | Inside FUN_0600C010, triggers secondary ICF |
| Primary ICF poll | 0x0600C11C-0x0600C124 | Polls FTCSR bit 7 in tight loop |
| Secondary ICF poll | 0x06034F34-0x06034F5E | FUN_06034F08, polls its own FTCSR |
| Secondary SINIT write | 0x0600C1F0 | FUN_0600C170 RTS delay slot → 0x21800000 |
| Callback pointer | 0x06063574 | Set by FUN_0600C010 to FUN_0600C170 |

### MINIT/SINIT Mechanism (Mednafen)

- Write to **0x01000000-0x017FFFFF** → `CPU[1].SetFTI()` → sets secondary's ICF
- Write to **0x01800000-0x01FFFFFF** → `CPU[0].SetFTI()` → sets primary's ICF
...
FTI: addr=01000000 target_cpu=1 master_PC=0600C0F0 slave_PC=06034F38  ← primary writes MINIT
FTI: addr=01800000 target_cpu=0 master_PC=0600C124 slave_PC=06034F5A  ← secondary writes SINIT
FTI: addr=01000000 target_cpu=1 master_PC=0600C0F0 slave_PC=06034F62  ← primary writes MINIT
FTI: addr=01800000 target_cpu=0 master_PC=0600C124 slave_PC=06034F5A  ← secondary writes SINIT
...
FTI: addr=01000000 target_cpu=1 master_PC=0600C0F0 slave_PC=0602829A  ← primary writes MINIT
FTI: addr=01000000 target_cpu=1 master_PC=0600C0F0 slave_PC=0602829A  ← (×178 total)
```

**Reimpl sync broken**:
- Primary writes MINIT (✓) — 178 times in 30 seconds
- Secondary detects ICF first time (✓) — exits poll, calls FUN_0600C170
- Secondary NEVER writes SINIT (✗) — crashes into trap at 0x0602829A
- **Zero SINIT writes** in reimpl vs alternating MINIT/SINIT in production

## The Secondary Crash

After the first ICF detection, the secondary calls FUN_0600C170 (the callback stored at
*(0x06063574)). FUN_0600C170's call chain encounters incorrect data state and the
secondary ends up at address **0x06028296**:
...
This is a standard SH-2 panic/halt pattern (`SETT; BT $`). The secondary hits this
...
- Bypass: `0x00, 0x09` — `nop` (fall through, skip waiting for secondary)

**Default build has original bytes** (byte-identical policy). The bypass is needed
for boot testing because:
1. The secondary's callback crashes due to incorrect data state (root cause not yet traced)
2. The secondary never writes SINIT, so the primary's ICF is never set
3. Without the bypass, the primary hangs forever at the ICF poll
...
The NOP bypass is **sufficient** for single-CPU boot progression. The primary SH-2
runs the game state machine, VDP setup, and attract mode logic independently.
The secondary being stuck doesn't prevent basic boot.
...
2. Verify the secondary's callback completes and writes SINIT
...
### Secondary SH-2 Lifecycle (Daytona USA)

1. **Boot**: Secondary starts OFF (SMPC default)
2. **Activation**: Primary issues SMPC SSHON → secondary starts via SetActive(true)
3. **BIOS init**: Secondary runs BIOS code, jumps to game-defined entry point
4. **Polling**: Secondary enters FUN_06034F08 — infinite loop polling FTCSR for ICF
5. **Sync**: Primary writes MINIT → secondary's ICF set → secondary calls callback
6. **Callback**: FUN_0600C170 does per-frame work, writes SINIT to signal primary
7. **Return**: Secondary returns to FUN_06034F08 polling loop
...
| Claim | Evidence |
|-------|----------|
| Secondary SH-2 activates in both production and reimpl | SH2_ACTIVATED trace fires for SH2-S |
...
| Secondary polls from FUN_06034F08 (PC=0x06034F3A) | FTCSR_READ trace shows cpu=SH2-S PC=06034F3A |
| Primary MINIT write at PC=0x0600C0F0 | FTI trace master_PC field |
| Secondary SINIT write at PC=0x06034F5A (production only) | FTI trace slave_PC field |
| Reimpl secondary crashes to 0x0602829A after first ICF | FTI trace slave_PC changes from 06034F3E to 0602829A |
