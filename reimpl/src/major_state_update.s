/* major_state_update -- L3 assembly (SH-2 mnemonics)
 * Translation unit: 0x06041D74 - 0x06042088
 * Auto-generated by tools/generate_l3_tu.py
 */

    .section .text.FUN_06041D74


    .global major_state_update
    .type major_state_update, @function
major_state_update:
    sts.l pr, @-r15
    add #-0x10, r15
    mov.l   .L_sym_060A5400, r14
    mov.l r4, @r15
    mov #0x1, r4
    mov.l @r14, r3
    mov.w   .L_const_0x338_w, r0
    mov.l @(r0, r3), r2
    cmp/eq r4, r2
    bf/s    .L_06041E0C
    mov r4, r12
    mov.l @r14, r2
    mov.w   .L_const_0x340_w, r0
    mov.l @(r0, r2), r0
    tst r0, r0
    bf      .L_06041DDE
    mov.l @r14, r3
    mov.w   DAT_06041df0, r0
    mov.l @(r0, r3), r2
    mov.l   .L_mask_16bit, r3
    cmp/eq r3, r2
    bf      .L_06041DDE
    mov.l @r14, r5
    mov.w   DAT_06041df2, r0
    mov.l   .L_track_surface_type_a, r3
    mov.l @(r0, r5), r5
    jsr @r3
    mov #0x0, r4
    mov r0, r4
    tst r4, r4
    bt      .L_06041DB6
    bra     .L_06041EDC
    mov #0x1, r0
.L_06041DB6:
    mov r15, r4
    mov.l   .L_ai_checkpoint_validate, r3
    jsr @r3
    add #0x4, r4
    mov.l @r14, r2
    mov r15, r3
    add #0x4, r3
    mov.b @r3, r1
    mov #0x40, r0
    mov.b r1, @(r0, r2)
    mov.l @r15, r3
    mov.l @r3, r2
    add #0x1, r2
    mov.l r2, @r3
    mov.l @r14, r3
    mov #0x4, r2
    mov.w   .L_const_0x338_w, r0
    mov.l r2, @(r0, r3)
    bra     .L_06041E0C
    nop
.L_06041DDE:
    mov.l @r14, r4
    mov.w   DAT_06041df0, r0
    mov.l @(r0, r4), r4
    tst r4, r4
    bf      .L_06041E04
    bra     .L_06041E0C
    mov r13, r12
.L_const_0x338_w:
    .2byte  0x0338
.L_const_0x340_w:
    .2byte  0x0340

    .global DAT_06041df0
DAT_06041df0:
    .2byte  0x0344

    .global DAT_06041df2
DAT_06041df2:
    .2byte  0x033C
.L_sym_060A5400:
    .4byte  sym_060A5400
.L_mask_16bit:
    .4byte  0x0000FFFF
.L_track_surface_type_a:
    .4byte  track_surface_type_a
.L_ai_checkpoint_validate:
    .4byte  ai_checkpoint_validate
.L_06041E04:
    mov.l @r14, r2
    mov #0x2, r3
    mov.w   .L_const_0x338_w_2, r0
    mov.l r3, @(r0, r2)
.L_06041E0C:
    mov.l @r14, r0
    mov.w   .L_const_0x338_w_2, r1
    mov.l @(r0, r1), r0
    cmp/eq #0x2, r0
    bf      .L_06041E56
    mov.l @r14, r6
    mov.w   DAT_06041eb6, r0
    mov.l @r14, r5
    mov.l @r14, r4
    mov.l   .L_track_impact_response, r3
    mov.l @(r0, r6), r6
    add #-0x4, r0
    mov.l @(r0, r5), r5
    add #-0x4, r0
    jsr @r3
    mov.l @(r0, r4), r4
    mov r0, r4
    mov.l @r15, r2
    mov.l @r2, r3
    add #0x1, r3
    tst r4, r4
    bf/s    .L_06041E56
    mov.l r3, @r2
    mov.l @r14, r3
    mov #0x3, r2
    mov.w   .L_const_0x338_w_2, r0
    mov r15, r4
    mov.l r2, @(r0, r3)
    mov.l   .L_ai_checkpoint_validate_2, r3
    jsr @r3
    add #0x4, r4
    mov.l @r14, r2
    mov r15, r3
    add #0x4, r3
    mov.b @r3, r1
    mov #0x40, r0
    mov.b r1, @(r0, r2)
.L_06041E56:
    mov.l @r14, r0
    mov.w   .L_const_0x338_w_2, r1
    mov.l @(r0, r1), r0
    cmp/eq #0x3, r0
    bf      .L_06041E80
    mov.w   .L_bit_7_w, r2
    mov.l   .L_sym_06035C4E, r3
    jsr @r3
    mov.l r2, @r15
    mov r0, r5
    mov.l @r15, r3
    exts.w r5, r4
    and r3, r4
    tst r4, r4
    bt      .L_06041E78
    bra     .L_06041E7A
    mov #0x1, r4
.L_06041E78:
    mov #0x0, r4
.L_06041E7A:
    tst r4, r4
    bt      .L_06041E80
    mov r13, r12
.L_06041E80:
    mov.l @r14, r0
    mov.w   .L_const_0x338_w_2, r1
    mov.l @(r0, r1), r0
    cmp/eq #0x4, r0
    bf      .L_06041EAA
    mov #0x40, r2
    mov.l   .L_sym_06035C4E, r3
    jsr @r3
    mov.l r2, @r15
    mov r0, r4
    mov.l @r15, r3
    exts.w r4, r5
    and r3, r5
    tst r5, r5
    bt      .L_06041EA2
    bra     .L_06041EA4
    mov #0x1, r5
.L_06041EA2:
    mov #0x0, r5
.L_06041EA4:
    tst r5, r5
    bt      .L_06041EAA
    mov r13, r12
.L_06041EAA:
    mov r12, r0
    cmp/eq #0x1, r0
    bf      .L_06041EC8
    bra     .L_06041EDC
    mov #0x1, r0
.L_const_0x338_w_2:
    .2byte  0x0338

    .global DAT_06041eb6
DAT_06041eb6:
    .2byte  0x0344
.L_bit_7_w:
    .2byte  0x0080
    .2byte  0xFFFF
.L_track_impact_response:
    .4byte  track_impact_response
.L_ai_checkpoint_validate_2:
    .4byte  ai_checkpoint_validate
.L_sym_06035C4E:
    .4byte  sym_06035C4E
.L_06041EC8:
    mov.l   .L_const_0xfff7, r4
    mov.l   .L_smpc_cmd_helper_b, r3
    jsr @r3
    nop
    mov.l @r14, r2
    mov.w   DAT_06041f90, r0
    mov.l r13, @(r0, r2)
    mov.l @r14, r3
    mov.l r13, @(52, r3)
    mov r13, r0
.L_06041EDC:
    add #0x10, r15
    lds.l @r15+, pr
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14

    .global track_edge_validate
    .type track_edge_validate, @function
track_edge_validate:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    sts.l pr, @-r15
    add #-0x10, r15
    mov.l   .L_ai_checkpoint_validate_3, r12
    mov.l   .L_sym_060A5400_2, r14
    mov.w   .L_const_0x348_w, r1
    mov.l @r14, r0
    mov.l @(r0, r1), r0
    cmp/eq #0x1, r0
    bf/s    .L_06041F60
    mov r4, r13
    mov.l @r14, r5
    mov.w   DAT_06041f94, r3
    mov.l @r14, r4
    mov.w   DAT_06041f96, r0
    add r3, r5
    mov.l   .L_track_road_edge_b, r3
    jsr @r3
    mov.l @(r0, r4), r4
    mov r15, r4
    mov.l @r13, r2
    add #0x1, r2
    mov.l r2, @r13
    jsr @r12
    add #0x4, r4
    mov r15, r2
    mov #0x40, r0
    mov #0x0, r5
    mov.l @r14, r3
    add #0x4, r2
    mov.b @r2, r1
    mov.b r1, @(r0, r3)
    mov.l @r14, r4
    mov.w   DAT_06041f96, r0
    mov.l   .L_track_road_width_main, r3
    jsr @r3
    mov.l @(r0, r4), r4
    mov r0, r4
    tst r4, r4
    bt      .L_06041F40
    bra     .L_0604206E
    mov #0x1, r0
.L_06041F40:
    mov r15, r4
    jsr @r12
    add #0x4, r4
    mov.l @r14, r3
    mov r15, r2
    add #0x4, r2
    mov.b @r2, r1
    mov #0x40, r0
    mov.b r1, @(r0, r3)
    mov.l @r13, r3
    add #0x1, r3
    mov.l r3, @r13
    mov.l @r14, r2
    mov #0x2, r3
    mov.w   .L_const_0x348_w, r0
    mov.l r3, @(r0, r2)
.L_06041F60:
    mov.l @r14, r0
    mov.w   .L_const_0x348_w, r1
    mov.l @(r0, r1), r0
    cmp/eq #0x2, r0
    bf      .L_06041FD4
    mov.l @r14, r7
    mov.w   DAT_06041f96, r0
    mov.l @r14, r6
    mov.l @r14, r5
    mov.l @r14, r4
    mov.l   .L_track_post_collision, r3
    mov.l @(r0, r7), r7
    add #0xC, r0
    mov.l @(r0, r6), r6
    add #-0x4, r0
    mov.l @(r0, r5), r5
    add #-0x4, r0
    jsr @r3
    mov.l @(r0, r4), r4
    mov r0, r4
    tst r4, r4
    bt      .L_06041FB4
    bra     .L_0604206E
    mov #0x1, r0

    .global DAT_06041f90
DAT_06041f90:
    .2byte  0x0338
.L_const_0x348_w:
    .2byte  0x0348

    .global DAT_06041f94
DAT_06041f94:
    .2byte  0x035C

    .global DAT_06041f96
DAT_06041f96:
    .2byte  0x034C
.L_const_0xfff7:
    .4byte  0x0000FFF7
.L_smpc_cmd_helper_b:
    .4byte  smpc_cmd_helper_b
.L_ai_checkpoint_validate_3:
    .4byte  ai_checkpoint_validate
.L_sym_060A5400_2:
    .4byte  sym_060A5400
.L_track_road_edge_b:
    .4byte  track_road_edge_b
.L_track_road_width_main:
    .4byte  track_road_width_main
.L_track_post_collision:
    .4byte  track_post_collision
.L_06041FB4:
    mov r15, r4
    jsr @r12
    add #0x4, r4
    mov.l @r14, r3
    mov r15, r2
    add #0x4, r2
    mov.b @r2, r1
    mov #0x40, r0
    mov.b r1, @(r0, r3)
    mov.l @r13, r3
    add #0x1, r3
    mov.l r3, @r13
    mov.l @r14, r2
    mov #0x3, r3
    mov.w   .L_const_0x348_w_2, r0
    mov.l r3, @(r0, r2)
.L_06041FD4:
    mov.l @r14, r0
    mov.w   .L_const_0x348_w_2, r1
    mov.l @(r0, r1), r0
    cmp/eq #0x3, r0
    bf      .L_06042004
    mov.w   DAT_0604207c, r2
    mov.l   .L_sym_06035C4E_2, r3
    jsr @r3
    mov.l r2, @r15
    mov r0, r4
    mov.l @r15, r3
    exts.w r4, r5
    and r3, r5
    tst r5, r5
    bt      .L_06041FF6
    bra     .L_06041FF8
    mov #0x1, r5
.L_06041FF6:
    mov #0x0, r5
.L_06041FF8:
    tst r5, r5
    bt      .L_06042004
    mov.l @r14, r3
    mov #0x4, r2
    mov.w   .L_const_0x348_w_2, r0
    mov.l r2, @(r0, r3)
.L_06042004:
    mov.l @r14, r0
    mov.w   .L_const_0x348_w_2, r1
    mov.l @(r0, r1), r0
    cmp/eq #0x4, r0
    bf      .L_06042036
    mov.l @r14, r5
    mov.w   DAT_0604207e, r0
    mov.l @r14, r4
    mov.l   .L_track_road_width_main_2, r3
    mov.l @(r0, r5), r5
    add #-0x10, r0
    jsr @r3
    mov.l @(r0, r4), r4
    mov r0, r4
    tst r4, r4
    bt      .L_06042028
    bra     .L_0604206E
    mov #0x1, r0
.L_06042028:
    mov.l @r13, r3
    add #0x1, r3
    mov.l r3, @r13
    mov.l @r14, r2
    mov #0x5, r3
    mov.w   .L_const_0x348_w_2, r0
    mov.l r3, @(r0, r2)
.L_06042036:
    mov.l @r14, r0
    mov.w   .L_const_0x348_w_2, r1
    mov.l @(r0, r1), r0
    cmp/eq #0x5, r0
    bf      .L_06042068
    mov #0x40, r2
    mov.l   .L_sym_06035C4E_2, r3
    jsr @r3
    mov.l r2, @r15
    mov r0, r4
    mov.l @r15, r3
    exts.w r4, r5
    and r3, r5
    tst r5, r5
    bt      .L_06042058
    bra     .L_0604205A
    mov #0x1, r5
.L_06042058:
    mov #0x0, r5
.L_0604205A:
    mov r5, r0
    cmp/eq #0x1, r0
    bf      .L_06042068
    mov.l @r14, r3
    mov #0x0, r2
    mov.w   .L_const_0x348_w_2, r0
    mov.l r2, @(r0, r3)
.L_06042068:
    mov.l @r14, r0
    mov.w   .L_const_0x348_w_2, r1
    mov.l @(r0, r1), r0
.L_0604206E:
    add #0x10, r15
    lds.l @r15+, pr
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14
.L_const_0x348_w_2:
    .2byte  0x0348

    .global DAT_0604207c
DAT_0604207c:
    .2byte  0x0100

    .global DAT_0604207e
DAT_0604207e:
    .2byte  0x035C
.L_sym_06035C4E_2:
    .4byte  sym_06035C4E
.L_track_road_width_main_2:
    .4byte  track_road_width_main
