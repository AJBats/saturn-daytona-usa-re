/* car_physics_init -- Car physics initialization and sprite/object registration
 *
 * Translation unit: 0x0600EC78 - 0x0600F424
 * Auto-generated by tools/generate_l3_tu.py, then annotated.
 *
 * This function initializes the car physics subsystem at race start:
 *   1. Calls vdp1_init to reset sprite hardware
 *   2. Computes car struct base from car_index * 48 (6*8) + array base
 *   3. Stores computed car struct pointers for the active car
 *   4. Dispatches subsystem init calls via indirect fn ptr table (sym_06000344)
 *   5. DMA-transfers car data to its working buffer
 *   6. Registers ~60 VDP1 sprites (slots 0x01-0x39) via vdp1_sprite_render:
 *        r4=slot, r5=size_param, r6=type/flags, r7=cdrom_base+offset
 *   7. Registers ~60 game objects (slots 0x41-0x79) via sym_06007540:
 *        r4=slot, r5=obj_id, r6=flags
 *   8. Registers 3 large VDP1 sprite groups (slots 0x101-0x103, 0x104-0x106)
 *   9. Loads palettes into VDP2 CRAM via memcpy_word_idx
 *  10. Final state init: clears race flags, sets interrupt mask
 *
 * Register allocation:
 *   r14 = vdp1_sprite_render fn ptr (phase 1-3) / memcpy_word_idx (phase 5)
 *   r13 = 0x002A0000 CD-ROM base address
 *   r12 = spr_size_0x108 (phase 1) / obj_register fn ptr (phase 4)
 *   r11 = 0 (constant zero)
 *   r10 = car struct offset (0x00C0) / palette size
 *   r9  = shared size param (varies by phase)
 */
...
car_physics_init:
    ...
    mov.l r8, @-r15
    sts.l pr, @-r15
    add #-0x4, r15                      ! allocate 4 bytes local (palette src tmp)
    mov.l   .L_fn_vdp1_init, r3        ! --- Phase 0: Reset VDP1 sprite hardware ---
    jsr @r3
    nop
    mov.l   .L_car_struct_ptr_b, r4      ! --- Phase 1: Compute car struct pointers ---
    mov.l   .L_car_array_base, r14
    mov.l   .L_car_index, r5
    mov.l @r5, r5                       ! r5 = car_index
    shll2 r5                            ! r5 *= 4
    shll r5                             ! r5 *= 2 (total: index * 8)
    mov.l @r14, r3                      ! r3 = car_array_base
    add r3, r5                          ! r5 = base + index*8
    mov r5, r2
    add #0x40, r2                       ! r2 = base + index*8 + 0x40
    mov.l r2, @r4                       ! store car_struct_ptr_b
    mov.w   .L_car_struct_offset, r10    ! r10 = 0xC0
    mov r2, r3
    add r10, r3                         ! r3 = ptr_b + 0xC0
    mov.l   .L_car_struct_ptr_a, r2
    mov.l r3, @r2                       ! store car_struct_ptr_a
    mov.l   .L_car_struct_array, r4     ! r4 -> 3-element car struct array
    mov r4, r6
    mov.w   DAT_0600ed62, r1           ! r1 = 0x03C0
    add r1, r5                          ! r5 = base + index*8 + 0x3C0
    mov.l r5, @r6                       ! array[0] = r5
    mov r6, r5
    add #0x4, r5
    mov.l @r6, r3
    add #0x20, r3
    mov.l r3, @r5                       ! array[1] = array[0] + 0x20
    mov r3, r2
    add #0x20, r2
    mov.l r2, @(8, r4)                  ! array[2] = array[1] + 0x20
    mov #0x0, r3                        ! --- Phase 2: Init race state ---
    mov.l   .L_race_state_a, r2
    mov.l r3, @r2                       ! race_state_a = 0
    ...
    jsr @r3                             ! dma_transfer(dest=@r14, src, size=0x60)
    mov.l @r14, r4
    mov #0x3, r2
    mov.l   .L_dma_config, r3
    mov #0x0, r11                       ! r11 = 0 (constant zero for rest of fn)
    mov.l r2, @r3                       ! dma_config = 3
    mov r11, r6                         ! r6 = 0 (flags = none)
    mov.l @r14, r2
    mov.l   .L_fp_half, r3
    add #0x60, r2
    mov.w r3, @r2                       ! car_struct[0x60] = 0x8000 (fp 0.5)
    mov.l   .L_vdp1_config_word, r2
    mov.w r11, @r2                      ! vdp1_config = 0
    mov.w   .L_spr_size_0x108, r12      ! r12 = 0x108 (reused size param)
    mov.l   .L_fn_vdp1_sprite, r14      ! r14 = vdp1_sprite_render
    mov.l   .L_cdrom_base, r13          ! r13 = 0x002A0000 (CD-ROM base)
    mov.w   DAT_0600ed6c, r7             ! --- Phase 5: Register VDP1 sprites (slots 1-6) ---
    mov r12, r5                         ! r5 = size param
    add r13, r7                         ! r7 = cdrom_base + offset
    jsr @r14                            ! vdp1_sprite_render(slot=1, size, flags, cdrom_addr)
    mov #0x1, r4
...
.L_spr_size_0xC10:
    .2byte  0x0C10
.L_sprite_reg_phase3:                       ! --- Sprite slots 0x20-0x33 ---
    jsr @r14                            ! complete slot 0x1F from bra above
    nop
...
DAT_0600f036:
    .2byte  0x2304
.L_sprite_reg_phase4:                       ! --- Sprite slots 0x34-0x39 ---
    add r13, r7
    mov #0x14, r6
    mov r12, r5
    jsr @r14
    ...
    jsr @r14
    mov #0x39, r4
    mov #0x1, r3                        ! --- Phase 6: Register game objects (slots 0x41-0x79) ---
    mov r11, r6                         ! r6 = 0 (flags)
    mov.l   .L_sprite_flag, r2
    mov r3, r5                          ! r5 = 1 (obj_id)
    mov.w r3, @r2                       ! sprite_flag = 1
    mov.l   .L_fn_obj_register, r12     ! r12 = obj_register fn
    jsr @r12                            ! obj_register(slot=0x41, obj_id=1, flags=0)
    mov #0x41, r4
    mov r11, r6
    mov #0x2, r5
    jsr @r12
...
.L_obj_reg_final:                           ! --- Phase 8: Extended object reg (0x104-0x106) ---
    ...
    mov.l   .L_fn_memcpy_word, r14      ! r14 = memcpy_word_idx (reuse for all palette copies)
    mov.l r2, @r15                      ! save palette src on stack
    mov.l   .L_vdp2_cram_0x060, r4
    jsr @r14                            ! memcpy_word_idx(cram_0x060, pal_bg, 0xC0)
    mov r2, r5
    mov r10, r6
