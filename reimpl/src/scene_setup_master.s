 ...
 *
 * Master scene initialization — the largest function in the game.
 * Registers all scene subsystem handlers into dispatch tables by
 * repeatedly calling vdp1_sprite_render (r4=slot_id, r5=size,
 * r6=flags, r7=handler_addr) and sym_06007540 (register_handler).
 *
 * Structure:
 *   Phase 1  (slots 0x01-0x0A): Primary VDP1 sprite handler table
 *   Phase 2  (slots 0x10-0x19): Secondary VDP1 handler batch (offset base)
 *   Phase 3  (slots 0x1A-0x24): Tertiary VDP1 handler batch
 *   Phase 4  (slots 0x25-0x2E): Fourth VDP1 handler batch
 *   Phase 5  (slots 0x2F-0x39): Fifth VDP1 handler batch
 *   Phase 6  (slots 0x01-0x16): Second dispatch table (register_handler)
 *   Phase 7  (slots 0x17-0x39): Continuation of second table
 *   Phase 8  (obj layer):       Object layer init + variant-conditional regs
 *   Phase 9  (course switch):   Course-type-specific handler (0/1/2)
 *   Phase 10 (entities):        Entity handler registration + callback copy
 *   Phase 11 (final table):     Final dispatch table + render layer regs
 *   Phase 12 (render layer):    Render pipeline handler registrations
 *   Phase 13 (final batch):     Last batch of scene handler registrations
 */
...
scene_setup_master:
    ...
    mov.w r2, @r3                       ! clear scene state flags
    mov r2, r3
    mov r3, r0
    mov r3, r6
    mov.w r0, @(6, r15)                 ! sp[6] = table_base (0)
    mov r3, r0
    mov.w r0, @(4, r15)                 ! sp[4] = offset_base (0)
    mov.l   .L_fn_handler_slot01, r7    ! r7 = handler address
    mov.w   .L_size_slot01, r5          ! r5 = slot size
    mov.w @(6, r15), r0
    mov.l   .L_fn_vdp1_sprite_render, r3
    mov r0, r4
    extu.w r4, r4
    jsr @r3                             ! register slot 0x1 — Phase 1 start
    add #0x1, r4
    mov #0x0, r6
    ...
    jsr @r3
    add #0xA, r4
    mov.l   .L_fn_handler_phase1_last, r7  ! last Phase 1 handler
    bra     .L_register_phase2           ! continue to Phase 2
    nop
.L_size_slot01:
    .2byte  0x0108
.L_size_slot02:
...
.L_size_slot0A:
    .2byte  0x0A28
.L_scene_state_flags:
    .4byte  sym_06089E44                ! scene state flags (WRAM High)
.L_fn_handler_slot01:
    .4byte  0x002A299B
.L_fn_vdp1_sprite_render:
    .4byte  vdp1_sprite_render          ! dispatch: register scene handler
.L_fn_handler_slot02:
    .4byte  0x002A29A3
.L_fn_handler_slot03:
    .4byte  0x002A2A53
...
.L_fn_handler_phase1_last:
    .4byte  0x002A0640
.L_register_phase2:                              ! --- Phase 2: slots 0x10-0x19 ---
    mov.w @(4, r15), r0
...
.L_fn_handler_slot19_b:
    .4byte  0x002A0F58
.L_register_phase3:                              ! --- Phase 3: slots 0x1A-0x24 ---
    mov.w @(6, r15), r0
...
.L_fn_handler_slot23_c:
    .4byte  0x002A1C04
.L_register_phase4:                              ! --- Phase 4: slots 0x25-0x2E ---
...
.L_fn_handler_slot2F_d:
    .4byte  0x002A211C
.L_register_phase5:                              ! --- Phase 5: slots 0x2F-0x39 ---
...
.L_fn_handler_slot39_e:
    .4byte  0x002A232C
.L_init_second_table:                            ! --- Phase 6: second dispatch table ---
    mov.l   .L_scene_table_idx, r2
    mov #0x1, r3                                 ! table index = 1
    mov #0x0, r6
    mov r3, r5
    mov.w r3, @r2
    mov #0x40, r2
    ...
    .2byte  0xFFFF
.L_scene_table_idx:
    .4byte  sym_06089E46                    ! scene table index (WRAM High)
.L_fn_register_handler:
    .4byte  sym_06007540                    ! handler registration function
.L_register_second_table:                        ! --- Phase 7: second table cont ---
...
.L_fn_register_handler_3:
    .4byte  sym_06007540
.L_init_obj_layer:                               ! --- Phase 8: object layer init ---
    mov.l   .L_fn_register_handler_4, r3
    jsr @r3                                      ! call register_handler (init)
    nop
    mov #0x38, r3
    mov #0x40, r6
    ...
    mov r3, r0
    mov.w r0, @(4, r15)
    mov.l   .L_game_variant_flag, r0        ! check game variant (Saturn mode etc.)
    mov.l @r0, r0
    tst r0, r0
    bt      .L_variant_flag_zero              ! if 0 → use fallback handlers
...
.L_obj_layer_table_base:
    .4byte  sym_06089E9C                    ! object layer table base (WRAM High)
...
.L_fn_obj_handler_slot13:
    .4byte  0x002A3E23
.L_obj_layer_idx:
    .4byte  sym_06089EA0                    ! object layer index (WRAM High)
.L_game_variant_flag:
    .4byte  sym_0607EAB8                    ! game variant flag (WRAM High)
...
.L_fn_variant_handler_3:
    .4byte  0x002A44BF
.L_variant_flag_zero:                            ! variant == 0: use fallback handlers
    ...
    jsr @r3
    add #0x3, r4
.L_check_multiplayer:                            ! --- check multiplayer mode ---
    mov.l   .L_multiplayer_flag, r0
    mov.b @r0, r0                                ! read multiplayer flag byte
    extu.b r0, r0
    tst r0, r0
    bt      .L_single_player_path                ! if 0 → single player path
...
.L_fn_fallback_handler_3:
    .4byte  0x002A463C
.L_multiplayer_flag:
    .4byte  sym_06078635                    ! multiplayer mode flag (WRAM High)
...
.L_fn_multi_handler_7:
    .4byte  0x002A47C4
.L_single_player_path:                           ! multiplayer == 0: single player regs
    ...
    jsr @r3
    add #0xB, r4
.L_after_mode_handlers:                          ! --- Phase 9: course-type dispatch ---
    mov.l   .L_course_type_flag, r0          ! load course type (0/1/2)
...
.L_fn_single_handler_B:
    .4byte  0x002A4AD7
.L_course_type_flag:
    .4byte  sym_0607EAD8                    ! course type (0/1/2) (WRAM High)
...
.L_fn_vdp1_sprite_render_9:
    .4byte  vdp1_sprite_render
.L_course_type_switch:                           ! --- course type dispatch ---
    cmp/eq #0x0, r0
    bt      .L_course_type_0                     ! Three Seven Speedway
    cmp/eq #0x1, r0
    bt      .L_course_type_1                     ! Dinosaur Canyon
    cmp/eq #0x2, r0
    bt      .L_course_type_2                     ! Seaside Street Galaxy
.L_register_entities:                            ! --- Phase 10: entity registration ---
    mov.l   .L_fn_entity_handler_0, r7
    mov.w   .L_size_entity_slot, r5
    mov.w   DAT_0602461e, r4                     ! entity ID 0x0E97
    mov.l   .L_fn_vdp1_sprite_render_10, r3
    jsr @r3
    mov #0x4C, r6
    mov.l   .L_entity_callback_src, r2       ! copy entity callback pointer
    mov.l   .L_entity_callback_dst, r3       ! from source to destination table
    mov.l @r2, r2
    mov.l r2, @r3
    mov r3, r2
    mov.l @r2, r2
...
.L_entity_callback_src:
    .4byte  sym_0606A4F4                    ! entity callback source (WRAM High)
.L_entity_callback_dst:
    .4byte  sym_06085F98                    ! entity callback dest (WRAM High)
...
.L_fn_entity_handler_A:
    .4byte  0x002A8BBB
.L_register_final_table:                         ! --- Phase 11: final dispatch table ---
    mov r5, r4
...
.L_fn_vdp1_sprite_render_11:
    .4byte  vdp1_sprite_render
.L_final_table_idx:
    .4byte  sym_06089E9E                    ! final table index (WRAM High)
.L_fn_final_handler_10:
    .4byte  0x002A7D76
.L_fn_final_handler_11:
    .4byte  0x002A7D8E
.L_register_render_layer:                        ! --- Phase 12: render layer ---
...
.L_fn_render_handler_1C:
    .4byte  0x002A7F0A
.L_register_final_batch:                         ! --- Phase 13: final batch ---
    mov.w @(6, r15), r0
