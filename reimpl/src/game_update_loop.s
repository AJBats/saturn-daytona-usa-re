/* game_update_loop -- L3 assembly (SH-2 mnemonics)
 * Translation unit: 0x06033BC8 - 0x06034E20
 * Auto-generated by tools/generate_l3_tu.py
 *
 * This large TU contains camera initialization, display element dispatch,
 * the physics velocity integration pipeline, and the full AI subsystem.
 * Most functions operate on a per-car data struct accessed via r14 (base ptr)
 * with field offsets loaded from constant pools.
 *
 * Function groups:
 *
 * Camera & Display:
 *   camera_attract_init()         — camera setup during attract mode
 *   disp_timeext_digit_0..2       — time extension digit renderers
 *   time_extend_digits()          — time extension digit group
 *   disp_sel_thunk_00..18         — selector display dispatch (index 0-18)
 *   selector_group_render()       — selector group rendering
 *   disp_selext_thunk_00..18      — extended selector dispatch
 *   score_display_render()        — score display setup
 *   disp_score_renderer()         — score digit rendering
 *   bonus_points_display()        — bonus score display
 *   display_frame_mgr()           — per-frame display management
 *   display_frame_flush()         — frame buffer flush
 *   disp_end_stub_0..9            — display termination stubs
 *
 * Physics (car velocity integration pipeline, r14=car struct base):
 *   phys_position_load()          — load position, multiply by heading cos
 *   phys_velocity_integrate()     — integrate velocity via sin(heading)
 *   phys_result_store()           — store intermediate physics result
 *   phys_perspective()            — perspective divide (fpdiv_setup)
 *   phys_final_integrate()        — final velocity integration step
 *   phys_lighting_setup()         — scene lighting from physics state
 *   ai_steering_response()        — AI steering result writeback
 *
 * AI Velocity (per-axis integration chains):
 *   ai_vel_x_entry..cleanup_c     — X-axis velocity integration + matrix push
 *   xaxis_integrate_damp()        — X-axis with damping
 *   yaxis_integrate()             — Y-axis velocity integration
 *   zaxis_integrate()             — Z-axis velocity integration
 *   ai_rot_a_entry..fix_c         — rotation axis A integration
 *   ai_rot_b_entry..fix_c         — rotation axis B integration
 *
 * AI Decision & Pathfinding:
 *   ai_decision_dispatch()        — main AI per-frame dispatch
 *   sym_0603446C                  — AI car struct field clear loop
 *   ai_speed_limit()              — speed limiting via isqrt distance
 *   ai_waypoint_follower()        — waypoint proximity detection
 *   ai_collision_trigger/response — collision velocity updates
 *   ai_car_pos_init()             — initial car position setup
 *   ai_car_full_init()            — full AI car initialization
 *   ai_spawn_helper_a..d          — AI spawn sub-functions
 *
 * Ranking & Checkpoint:
 *   position_calc_current()       — calculate current race position
 *   ranking_system_full()         — full ranking recalculation
 *   lap_complete_check()          — lap completion detection
 *   checkpoint_validate()         — checkpoint boundary validation
 *   section_transition()          — track section transitions
 *   ai_checkpoint_section/validate — AI checkpoint handling
 *   ai_section_transition/update  — AI section tracking
 *
 * Track & Recovery:
 *   track_intersect_test()        — track boundary intersection
 *   mesh_boundary_check()         — mesh bounds testing
 *   ai_crash_recovery()           — crash recovery AI
 *   ai_drafting_calc()            — drafting/slipstream calculation
 *   ai_brake_zone_calc/adjust/main — brake zone management
 *   ai_throttle_modulate/mod/adjust — throttle control
 *   ai_recovery_handler()         — full recovery sequence
 *
 * Car data struct (r14-relative offsets from pool constants):
 *   +0x01 = type/mode byte
 *   +0x02 = heading angle (word)
 *   +0x04 = position X (long, 16.16 FP)
 *   +0x08 = position Z (long, 16.16 FP)
 *   +0x0C = position Y (long, 16.16 FP)
 *   +0x14 = velocity component A
 *   +0x18 = velocity component B
 *   +0x1C = velocity component C
 *   +0x20 = heading/angle field
 *   +0x24 = steering word
 *   +0x26 = AI status byte
 *   +0x28 = AI waypoint word
 *   +0x2C = struct stride (44 bytes per car, 5 cars total)
 */

    .section .text.FUN_06033BC8


    .global camera_attract_init
    .type camera_attract_init, @function
/* camera_attract_init() — initialize camera for attract mode demo playback.
 * Reads vehicle struct field at offset +0x1EC via sym_0607E944 base pointer,
 * checks if value is in range [0xDC, 0xEC]. If in range, renders time
 * extension digits (0-2) then score display. Otherwise calls sym_0603446C.
 * Uses r14 as persistent base pointer for remaining physics/AI chain.
 */
camera_attract_init:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov.l r9, @-r15
    mov.l r8, @-r15
    mov.l   .L_sym_0607E944, r0
    mov.l @r0, r0
    mov.l   .L_offset_0x1ec, r1
    mov.l @(r0, r1), r1
    mov.l   .L_offset_0x0dc, r0
    cmp/ge r0, r1
    bf      .L_06033C00
    mov.l   .L_offset_0x0ec, r0
    cmp/gt r0, r1
    bt      .L_06033C00
    bra     .L_06033C02
    sett
    .2byte  0x0000
.L_sym_0607E944:
    .4byte  sym_0607E944
.L_offset_0x1ec:
    .4byte  0x000001EC
.L_offset_0x0dc:
    .4byte  0x000000DC
.L_offset_0x0ec:
    .4byte  0x000000EC
.L_06033C00:
    clrt
.L_06033C02:
    bf      .L_06033C30
    mov #0x0, r4
    mov.l   .L_sym_06033F54, r0

    .global disp_timeext_digit_0
    .type disp_timeext_digit_0, @function
disp_timeext_digit_0:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov #0x1, r4
    mov.l   .L_sym_06033F54, r0

    .global disp_timeext_digit_1
    .type disp_timeext_digit_1, @function
disp_timeext_digit_1:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov #0x2, r4
    mov.l   .L_sym_06033F54, r0

    .global disp_timeext_digit_2
    .type disp_timeext_digit_2, @function
disp_timeext_digit_2:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    bra     .L_06033C3A
    nop
.L_sym_06033F54:
    .4byte  sym_06033F54
.L_06033C30:
    mov.l   .L_sym_0603446C, r0

    .global time_extend_digits
    .type time_extend_digits, @function
time_extend_digits:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
.L_06033C3A:
    mov.l   .L_sym_0607E944_2, r0
    mov.l @r0, r0
    mov.l   .L_offset_0x1ec_2, r1
    mov.l @(r0, r1), r1
    mov.l   .L_const_0x0ed, r0
    cmp/ge r0, r1
    bf      .L_06033C68
    mov.l   .L_offset_0x104, r0
    cmp/gt r0, r1
    bt      .L_06033C68
    bra     .L_06033C6A
    sett
    .2byte  0x0000
.L_sym_0603446C:
    .4byte  sym_0603446C
.L_sym_0607E944_2:
    .4byte  sym_0607E944
.L_offset_0x1ec_2:
    .4byte  0x000001EC
.L_const_0x0ed:
    .4byte  0x000000ED
.L_offset_0x104:
    .4byte  0x00000104
.L_06033C68:
    clrt
.L_06033C6A:
    bf      .L_06033D34
    mov #0x5, r4

    .global disp_sel_thunk_00
    .type disp_sel_thunk_00, @function
disp_sel_thunk_00:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x6, r4

    .global disp_sel_thunk_01
    .type disp_sel_thunk_01, @function
disp_sel_thunk_01:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x7, r4

    .global disp_sel_thunk_02
    .type disp_sel_thunk_02, @function
disp_sel_thunk_02:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x8, r4

    .global disp_sel_thunk_03
    .type disp_sel_thunk_03, @function
disp_sel_thunk_03:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x9, r4

    .global disp_sel_thunk_04
    .type disp_sel_thunk_04, @function
disp_sel_thunk_04:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0xA, r4

    .global disp_sel_thunk_05
    .type disp_sel_thunk_05, @function
disp_sel_thunk_05:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0xB, r4

    .global disp_sel_thunk_06
    .type disp_sel_thunk_06, @function
disp_sel_thunk_06:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0xC, r4

    .global disp_sel_thunk_07
    .type disp_sel_thunk_07, @function
disp_sel_thunk_07:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0xD, r4

    .global disp_sel_thunk_08
    .type disp_sel_thunk_08, @function
disp_sel_thunk_08:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0xE, r4

    .global disp_sel_thunk_09
    .type disp_sel_thunk_09, @function
disp_sel_thunk_09:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0xF, r4

    .global disp_sel_thunk_10
    .type disp_sel_thunk_10, @function
disp_sel_thunk_10:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x10, r4

    .global disp_sel_thunk_11
    .type disp_sel_thunk_11, @function
disp_sel_thunk_11:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x11, r4

    .global disp_sel_thunk_12
    .type disp_sel_thunk_12, @function
disp_sel_thunk_12:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x12, r4

    .global disp_sel_thunk_13
    .type disp_sel_thunk_13, @function
disp_sel_thunk_13:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x13, r4

    .global disp_sel_thunk_14
    .type disp_sel_thunk_14, @function
disp_sel_thunk_14:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x14, r4

    .global disp_sel_thunk_15
    .type disp_sel_thunk_15, @function
disp_sel_thunk_15:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x15, r4

    .global disp_sel_thunk_16
    .type disp_sel_thunk_16, @function
disp_sel_thunk_16:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x16, r4

    .global disp_sel_thunk_17
    .type disp_sel_thunk_17, @function
disp_sel_thunk_17:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x17, r4

    .global disp_sel_thunk_18
    .type disp_sel_thunk_18, @function
disp_sel_thunk_18:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x18, r4

    .global selector_group_render
    .type selector_group_render, @function
selector_group_render:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
.L_06033D34:
    mov.l   .L_sym_0607E944_3, r0
    mov.l @r0, r0
    mov.l   .L_offset_0x1ec_3, r1
    mov.l @(r0, r1), r1
    mov.l   .L_offset_0x1b8, r0
    cmp/ge r0, r1
    bf      .L_06033D5C
    mov.l   .L_offset_0x1e0, r0
    cmp/gt r0, r1
    bt      .L_06033D5C
    bra     .L_06033D5E
    sett
.L_sym_0607E944_3:
    .4byte  sym_0607E944
.L_offset_0x1ec_3:
    .4byte  0x000001EC
.L_offset_0x1b8:
    .4byte  0x000001B8
.L_offset_0x1e0:
    .4byte  0x000001E0
.L_06033D5C:
    clrt
.L_06033D5E:
    bt      .L_06033D64
    bra     .L_06033E8C
    nop
.L_06033D64:
    mov.l   .L_sym_0607EAE0, r0
    mov.l @r0, r0
    tst r0, r0
    bf      .L_06033D74
    bra     .L_06033E92
    nop
.L_sym_0607EAE0:
    .4byte  sym_0607EAE0
.L_06033D74:
    mov.l   .L_const_0x019, r4

    .global disp_selext_thunk_00
    .type disp_selext_thunk_00, @function
disp_selext_thunk_00:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06083250, r0
    mov.l @r0, r13
    mov.l   .L_const_0x13b, r0
    cmp/ge r0, r13
    bf      .L_06033E52
    mov.l   .L_const_0x01a, r4

    .global disp_selext_thunk_01
    .type disp_selext_thunk_01, @function
disp_selext_thunk_01:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x01b, r4

    .global disp_selext_thunk_02
    .type disp_selext_thunk_02, @function
disp_selext_thunk_02:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov.l   .L_offset_0x01c, r4

    .global disp_selext_thunk_03
    .type disp_selext_thunk_03, @function
disp_selext_thunk_03:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x01d, r4

    .global disp_selext_thunk_04
    .type disp_selext_thunk_04, @function
disp_selext_thunk_04:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x276, r0
    cmp/ge r0, r13
    bf      .L_06033E52
    mov #0x1E, r4

    .global disp_selext_thunk_05
    .type disp_selext_thunk_05, @function
disp_selext_thunk_05:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x1F, r4

    .global disp_selext_thunk_06
    .type disp_selext_thunk_06, @function
disp_selext_thunk_06:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x20, r4

    .global disp_selext_thunk_07
    .type disp_selext_thunk_07, @function
disp_selext_thunk_07:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x21, r4

    .global disp_selext_thunk_08
    .type disp_selext_thunk_08, @function
disp_selext_thunk_08:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x22, r4

    .global disp_selext_thunk_09
    .type disp_selext_thunk_09, @function
disp_selext_thunk_09:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x3b1, r0
    cmp/ge r0, r13
    bf      .L_06033E52
    mov #0x23, r4

    .global disp_selext_thunk_10
    .type disp_selext_thunk_10, @function
disp_selext_thunk_10:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x24, r4

    .global disp_selext_thunk_11
    .type disp_selext_thunk_11, @function
disp_selext_thunk_11:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x25, r4

    .global disp_selext_thunk_12
    .type disp_selext_thunk_12, @function
disp_selext_thunk_12:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x26, r4

    .global disp_selext_thunk_13
    .type disp_selext_thunk_13, @function
disp_selext_thunk_13:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x27, r4

    .global disp_selext_thunk_14
    .type disp_selext_thunk_14, @function
disp_selext_thunk_14:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x28, r4

    .global disp_selext_thunk_15
    .type disp_selext_thunk_15, @function
disp_selext_thunk_15:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x29, r4

    .global disp_selext_thunk_16
    .type disp_selext_thunk_16, @function
disp_selext_thunk_16:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x2A, r4

    .global disp_selext_thunk_17
    .type disp_selext_thunk_17, @function
disp_selext_thunk_17:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x2B, r4

    .global disp_selext_thunk_18
    .type disp_selext_thunk_18, @function
disp_selext_thunk_18:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov #0x2C, r4

    .global score_display_render
    .type score_display_render, @function
score_display_render:
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
.L_06033E52:
    mov.l   .L_sym_06083250, r1
    mov.l @r1, r0
    add #0x1, r0
    mov.l   .L_offset_0xec4, r2
    cmp/ge r2, r0
    bf      .L_06033E60
    mov.l   .L_offset_0xec4, r0
.L_06033E60:
    bra     .L_06033E92
    mov.l r0, @r1
.L_const_0x019:
    .4byte  0x00000019
.L_sym_06083250:
    .4byte  sym_06083250
.L_const_0x13b:
    .4byte  0x0000013B
.L_const_0x01a:
    .4byte  0x0000001A
.L_const_0x01b:
    .4byte  0x0000001B
.L_offset_0x01c:
    .4byte  0x0000001C
.L_const_0x01d:
    .4byte  0x0000001D
.L_const_0x276:
    .4byte  0x00000276
.L_const_0x3b1:
    .4byte  0x000003B1
.L_offset_0xec4:
    .4byte  0x00000EC4
.L_06033E8C:
    xor r0, r0
    mov.l   .L_sym_06083250_2, r1
    mov.l r0, @r1
.L_06033E92:
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    mov.l @r15+, r14
    rts
    nop
.L_sym_06083250_2:
    .4byte  sym_06083250

    .global disp_score_renderer
    .type disp_score_renderer, @function
disp_score_renderer:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov.l r9, @-r15
    mov.l r8, @-r15
    mov.l   .L_sym_06063E08, r0
    mov.w @r0, r0
    cmp/eq #0x1, r0
    bf      .L_06033EC6
    mov.l   .L_sym_06083254, r1
    mov.b @r1, r0
    add #0x1, r0
    mov.b r0, @r1
.L_06033EC6:
    mov.l   .L_sym_06083254, r0
    mov.b @r0, r0
    cmp/eq #0x1, r0
    bf      .L_06033EFE
    mov.l   .L_zero, r4
    mov.l   .L_sym_06033F54_2, r0

    .global disp_score_digit_0
    .type disp_score_digit_0, @function
disp_score_digit_0:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x001, r4
    mov.l   .L_sym_06033F54_2, r0

    .global disp_score_digit_1
    .type disp_score_digit_1, @function
disp_score_digit_1:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x002, r4
    mov.l   .L_sym_06033F54_2, r0

    .global disp_score_digit_2
    .type disp_score_digit_2, @function
disp_score_digit_2:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_offset_0x004, r4
    mov.l   .L_sym_06033F54_2, r0

    .global bonus_points_display
    .type bonus_points_display, @function
bonus_points_display:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
.L_06033EFE:
    mov.l   .L_sym_06083254, r0
    mov.b @r0, r0
    cmp/eq #0x2, r0
    bf      .L_06033F1C
    mov.l   .L_const_0x006, r4
.L_06033F08:
    mov.l r4, @-r15
    sts.l pr, @-r15
    bsr     sym_06033F54
    nop
    lds.l @r15+, pr
    mov.l @r15+, r4
    add #0x1, r4
    mov.l   .L_const_0x019_2, r0
    cmp/ge r0, r4
    bf      .L_06033F08
.L_06033F1C:
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    mov.l @r15+, r14
    rts
    nop
    .2byte  0x0000
.L_sym_06063E08:
    .4byte  sym_06063E08
.L_sym_06083254:
    .4byte  sym_06083254
.L_zero:
    .4byte  0x00000000
.L_sym_06033F54_2:
    .4byte  sym_06033F54
.L_const_0x001:
    .4byte  0x00000001
.L_const_0x002:
    .4byte  0x00000002
.L_offset_0x004:
    .4byte  0x00000004
.L_const_0x006:
    .4byte  0x00000006
.L_const_0x019_2:
    .4byte  0x00000019

    .global sym_06033F54
sym_06033F54:
    mov.l r13, @-r15
    mov.l   .L_offset_0x02c, r1
    mulu.w r4, r1
    sts macl, r0
    mov.l   .L_DAT_06082A7C, r14
    add r0, r14
    mov.l   .L_sym_0607E944_4, r0
    mov.l @r0, r10
    mov.l   .L_const_0x001_2, r0
    mov.b @(r0, r14), r0
    cmp/eq #0x1, r0
    bt      disp_end_stub_0
    cmp/eq #0x4, r0
    bt      disp_end_stub_0
    mov.l   .L_const_0x026, r0
    mov.b @(r0, r14), r0
    cmp/eq #0x0, r0
    bt      disp_end_stub_2
    cmp/eq #0x1, r0
    bt      disp_end_stub_6
    cmp/eq #0x2, r0
    bt      disp_end_stub_7

    .global disp_end_stub_0
    .type disp_end_stub_0, @function
disp_end_stub_0:
    sts.l pr, @-r15
    bsr     .L_06034000
    nop
    lds.l @r15+, pr

    .global disp_end_stub_1
    .type disp_end_stub_1, @function
disp_end_stub_1:
    sts.l pr, @-r15
    bsr     .L_06034168
    nop
    lds.l @r15+, pr

    .global display_frame_mgr
    .type display_frame_mgr, @function
display_frame_mgr:
    sts.l pr, @-r15
    bsr     .L_060346C0
    nop
    lds.l @r15+, pr
    bra     .L_06033FFC
    nop
.L_offset_0x02c:
    .4byte  0x0000002C
.L_DAT_06082A7C:
    .4byte  DAT_06082A7C
.L_sym_0607E944_4:
    .4byte  sym_0607E944
.L_const_0x001_2:
    .4byte  0x00000001
.L_const_0x026:
    .4byte  0x00000026

    .global disp_end_stub_2
    .type disp_end_stub_2, @function
disp_end_stub_2:
    sts.l pr, @-r15
    bsr     .L_0603449C
    nop
    lds.l @r15+, pr

    .global disp_end_stub_3
    .type disp_end_stub_3, @function
disp_end_stub_3:
    sts.l pr, @-r15
    bsr     .L_06034000
    nop
    lds.l @r15+, pr

    .global disp_end_stub_4
    .type disp_end_stub_4, @function
disp_end_stub_4:
    sts.l pr, @-r15
    bsr     .L_06034168
    nop
    lds.l @r15+, pr

    .global disp_end_stub_5
    .type disp_end_stub_5, @function
disp_end_stub_5:
    sts.l pr, @-r15
    bsr     .L_060346C0
    nop
    lds.l @r15+, pr
    bra     .L_06033FFC
    nop

    .global disp_end_stub_6
    .type disp_end_stub_6, @function
disp_end_stub_6:
    sts.l pr, @-r15
    bsr     .L_06034560
    nop
    lds.l @r15+, pr

    .global disp_end_stub_7
    .type disp_end_stub_7, @function
disp_end_stub_7:
    sts.l pr, @-r15
    bsr     .L_0603449C
    nop
    lds.l @r15+, pr

    .global disp_end_stub_8
    .type disp_end_stub_8, @function
disp_end_stub_8:
    sts.l pr, @-r15
    bsr     .L_0603458C
    nop
    lds.l @r15+, pr

    .global disp_end_stub_9
    .type disp_end_stub_9, @function
disp_end_stub_9:
    sts.l pr, @-r15
    bsr     .L_06034640
    nop
    lds.l @r15+, pr

    .global display_frame_flush
    .type display_frame_flush, @function
display_frame_flush:
    sts.l pr, @-r15
    bsr     .L_060346C0
    nop
    lds.l @r15+, pr
.L_06033FFC:
    rts
    mov.l @r15+, r13
.L_06034000:
    mov.l   .L_const_0x001_3, r0
    mov.b @(r0, r14), r0
    cmp/eq #0x3, r0
    bf      .L_06034024
    mov.l   .L_const_0x012, r0
    mov.w @(r0, r14), r1
    exts.w r1, r1
    mov.l   .L_offset_0x01c_2, r0
    mov.l @(r0, r14), r2
    add r1, r2
    rts
    mov.l r2, @(r0, r14)
.L_const_0x001_3:
    .4byte  0x00000001
.L_const_0x012:
    .4byte  0x00000012
.L_offset_0x01c_2:
    .4byte  0x0000001C
.L_06034024:
    mov.l   .L_const_0x012_2, r0
    mov.w @(r0, r14), r2
    mov.l   .L_const_0x002_2, r0
    mov.w @(r0, r14), r1
    add r2, r1
    mov.w r1, @(r0, r14)
    mov r1, r13
    mov r13, r4
    mov.l   .L_cos_lookup, r0

    .global phys_position_load
    .type phys_position_load, @function
/* phys_position_load() — load car heading, compute cos(heading) * speed.
 * Reads heading (+0x20) and velocity (+0x14), multiplies via dmuls.l/xtrct,
 * adds to position (+0x04), then calls sin_lookup for next stage.
 * Part of the physics integration pipeline (cos → sin → perspective → store).
 */
phys_position_load:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov r0, r12
    mov.l   .L_offset_0x020, r0
    mov.l @(r0, r14), r11
    dmuls.l r12, r11
    sts mach, r0
    sts macl, r1
    xtrct r0, r1
    mov.l   .L_offset_0x014, r0
    mov.l @(r0, r14), r0
    add r0, r1
    mov.l   .L_offset_0x004_2, r0
    mov.l r1, @(r0, r14)
    mov r13, r4
    mov.l   .L_sin_lookup, r0

    .global phys_velocity_integrate
    .type phys_velocity_integrate, @function
/* phys_velocity_integrate() — sin(heading) * speed, integrate into position.
 * Continues from phys_position_load. Multiplies sin result by speed (r11),
 * adds to Y position (+0x0C). If car mode byte (+0x01) == 2, branches to
 * special lighting path; otherwise handles steering subdivision.
 */
phys_velocity_integrate:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    dmuls.l r0, r11
    sts mach, r0
    sts macl, r1
    xtrct r0, r1
    mov.l   .L_offset_0x01c_3, r0
    mov.l @(r0, r14), r0
    add r0, r1
    mov.l   .L_offset_0x00c, r0
    mov.l r1, @(r0, r14)
    mov.l   .L_const_0x001_4, r0
    mov.b @(r0, r14), r0
    cmp/eq #0x2, r0
    bt      .L_0603410C
    mov.l   .L_offset_0x024, r0
    mov.w @(r0, r14), r1
    tst r1, r1
    bf      .L_060340B4
    bra     .L_060340EC
    xor r2, r2
.L_const_0x012_2:
    .4byte  0x00000012
.L_const_0x002_2:
    .4byte  0x00000002
.L_cos_lookup:
    .4byte  cos_lookup
.L_offset_0x020:
    .4byte  0x00000020          /* offset: heading/angle */
.L_offset_0x014:
    .4byte  0x00000014          /* offset: velocity A */
.L_offset_0x004_2:
    .4byte  0x00000004          /* offset: position X */
.L_sin_lookup:
    .4byte  sin_lookup
.L_offset_0x01c_3:
    .4byte  0x0000001C          /* offset: velocity C */
.L_offset_0x00c:
    .4byte  0x0000000C          /* offset: position Y */
.L_const_0x001_4:
    .4byte  0x00000001          /* offset: type/mode byte */
.L_offset_0x024:
    .4byte  0x00000024          /* offset: steering word */
.L_060340B4:
    dmuls.l r12, r1
    sts mach, r0
    sts macl, r10
    xtrct r0, r10
    mov r10, r4
    mov.l   .L_sin_lookup_2, r0

    .global phys_result_store
    .type phys_result_store, @function
phys_result_store:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov r0, r9
    mov r10, r4
    mov.l   .L_cos_lookup_2, r0

    .global phys_perspective
    .type phys_perspective, @function
phys_perspective:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov r0, r5
    mov r9, r4
    mov.l   .L_fpdiv_setup, r0

    .global phys_final_integrate
    .type phys_final_integrate, @function
phys_final_integrate:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    dmuls.l r0, r11
    sts mach, r0
    sts macl, r2
    xtrct r0, r2
.L_060340EC:
    mov.l   .L_offset_0x018, r0
    mov.l @(r0, r14), r1
    add r1, r2
    mov.l   .L_offset_0x008, r0
    bra     .L_0603413E
    mov.l r2, @(r0, r14)
.L_sin_lookup_2:
    .4byte  sin_lookup
.L_cos_lookup_2:
    .4byte  cos_lookup
.L_fpdiv_setup:
    .4byte  fpdiv_setup
.L_offset_0x018:
    .4byte  0x00000018
.L_offset_0x008:
    .4byte  0x00000008
.L_0603410C:
    mov.l   .L_offset_0x004_3, r0
    mov.l @(r0, r14), r4
    mov.l   .L_sym_06083238, r0
    mov.l r4, @r0
    mov.l   .L_offset_0x00c_2, r0
    mov.l @(r0, r14), r5
    mov.l   .L_sym_06083240, r0
    mov.l r5, @r0
    mov.l   .L_sym_06006838, r0

    .global phys_lighting_setup
    .type phys_lighting_setup, @function
phys_lighting_setup:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov r0, r4
    mov.l   .L_sym_06083238, r5
    mov.l   .L_sym_06083244, r6
    mov.l   .L_scene_render_alt, r0

    .global ai_steering_response
    .type ai_steering_response, @function
ai_steering_response:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_0608323C, r0
    mov.l @r0, r1
    mov.l   .L_offset_0x008_2, r0
    mov.l r1, @(r0, r14)
.L_0603413E:
    rts
    nop
    .2byte  0x0000
.L_offset_0x004_3:
    .4byte  0x00000004
.L_sym_06083238:
    .4byte  sym_06083238
.L_offset_0x00c_2:
    .4byte  0x0000000C
.L_sym_06083240:
    .4byte  sym_06083240
.L_sym_06006838:
    .4byte  sym_06006838
.L_sym_06083244:
    .4byte  sym_06083244
.L_scene_render_alt:
    .4byte  scene_render_alt
.L_sym_0608323C:
    .4byte  sym_0608323C
.L_offset_0x008_2:
    .4byte  0x00000008
.L_06034168:
    mov.l   .L_const_0x001_5, r0
    mov.b @(r0, r14), r0
    cmp/eq #0x1, r0
    bf      .L_060341F4
    mov.l   .L_sym_06026DBC, r0

    .global ai_vel_x_entry
    .type ai_vel_x_entry, @function
ai_vel_x_entry:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC, r13
    mov.l @r13, r13
    mov.l   .L_offset_0x004_4, r0
    mov.l @(r0, r14), r4
    mov.l   .L_offset_0x008_3, r0
    mov.l @(r0, r14), r5
    mov.l   .L_offset_0x00c_3, r0
    mov.l @(r0, r14), r6
    mov.l   .L_sym_06026E2E, r0

    .global ai_vel_x_step_a
    .type ai_vel_x_step_a, @function
ai_vel_x_step_a:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x002_3, r0
    mov.w @(r0, r14), r4
    mov.l   .L_mat_rot_y, r0

    .global ai_vel_x_step_b
    .type ai_vel_x_step_b, @function
ai_vel_x_step_b:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x011, r0
    mov.b @(r0, r14), r4
    mov.l   .L_camera_param_load, r0

    .global ai_vel_x_step_c
    .type ai_vel_x_step_c, @function
ai_vel_x_step_c:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06026DF8, r0

    .global xaxis_integrate_damp
    .type xaxis_integrate_damp, @function
xaxis_integrate_damp:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC, r13
    mov.l @r13, r13
    bra     .L_06034468
    nop
    .2byte  0x0000
.L_const_0x001_5:
    .4byte  0x00000001
.L_sym_06026DBC:
    .4byte  sym_06026DBC
.L_sym_06089EDC:
    .4byte  sym_06089EDC
.L_offset_0x004_4:
    .4byte  0x00000004
.L_offset_0x008_3:
    .4byte  0x00000008
.L_offset_0x00c_3:
    .4byte  0x0000000C
.L_sym_06026E2E:
    .4byte  sym_06026E2E
.L_const_0x002_3:
    .4byte  0x00000002
.L_mat_rot_y:
    .4byte  mat_rot_y
.L_const_0x011:
    .4byte  0x00000011
.L_camera_param_load:
    .4byte  camera_param_load
.L_sym_06026DF8:
    .4byte  sym_06026DF8
.L_060341F4:
    cmp/eq #0x2, r0
    bf      .L_06034280
    mov.l   .L_sym_06026DBC_2, r0

    .global ai_vel_x_cleanup_entry
    .type ai_vel_x_cleanup_entry, @function
ai_vel_x_cleanup_entry:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_2, r13
    mov.l @r13, r13
    mov.l   .L_offset_0x004_5, r0
    mov.l @(r0, r14), r4
    mov.l   .L_offset_0x008_4, r0
    mov.l @(r0, r14), r5
    mov.l   .L_offset_0x00c_4, r0
    mov.l @(r0, r14), r6
    mov.l   .L_sym_06026E2E_2, r0

    .global ai_vel_x_cleanup_a
    .type ai_vel_x_cleanup_a, @function
ai_vel_x_cleanup_a:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x002_4, r0
    mov.w @(r0, r14), r4
    mov.l   .L_fp_half, r0
    add r0, r4
    exts.w r4, r4
    mov.l   .L_mat_rot_y_2, r0

    .global ai_vel_x_cleanup_b
    .type ai_vel_x_cleanup_b, @function
ai_vel_x_cleanup_b:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x011_2, r0
    mov.b @(r0, r14), r4
    mov.l   .L_camera_scene_setup, r0

    .global ai_vel_x_cleanup_c
    .type ai_vel_x_cleanup_c, @function
ai_vel_x_cleanup_c:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06026DF8_2, r0

    .global yaxis_integrate
    .type yaxis_integrate, @function
yaxis_integrate:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_2, r13
    mov.l @r13, r13
    bra     .L_06034468
    nop
.L_sym_06026DBC_2:
    .4byte  sym_06026DBC
.L_sym_06089EDC_2:
    .4byte  sym_06089EDC
.L_offset_0x004_5:
    .4byte  0x00000004
.L_offset_0x008_4:
    .4byte  0x00000008
.L_offset_0x00c_4:
    .4byte  0x0000000C
.L_sym_06026E2E_2:
    .4byte  sym_06026E2E
.L_const_0x002_4:
    .4byte  0x00000002
.L_fp_half:
    .4byte  0x00008000
.L_mat_rot_y_2:
    .4byte  mat_rot_y
.L_const_0x011_2:
    .4byte  0x00000011
.L_camera_scene_setup:
    .4byte  camera_scene_setup
.L_sym_06026DF8_2:
    .4byte  sym_06026DF8
.L_06034280:
    cmp/eq #0x3, r0
    bf      .L_06034304
    mov.l   .L_sym_06026DBC_3, r0

    .global ai_vel_y_entry
    .type ai_vel_y_entry, @function
ai_vel_y_entry:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_3, r13
    mov.l @r13, r13
    mov.l   .L_offset_0x014_2, r0
    mov.l @(r0, r14), r4
    mov.l   .L_offset_0x018_2, r0
    mov.l @(r0, r14), r5
    mov.l   .L_offset_0x01c_4, r0
    mov.l @(r0, r14), r6
    mov.l   .L_sym_06026E2E_3, r0

    .global ai_vel_y_step_a
    .type ai_vel_y_step_a, @function
ai_vel_y_step_a:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x002_5, r0
    mov.w @(r0, r14), r4
    mov.l   .L_mat_rot_y_3, r0

    .global ai_vel_y_step_b
    .type ai_vel_y_step_b, @function
ai_vel_y_step_b:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x011_3, r0
    mov.b @(r0, r14), r4
    mov.l   .L_camera_scene_setup_2, r0

    .global ai_vel_y_step_c
    .type ai_vel_y_step_c, @function
ai_vel_y_step_c:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06026DF8_3, r0

    .global zaxis_integrate
    .type zaxis_integrate, @function
zaxis_integrate:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_3, r13
    mov.l @r13, r13
    bra     .L_06034468
    nop
    .2byte  0x0000
.L_sym_06026DBC_3:
    .4byte  sym_06026DBC
.L_sym_06089EDC_3:
    .4byte  sym_06089EDC
.L_offset_0x014_2:
    .4byte  0x00000014
.L_offset_0x018_2:
    .4byte  0x00000018
.L_offset_0x01c_4:
    .4byte  0x0000001C
.L_sym_06026E2E_3:
    .4byte  sym_06026E2E
.L_const_0x002_5:
    .4byte  0x00000002
.L_mat_rot_y_3:
    .4byte  mat_rot_y
.L_const_0x011_3:
    .4byte  0x00000011
.L_camera_scene_setup_2:
    .4byte  camera_scene_setup
.L_sym_06026DF8_3:
    .4byte  sym_06026DF8
.L_06034304:
    cmp/eq #0x4, r0
    bt      .L_0603430C
    bra     .L_06034468
    nop
.L_0603430C:
    mov.l   .L_sym_06026DBC_4, r0

    .global ai_vel_z_entry
    .type ai_vel_z_entry, @function
ai_vel_z_entry:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_4, r13
    mov.l @r13, r13
    mov.l   .L_offset_0x004_6, r0
    mov.l @(r0, r14), r4
    mov.l   .L_offset_0x008_5, r0
    mov.l @(r0, r14), r5
    mov.l   .L_offset_0x00c_5, r0
    mov.l @(r0, r14), r6
    mov.l   .L_sym_06026E2E_4, r0

    .global ai_vel_z_step_a
    .type ai_vel_z_step_a, @function
ai_vel_z_step_a:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x002_6, r0
    mov.w @(r0, r14), r4
    mov.l   .L_mat_rot_y_4, r0

    .global ai_vel_z_step_b
    .type ai_vel_z_step_b, @function
ai_vel_z_step_b:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x011_4, r0
    mov.b @(r0, r14), r4
    mov.l   .L_camera_param_load_2, r0

    .global ai_vel_z_step_c
    .type ai_vel_z_step_c, @function
ai_vel_z_step_c:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06026DF8_4, r0

    .global ai_rot_a_entry
    .type ai_rot_a_entry, @function
ai_rot_a_entry:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_4, r13
    mov.l @r13, r13
    mov.l   .L_sym_06083250_3, r0
    mov.l @r0, r1
    mov.l   .L_offset_0x4ec, r0
    cmp/ge r0, r1
    bf      .L_0603441C
    mov.l   .L_sym_06026DBC_4, r0

    .global ai_rot_a_step
    .type ai_rot_a_step, @function
ai_rot_a_step:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_4, r13
    mov.l @r13, r13
    mov.l   .L_offset_0x004_6, r0
    mov.l @(r0, r14), r4
    mov.l   .L_wram_low_80000, r0
    sub r0, r4
    mov.l   .L_offset_0x008_5, r0
    mov.l @(r0, r14), r5
    mov.l   .L_offset_0x00c_5, r0
    mov.l @(r0, r14), r6
    mov.l   .L_fp_62, r0
    sub r0, r6
    mov.l   .L_sym_06026E2E_4, r0

    .global ai_rot_a_fix_a
    .type ai_rot_a_fix_a, @function
ai_rot_a_fix_a:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x002_6, r0
    mov.w @(r0, r14), r4
    mov.l   .L_mat_rot_y_4, r0

    .global ai_rot_a_fix_b
    .type ai_rot_a_fix_b, @function
ai_rot_a_fix_b:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x011_4, r0
    mov.b @(r0, r14), r4
    mov.l   .L_camera_param_load_2, r0

    .global ai_rot_a_fix_c
    .type ai_rot_a_fix_c, @function
ai_rot_a_fix_c:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06026DF8_4, r0

    .global ai_rot_b_entry
    .type ai_rot_b_entry, @function
ai_rot_b_entry:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_4, r13
    mov.l @r13, r13
    mov.l   .L_sym_06083250_3, r0
    mov.l @r0, r1
    mov.l   .L_const_0x627, r0
    cmp/ge r0, r1
    bf      .L_0603441C
    mov.l   .L_sym_06026DBC_4, r0

    .global ai_rot_b_step
    .type ai_rot_b_step, @function
ai_rot_b_step:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_4, r13
    mov.l @r13, r13
    mov.l   .L_offset_0x004_6, r0
    mov.l @(r0, r14), r4
    mov.l   .L_fp_25, r0
    sub r0, r4
    mov.l   .L_offset_0x008_5, r0
    mov.l @(r0, r14), r5
    mov.l   .L_offset_0x00c_5, r0
    mov.l @(r0, r14), r6
    mov.l   .L_fp_28, r0
    sub r0, r6
    mov.l   .L_sym_06026E2E_4, r0

    .global ai_rot_b_fix_a
    .type ai_rot_b_fix_a, @function
ai_rot_b_fix_a:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x002_6, r0
    mov.w @(r0, r14), r4
    add r0, r4
    mov.l   .L_mat_rot_y_4, r0

    .global ai_rot_b_fix_b
    .type ai_rot_b_fix_b, @function
ai_rot_b_fix_b:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x011_4, r0
    mov.b @(r0, r14), r4
    mov.l   .L_camera_param_load_2, r0

    .global ai_rot_b_fix_c
    .type ai_rot_b_fix_c, @function
ai_rot_b_fix_c:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06026DF8_4, r0

    .global ai_decision_dispatch
    .type ai_decision_dispatch, @function
/* ai_decision_dispatch() — pop matrix stack A (sym_06026DF8 = mat_pop_a),
 * then load current matrix stack A pointer into r13 for use by following
 * AI velocity/rotation integration chain. Falls through to return.
 */
ai_decision_dispatch:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_4, r13
    mov.l @r13, r13
.L_0603441C:
    bra     .L_06034468
    nop
.L_sym_06026DBC_4:
    .4byte  sym_06026DBC
.L_sym_06089EDC_4:
    .4byte  sym_06089EDC
.L_offset_0x004_6:
    .4byte  0x00000004
.L_offset_0x008_5:
    .4byte  0x00000008
.L_offset_0x00c_5:
    .4byte  0x0000000C
.L_sym_06026E2E_4:
    .4byte  sym_06026E2E
.L_const_0x002_6:
    .4byte  0x00000002
.L_mat_rot_y_4:
    .4byte  mat_rot_y
.L_const_0x011_4:
    .4byte  0x00000011
.L_camera_param_load_2:
    .4byte  camera_param_load
.L_sym_06026DF8_4:
    .4byte  sym_06026DF8
.L_sym_06083250_3:
    .4byte  sym_06083250
.L_offset_0x4ec:
    .4byte  0x000004EC          /* struct offset (0x4EC) — deep in car data */
.L_wram_low_80000:
    .4byte  0x00280000          /* 16.16 FP: 40.0 (camera distance?) */
.L_fp_62:
    .4byte  0x003E0000          /* 16.16 FP: 62.0 */
.L_const_0x627:
    .4byte  0x00000627          /* struct offset (0x627) — AI config data */
.L_fp_25:
    .4byte  0x00190000          /* 16.16 FP: 25.0 */
.L_fp_28:
    .4byte  0x001C0000          /* 16.16 FP: 28.0 */
.L_06034468:
    rts
    nop

    .global sym_0603446C
/* ai_clear_status — clear AI status byte for all 5 cars.
 * Loops over car structs (stride 0x2C=44 bytes, 5 entries at DAT_06082A7C),
 * zeroing the AI status byte at offset +0x26 in each.
 */
sym_0603446C:
    xor r7, r7                 /* r7 = car index (0..4) */
.L_0603446E:
    mov.l   .L_offset_0x02c_2, r1 /* r1 = 0x2C (struct stride) */
    mulu.w r7, r1              /* MACL = index * stride */
    sts macl, r0
    mov.l   .L_DAT_06082A7C_2, r14 /* r14 = car array base */
    add r0, r14                /* r14 = &car[index] */
    xor r1, r1                 /* r1 = 0 */
    mov.l   .L_const_0x026_2, r0 /* r0 = 0x26 (status byte offset) */
    mov.b r1, @(r0, r14)      /* car[i].status = 0 */
    add #0x1, r7               /* next car */
    mov.l   .L_const_0x005, r0 /* r0 = 5 (car count) */
    cmp/ge r0, r7
    bf      .L_0603446E        /* loop until all 5 cleared */
    rts
    nop
    .2byte  0x0000
.L_offset_0x02c_2:
    .4byte  0x0000002C          /* car struct stride (44 bytes) */
.L_DAT_06082A7C_2:
    .4byte  DAT_06082A7C        /* AI car array base */
.L_const_0x026_2:
    .4byte  0x00000026          /* offset: AI status byte */
.L_const_0x005:
    .4byte  0x00000005          /* car count (5 cars) */
.L_0603449C:
    mov.l   .L_const_0x001_6, r0
    mov.b @(r0, r14), r0
    cmp/eq #0x2, r0
    bf      .L_0603454A
    mov.l   .L_offset_0x004_7, r0
    mov.l @(r0, r14), r3
    mov.l   .L_offset_0x00c_6, r0
    mov.l @(r0, r14), r4
    mov.l   .L_offset_0x010, r0
    mov.l @(r0, r10), r5
    mov.l   .L_offset_0x018_3, r0
    mov.l @(r0, r10), r6
    mov.l r13, @-r15
    mov.l r1, @-r15
    mov.l r2, @-r15
    mov.l r3, @-r15
    mov.l r4, @-r15
    mov.l r5, @-r15
    mov.l   .L_sym_06083238_2, r13
    mov.l r3, @(0, r13)
    mov.l r4, @(4, r13)
    mov.l r5, @(8, r13)
    mov.l r6, @(12, r13)
    mov.l @(0, r13), r1
    mov.l @(8, r13), r0
    sub r1, r0
    mov r0, r4
    mov r0, r5
    dmuls.l r4, r5
    sts mach, r0
    sts macl, r1
    xtrct r0, r1
    mov.l r1, @(16, r13)
    mov.l @(4, r13), r1
    mov.l @(12, r13), r0
    sub r1, r0
    mov r0, r4
    mov r0, r5
    dmuls.l r4, r5
    sts mach, r0
    sts macl, r1
    xtrct r0, r1
    mov.l @(16, r13), r4
    add r1, r4
    cmp/pz r4
    bt      .L_06034524
    shlr2 r4
    mov.l   .L_isqrt, r0

    .global ai_speed_limit
    .type ai_speed_limit, @function
ai_speed_limit:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    bra     .L_0603452E
    shll r0
.L_const_0x001_6:
    .4byte  0x00000001
.L_offset_0x004_7:
    .4byte  0x00000004
.L_offset_0x00c_6:
    .4byte  0x0000000C
.L_offset_0x010:
    .4byte  0x00000010
.L_offset_0x018_3:
    .4byte  0x00000018
.L_sym_06083238_2:
    .4byte  sym_06083238
.L_isqrt:
    .4byte  isqrt
.L_06034524:
    mov.l   .L_isqrt_2, r0

    .global ai_waypoint_follower
    .type ai_waypoint_follower, @function
ai_waypoint_follower:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
.L_0603452E:
    mov.l @r15+, r5
    mov.l @r15+, r4
    mov.l @r15+, r3
    mov.l @r15+, r2
    mov.l @r15+, r1
    mov.l @r15+, r13
    mov r0, r2
    mov r2, r8
    mov.l   .L_fp_eight, r0
    cmp/ge r0, r2
    bt      .L_0603454A
    mov.l   .L_const_0x001_7, r1
    mov.l   .L_const_0x026_3, r0
    mov.b r1, @(r0, r14)
.L_0603454A:
    rts
    nop
    .2byte  0x0000
.L_isqrt_2:
    .4byte  isqrt
.L_fp_eight:
    .4byte  0x00080000
.L_const_0x001_7:
    .4byte  0x00000001
.L_const_0x026_3:
    .4byte  0x00000026
.L_06034560:
    mov.l   .L_offset_0x028, r0
    mov.l @(r0, r10), r1
    mov.l   .L_const_0x002_7, r0
    mov.w r1, @(r0, r14)
    mov.l   .L_offset_0x00c_7, r0
    mov.l @(r0, r10), r4
    shal r4
    mov.l   .L_offset_0x028, r0
    mov.l r4, @(r0, r14)
    mov.l   .L_const_0x026_4, r0
    mov #0x2, r1
    mov.b r1, @(r0, r14)
    rts
    nop
.L_offset_0x028:
    .4byte  0x00000028
.L_const_0x002_7:
    .4byte  0x00000002
.L_offset_0x00c_7:
    .4byte  0x0000000C
.L_const_0x026_4:
    .4byte  0x00000026
.L_0603458C:
    mov.l   .L_const_0x002_8, r0
    mov.w @(r0, r14), r9
    mov.l   .L_quarter_turn, r1
    add r1, r9
    mov.l   .L_offset_0x028_2, r0
    mov.l @(r0, r14), r8
    mov r9, r4
    mov.l   .L_cos_lookup_3, r0

    .global ai_collision_trigger
    .type ai_collision_trigger, @function
ai_collision_trigger:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    dmuls.l r0, r8
    sts mach, r0
    sts macl, r3
    xtrct r0, r3
    mov.l   .L_offset_0x004_8, r0
    mov.l @(r0, r14), r1
    add r3, r1
    mov.l r1, @(r0, r14)
    mov r9, r4
    mov.l   .L_sin_lookup_3, r0

    .global ai_collision_response
    .type ai_collision_response, @function
ai_collision_response:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    dmuls.l r0, r8
    sts mach, r0
    sts macl, r3
    xtrct r0, r3
    mov.l   .L_offset_0x00c_8, r0
    mov.l @(r0, r14), r1
    add r3, r1
    mov.l r1, @(r0, r14)
    mov.l   .L_offset_0x004_8, r0
    mov.l @(r0, r14), r4
    mov.l   .L_sym_06083238_3, r0
    mov.l r4, @r0
    mov.l   .L_offset_0x00c_8, r0
    mov.l @(r0, r14), r5
    mov.l   .L_sym_06083240_2, r0
    mov.l r5, @r0
    mov.l   .L_sym_06006838_2, r0

    .global ai_reset_stub
    .type ai_reset_stub, @function
ai_reset_stub:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov r0, r4
    mov.l   .L_sym_06083238_3, r5
    mov.l   .L_sym_06083244_2, r6
    mov.l   .L_scene_render_alt_2, r0

    .global ai_car_pos_init
    .type ai_car_pos_init, @function
ai_car_pos_init:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_0608323C_2, r0
    mov.l @r0, r1
    mov.l   .L_offset_0x008_6, r0
    mov.l r1, @(r0, r14)
    rts
    nop
    .2byte  0x0000
.L_const_0x002_8:
    .4byte  0x00000002
.L_quarter_turn:
    .4byte  0x00004000
.L_offset_0x028_2:
    .4byte  0x00000028
.L_cos_lookup_3:
    .4byte  cos_lookup
.L_offset_0x004_8:
    .4byte  0x00000004
.L_sin_lookup_3:
    .4byte  sin_lookup
.L_offset_0x00c_8:
    .4byte  0x0000000C
.L_sym_06083238_3:
    .4byte  sym_06083238
.L_sym_06083240_2:
    .4byte  sym_06083240
.L_sym_06006838_2:
    .4byte  sym_06006838
.L_sym_06083244_2:
    .4byte  sym_06083244
.L_scene_render_alt_2:
    .4byte  scene_render_alt
.L_sym_0608323C_2:
    .4byte  sym_0608323C
.L_offset_0x008_6:
    .4byte  0x00000008
.L_06034640:
    mov.l   .L_sym_06026DBC_5, r0

    .global ai_spawn_helper_a
    .type ai_spawn_helper_a, @function
ai_spawn_helper_a:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_5, r13
    mov.l @r13, r13
    mov.l   .L_offset_0x004_9, r0
    mov.l @(r0, r14), r4
    mov.l   .L_offset_0x008_7, r0
    mov.l @(r0, r14), r5
    mov.l   .L_offset_0x00c_9, r0
    mov.l @(r0, r14), r6
    mov.l   .L_sym_06026E2E_5, r0

    .global ai_spawn_helper_b
    .type ai_spawn_helper_b, @function
ai_spawn_helper_b:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x002_9, r0
    mov.w @(r0, r14), r4
    mov.l   .L_mat_rot_y_5, r0

    .global ai_spawn_helper_c
    .type ai_spawn_helper_c, @function
ai_spawn_helper_c:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_const_0x011_5, r0
    mov.b @(r0, r14), r4
    mov.l   .L_camera_scene_setup_3, r0

    .global ai_spawn_helper_d
    .type ai_spawn_helper_d, @function
ai_spawn_helper_d:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06026DF8_5, r0

    .global ai_car_full_init
    .type ai_car_full_init, @function
ai_car_full_init:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l   .L_sym_06089EDC_5, r13
    mov.l @r13, r13
    rts
    nop
    .2byte  0x0000
.L_sym_06026DBC_5:
    .4byte  sym_06026DBC
.L_sym_06089EDC_5:
    .4byte  sym_06089EDC
.L_offset_0x004_9:
    .4byte  0x00000004
.L_offset_0x008_7:
    .4byte  0x00000008
.L_offset_0x00c_9:
    .4byte  0x0000000C
.L_sym_06026E2E_5:
    .4byte  sym_06026E2E
.L_const_0x002_9:
    .4byte  0x00000002
.L_mat_rot_y_5:
    .4byte  mat_rot_y
.L_const_0x011_5:
    .4byte  0x00000011
.L_camera_scene_setup_3:
    .4byte  camera_scene_setup
.L_sym_06026DF8_5:
    .4byte  sym_06026DF8
.L_060346C0:
    mov.l   .L_const_0x001_8, r0
    mov.b @(r0, r14), r0
    add #-0x1, r0
    mov.l   .L_sym_060631B4, r1
    mov.b @(r0, r1), r2
    mov.l   .L_sym_060631B8, r1
    mov.b @(r0, r1), r3
    mov.l   .L_offset_0x010_2, r0
    mov.b @(r0, r14), r1
    add #0x1, r1
    mov.b r1, @(r0, r14)
    cmp/ge r2, r1
    bf      .L_060346EC
    xor r2, r2
    mov.b r2, @(r0, r14)
    mov.l   .L_const_0x011_6, r0
    mov.b @(r0, r14), r1
    add #0x1, r1
    cmp/ge r3, r1
    bf      .L_060346EA
    mov.l   .L_zero_2, r1
.L_060346EA:
    mov.b r1, @(r0, r14)
.L_060346EC:
    rts
    nop
.L_const_0x001_8:
    .4byte  0x00000001
.L_sym_060631B4:
    .4byte  sym_060631B4
.L_sym_060631B8:
    .4byte  sym_060631B8
.L_offset_0x010_2:
    .4byte  0x00000010
.L_const_0x011_6:
    .4byte  0x00000011
.L_zero_2:
    .4byte  0x00000000

    .global vblank_handler
    .type vblank_handler, @function
/* vblank_handler(r4=car_struct_ptr) — per-car VBlank update.
 * Saves car base to r14, copies position field, then chains through
 * ranking stubs and position calculation. r14 persists as car pointer
 * throughout the entire per-car update sequence.
 */
vblank_handler:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov.l r9, @-r15
    mov.l r8, @-r15
    mov r4, r14
    mov.l   .L_offset_0x00c_10, r0
    mov.l @(r0, r14), r2
    mov.l   .L_offset_0x010_3, r0
    mov.l r2, @(r0, r14)

    .global ai_rank_stub_a
    .type ai_rank_stub_a, @function
ai_rank_stub_a:
    sts.l pr, @-r15
    bsr     .L_06034754
    nop
    lds.l @r15+, pr

    .global ai_rank_stub_b
    .type ai_rank_stub_b, @function
ai_rank_stub_b:
    sts.l pr, @-r15
    bsr     .L_060347A8
    nop
    lds.l @r15+, pr

    .global position_calc_current
    .type position_calc_current, @function
position_calc_current:
    sts.l pr, @-r15
    bsr     .L_06034848
    nop
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    mov.l @r15+, r14
    rts
    nop
    .2byte  0x0000
.L_offset_0x00c_10:
    .4byte  0x0000000C
.L_offset_0x010_3:
    .4byte  0x00000010
.L_06034754:
    mov.l   .L_sym_0607E944_5, r0
    mov.l @r0, r1
    mov.l   .L_offset_0x008_8, r0
    mov.l   .L_fp_300, r5
    mov.l @(r0, r1), r4
    shll16 r4
    mov.l   .L_fpdiv_setup_2, r0

    .global ranking_system_full
    .type ranking_system_full, @function
/* ranking_system_full() — recalculate race position from distance traveled.
 * Uses fpdiv_setup to divide, adds result to cumulative distance at +0x08,
 * wraps to zero if exceeds track length. Stores integer part at +offset.
 */
ranking_system_full:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov r0, r2
    mov.l   .L_offset_0x008_8, r0
    mov.l @(r0, r14), r1
    add r2, r1
    mov.l   .L_fp_6, r3
    cmp/ge r3, r1
    bf      .L_0603477A
    xor r1, r1
.L_0603477A:
    mov.l r1, @(r0, r14)
    mov r1, r2
    shlr16 r2
    mov.l   .L_dma_direct_imm, r0
    and r0, r2
    mov.l   .L_offset_0x00c_11, r0
    mov.l r2, @(r0, r14)
    rts
    nop
.L_sym_0607E944_5:
    .4byte  sym_0607E944
.L_offset_0x008_8:
    .4byte  0x00000008
.L_fp_300:
    .4byte  0x012C0000
.L_fpdiv_setup_2:
    .4byte  fpdiv_setup
.L_fp_6:
    .4byte  0x00060000
.L_dma_direct_imm:
    .4byte  0x00000007
.L_offset_0x00c_11:
    .4byte  0x0000000C
.L_060347A8:
    mov.l   .L_offset_0x00c_12, r0
    mov.l @(r0, r14), r1
    mov #0x1, r3
    cmp/eq r3, r1
    bf      .L_060347C8
    mov.l   .L_offset_0x010_4, r0
    mov.l @(r0, r14), r0
    xor r1, r0
    tst r0, r0
    bt      .L_060347C8
    mov.l   .L_const_0x001_9, r0
    mov.l   .L_neg_one, r1
    mov.b r1, @(r0, r14)
    mov.l   .L_const_0x002_10, r0
    xor r1, r1
    mov.b r1, @(r0, r14)
.L_060347C8:
    mov.l   .L_offset_0x00c_12, r0
    mov.l @(r0, r14), r1
    mov #0x5, r3
    cmp/eq r3, r1
    bf      .L_060347E8
    mov.l   .L_offset_0x010_4, r0
    mov.l @(r0, r14), r0
    xor r1, r0
    tst r0, r0
    bt      .L_060347E8
    mov.l   .L_offset_0x004_10, r0
    mov.l   .L_neg_one, r1
    mov.b r1, @(r0, r14)
    mov.l   .L_const_0x005_2, r0
    xor r1, r1
    mov.b r1, @(r0, r14)
.L_060347E8:
    mov.l   .L_const_0x002_10, r0
    mov.b @(r0, r14), r1
    add #0x1, r1
    mov.b r1, @(r0, r14)
    mov #0x3, r2
    and r2, r1
    mov.l   .L_zero_3, r0
    mov.b r1, @(r0, r14)
    mov #0x3, r2
    cmp/eq r2, r1
    bf      .L_06034804
    mov.l   .L_const_0x001_9, r0
    xor r1, r1
    mov.b r1, @(r0, r14)
.L_06034804:
    mov.l   .L_const_0x005_2, r0
    mov.b @(r0, r14), r1
    add #0x1, r1
    mov.b r1, @(r0, r14)
    mov #0x3, r2
    and r2, r1
    mov.l   .L_const_0x003, r0
    mov.b r1, @(r0, r14)
    mov #0x3, r2
    cmp/eq r2, r1
    bf      .L_06034820
    mov.l   .L_offset_0x004_10, r0
    xor r1, r1
    mov.b r1, @(r0, r14)
.L_06034820:
    rts
    nop
.L_offset_0x00c_12:
    .4byte  0x0000000C
.L_offset_0x010_4:
    .4byte  0x00000010
.L_const_0x001_9:
    .4byte  0x00000001
.L_neg_one:
    .4byte  0xFFFFFFFF
.L_const_0x002_10:
    .4byte  0x00000002
.L_offset_0x004_10:
    .4byte  0x00000004
.L_const_0x005_2:
    .4byte  0x00000005
.L_zero_3:
    .4byte  0x00000000
.L_const_0x003:
    .4byte  0x00000003
.L_06034848:
    mov.l   .L_sym_06083255, r0
    mov.b @r0, r0
    tst r0, r0
    bt      .L_060348EE
    mov.l   .L_sym_0607EBC4, r0
    mov.l @r0, r0
    mov.l   .L_fp_2_8000, r1
    tst r1, r0
    bf      .L_0603486C
    bra     .L_060348EE
    nop
    .2byte  0x0000
.L_sym_06083255:
    .4byte  sym_06083255
.L_sym_0607EBC4:
    .4byte  sym_0607EBC4
.L_fp_2_8000:
    .4byte  0x00028000
.L_0603486C:
    mov.l   .L_offset_0x010_5, r0
    mov.l @(r0, r14), r1
    mov.l   .L_offset_0x00c_13, r0
    mov.l @(r0, r14), r0
    cmp/eq r1, r0
    bt      .L_060348EE
    mov.l   .L_sym_0607E944_6, r0
    mov.l @r0, r1
    mov.l   .L_offset_0x008_9, r0
    mov.l @(r0, r1), r1
    mov.w   DAT_0603489e, r0
    cmp/ge r0, r1
    bt      .L_060348B0
    mov.l   .L_offset_0x00c_13, r0
    mov.l @(r0, r14), r0
    cmp/eq #0x0, r0
    bt      .L_060348E0
    cmp/eq #0x3, r0
    bt      .L_060348C8
    cmp/eq #0x4, r0
    bt      .L_060348D4
    cmp/eq #0x5, r0
    bt      .L_060348E0
    bra     .L_060348EE
    nop

    .global DAT_0603489e
DAT_0603489e:
    .2byte  0x0104
.L_offset_0x010_5:
    .4byte  0x00000010
.L_offset_0x00c_13:
    .4byte  0x0000000C
.L_sym_0607E944_6:
    .4byte  sym_0607E944
.L_offset_0x008_9:
    .4byte  0x00000008
.L_060348B0:
    mov.l   .L_offset_0x00c_14, r0
    mov.l @(r0, r14), r0
    cmp/eq #0x0, r0
    bt      .L_060348E0
    cmp/eq #0x3, r0
    bt      .L_060348C8
    cmp/eq #0x4, r0
    bt      .L_060348D4
    bra     .L_060348EE
    nop
.L_offset_0x00c_14:
    .4byte  0x0000000C
.L_060348C8:
    mov.l   .L_fp_neg_20974_c401, r5
    bra     .L_060348E2
    nop
    .2byte  0x0000
.L_fp_neg_20974_c401:
    .4byte  0xAE113BFF
.L_060348D4:
    mov.l   .L_fp_neg_20974_c301, r5
    bra     .L_060348E2
    nop
    .2byte  0x0000
.L_fp_neg_20974_c301:
    .4byte  0xAE113CFF
.L_060348E0:
    mov.l   .L_fp_neg_20974_c201, r5
.L_060348E2:
    mov.l   .L_zero_4, r4
    mov.l   .L_sound_cmd_dispatch, r0

    .global lap_complete_check
    .type lap_complete_check, @function
lap_complete_check:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
.L_060348EE:
    rts
    nop
    .2byte  0x0000
.L_fp_neg_20974_c201:
    .4byte  0xAE113DFF
.L_zero_4:
    .4byte  0x00000000
.L_sound_cmd_dispatch:
    .4byte  sound_cmd_dispatch

    .global terrain_data_lookup
    .type terrain_data_lookup, @function
terrain_data_lookup:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov.l r9, @-r15
    mov.l r8, @-r15
    mov.l   .L_sym_0607E944_7, r1
    mov.l @r1, r14
    mov.l   .L_offset_0x0b8, r0
    mov.l @(r0, r14), r0
    cmp/eq #0x2C, r0
    bt      .L_06034934
    mov.l   .L_offset_0x1bc, r0
    mov.l @(r0, r14), r0
    cmp/eq #0x28, r0
    bt      .L_06034934
    bra     .L_06034942
    nop
    .2byte  0x0000
.L_sym_0607E944_7:
    .4byte  sym_0607E944
.L_offset_0x0b8:
    .4byte  0x000000B8
.L_offset_0x1bc:
    .4byte  0x000001BC
.L_06034934:
    mov.l   .L_zero_5, r4
    mov.l   .L_fp_neg_20974_d701, r5
    mov.l   .L_sound_cmd_dispatch_2, r0

    .global checkpoint_validate
    .type checkpoint_validate, @function
checkpoint_validate:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
.L_06034942:
    mov.w   DAT_06034976, r0
    mov.l @(r0, r14), r5
    cmp/pz r5
    bt      .L_0603494C
    neg r5, r5
.L_0603494C:
    shlr8 r5
    mov #0x7, r3
    shlr2 r5
    cmp/ge r5, r3
    bt      .L_06034958
    mov #0x7, r5
.L_06034958:
    mov #0x2, r4
    mov.l   .L_sound_cmd_dispatch_2, r0

    .global section_transition
    .type section_transition, @function
section_transition:
    sts.l pr, @-r15
    jsr @r0
    nop
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    mov.l @r15+, r14
    rts
    nop

    .global DAT_06034976
DAT_06034976:
    .2byte  0x005C
.L_zero_5:
    .4byte  0x00000000
.L_fp_neg_20974_d701:
    .4byte  0xAE1128FF
.L_sound_cmd_dispatch_2:
    .4byte  sound_cmd_dispatch

    .global ai_checkpoint_section
    .type ai_checkpoint_section, @function
ai_checkpoint_section:
    mov.l r14, @-r15
    mov.l r13, @-r15
    sts.l pr, @-r15
    mov r4, r13
    add #-0x8, r15
    mov.l   .L_input_proc_analog, r3
    jsr @r3
    mov r15, r4
    mov r15, r2
    mov #0x0, r3
    mov r15, r5
    mov.b r3, @r2
    mov.l   .L_input_proc_buttons, r3
    jsr @r3
    mov #0x0, r4
    mov r0, r14
    mov.l   .L_input_proc_extended, r3
    jsr @r3
    mov r13, r4
    mov r14, r0
    add #0x8, r15
    lds.l @r15+, pr
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14

    .global ai_checkpoint_validate
    .type ai_checkpoint_validate, @function
ai_checkpoint_validate:
    sts.l pr, @-r15
    mov.l   .L_input_proc_extended, r3
    jsr @r3
    nop
    lds.l @r15+, pr
    rts
    mov #0x0, r0

    .global ai_section_transition
    .type ai_section_transition, @function
ai_section_transition:
    mov.l r14, @-r15
    mov.l r13, @-r15
    sts.l pr, @-r15
    mov r4, r13
    add #-0x8, r15
    bsr     ai_section_update
    mov r15, r4
    mov r0, r14
    tst r14, r14
    bt      .L_060349E8
    bra     .L_06034A06
    mov r14, r0
.L_input_proc_analog:
    .4byte  input_proc_analog
.L_input_proc_buttons:
    .4byte  input_proc_buttons
.L_input_proc_extended:
    .4byte  input_proc_extended
.L_060349E8:
    mov r13, r5
    mov.l   .L_sym_06035F16, r3
    jsr @r3
    mov r15, r4
    mov.b @r13, r4
    mov.w   DAT_06034a8e, r2
    extu.b r4, r4
    cmp/eq r2, r4
    bt      .L_06034A02
    mov #0x20, r2
    and r4, r2
    tst r2, r2
    bf      .L_06034A04
.L_06034A02:
    mov #-0x8, r14
.L_06034A04:
    mov r14, r0
.L_06034A06:
    add #0x8, r15
    lds.l @r15+, pr
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14

    .global ai_section_update
    .type ai_section_update, @function
ai_section_update:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov.l r9, @-r15
    sts.l pr, @-r15
    add #-0x8, r15
    mov #0x0, r12
    mov.l   .L_sym_06035E5E, r14
    mov r4, r11
    mov #-0x3, r10
    mov r12, r13
.L_06034A2A:
    stc sr, r0
    shlr2 r0
    shlr2 r0
    and #0xF, r0
    mov r0, r9
    stc sr, r0
    mov.w   .L_neg_241_w, r3
    and r3, r0
    or #0xF0, r0
    ldc r0, sr
    jsr @r14
    mov r11, r4
    jsr @r14
    mov r15, r4
    mov r9, r0
    and #0xF, r0
    shll2 r0
    shll2 r0
    stc sr, r2
    mov.w   .L_neg_241_w, r3
    and r3, r2
    or r2, r0
    ldc r0, sr
    mov r11, r4
    mov r15, r2
    mov.l @r4, r3
    mov.l @r2, r2
    cmp/eq r2, r3
    bf      .L_06034A72
    mov r15, r2
    mov.l @(4, r4), r3
    mov.l @(4, r2), r2
    cmp/eq r2, r3
    bf      .L_06034A72
    bra     .L_06034A7A
    mov r12, r10
.L_06034A72:
    add #0x1, r13
    mov #0x64, r2
    cmp/ge r2, r13
    bf      .L_06034A2A
.L_06034A7A:
    mov r10, r0
    add #0x8, r15
    lds.l @r15+, pr
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14

    .global DAT_06034a8e
DAT_06034a8e:
    .2byte  0x00FF
.L_neg_241_w:
    .2byte  0xFF0F
    .2byte  0xFFFF
.L_sym_06035F16:
    .4byte  sym_06035F16
.L_sym_06035E5E:
    .4byte  sym_06035E5E

    .global ai_nop_stub
    .type ai_nop_stub, @function
ai_nop_stub:
    mov.l r14, @-r15
    mov r4, r14

    .global ai_pit_stop_logic
    .type ai_pit_stop_logic, @function
ai_pit_stop_logic:
    sts.l pr, @-r15
    add #-0x10, r15
    mov r15, r4
    mov.l   .L_input_proc_analog_2, r3
    jsr @r3
    add #0x8, r4
    mov r15, r2
    mov #0x1, r3
    mov r15, r6
    mov r15, r5
    add #0x8, r2
    add #0x8, r5
    mov.b r3, @r2
    mov.l   .L_input_proc_digital, r3
    jsr @r3
    mov #0x0, r4
    mov r0, r4
    mov r15, r2
    mov.b @(2, r2), r0
    mov r0, r3
    mov.b r3, @r14
    mov r15, r3
    mov.b @(3, r3), r0
    mov r15, r3
    mov.b r0, @(1, r14)
    mov.b @(5, r3), r0
    mov r15, r3
    mov.b r0, @(2, r14)
    mov.b @(6, r3), r0
    mov r15, r3
    mov.b r0, @(3, r14)
    mov.b @(7, r3), r0
    mov r0, r2
    mov.b r0, @(4, r14)
    mov r4, r0
    add #0x10, r15
    lds.l @r15+, pr
    rts
    mov.l @r15+, r14

    .global track_intersect_test
    .type track_intersect_test, @function
track_intersect_test:
    mov.l r14, @-r15
    sts.l pr, @-r15
    mov r4, r14
    add #-0x14, r15
    mov r15, r4
    mov.l   .L_input_proc_analog_2, r3
    jsr @r3
    add #0xC, r4
    mov r15, r2
    mov #0x2, r3
    mov r15, r6
    mov r15, r5
    add #0xC, r2
    add #0x4, r6
    add #0xC, r5
    mov.b r3, @r2
    mov.l   .L_input_proc_digital, r3
    jsr @r3
    mov #0x0, r4
    mov r0, r4
    tst r4, r4
    bt      .L_06034B24
    mov r4, r0
    add #0x14, r15
    lds.l @r15+, pr
    rts
    mov.l @r15+, r14
.L_06034B24:
    mov r15, r3
    mov r14, r5
    add #0x4, r3
    mov.l @r3, r2
    mov.l   .L_mask_24bit, r3
    and r3, r2
    mov.l r2, @r15
    mov.l   .L_smpc_secondary_proc, r3
    jsr @r3
    mov r2, r4
    mov r0, r4
    add #0x14, r15
    lds.l @r15+, pr
    rts
    mov.l @r15+, r14
    .2byte  0xFFFF
.L_input_proc_analog_2:
    .4byte  input_proc_analog
.L_input_proc_digital:
    .4byte  input_proc_digital
.L_mask_24bit:
    .4byte  0x00FFFFFF
.L_smpc_secondary_proc:
    .4byte  smpc_secondary_proc

    .global mesh_boundary_check
    .type mesh_boundary_check, @function
mesh_boundary_check:
    mov.l r14, @-r15
    mov r4, r14
    mov.l r13, @-r15
    mov r5, r13

    .global ai_crash_recovery
    .type ai_crash_recovery, @function
ai_crash_recovery:
    sts.l pr, @-r15
    add #-0x10, r15
    mov r15, r4
    mov.l   .L_input_proc_analog_3, r3
    jsr @r3
    add #0x8, r4
    mov r15, r2
    mov #0x3, r3
    extu.b r14, r14
    mov r15, r6
    mov r15, r5
    add #0x8, r2
    mov r14, r0
    add #0x8, r5
    mov.b r3, @r2
    mov r15, r2
    add #0x8, r2
    mov.b r0, @(1, r2)
    mov.l   .L_input_proc_digital_2, r3
    jsr @r3
    mov #0x0, r4
    mov r0, r4
    mov r15, r2
    mov r4, r0
    mov.l @(4, r2), r3
    mov.l r3, @r13
    add #0x10, r15
    lds.l @r15+, pr
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14

    .global road_segment_query
    .type road_segment_query, @function
road_segment_query:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov r4, r14
    mov.l r12, @-r15
    mov r6, r13
    mov.l r11, @-r15
    mov r7, r12

    .global ai_drafting_calc
    .type ai_drafting_calc, @function
/* ai_drafting_calc() — compute drafting/slipstream effect between cars.
 * Computes distance between two car positions, checks proximity threshold,
 * applies speed bonus if within slipstream distance.
 */
ai_drafting_calc:
    sts.l pr, @-r15
    mov r5, r11
    add #-0xC, r15
    mov r15, r4
    mov.l   .L_input_proc_analog_3, r3
    jsr @r3
    add #0x4, r4
    mov r15, r2
    mov #0x4, r3
    extu.w r11, r11
    extu.b r13, r13
    extu.b r12, r12
    add #0x4, r2
    mov.b r3, @r2
    mov r15, r2
    extu.b r14, r3
    add #0x4, r2
    mov r3, r0
    mov.b r0, @(1, r2)
    mov r15, r3
    mov r11, r0
    add #0x4, r3
    mov.w r0, @(2, r3)
    mov r15, r3
    mov r13, r0
    add #0x4, r3
    mov.b r0, @(6, r3)
    mov r15, r3
    mov r12, r0
    add #0x4, r3
    mov.b r0, @(7, r3)
    mov.w   .L_const_0x081_w, r0
    and r14, r0
    cmp/eq #0x1, r0
    bf      .L_06034C2C
    mov.w   .L_const_0x0dac_w, r2
    bra     .L_06034C04
    nop
.L_const_0x081_w:
    .2byte  0x0081
.L_const_0x0dac_w:
    .2byte  0x0DAC
.L_input_proc_analog_3:
    .4byte  input_proc_analog
.L_input_proc_digital_2:
    .4byte  input_proc_digital
.L_06034C00:
    mov.l @r15, r2
    add #-0x1, r2
.L_06034C04:
    mov.l r2, @r15
    mov.l @r15, r3
    cmp/pl r3
    bt      .L_06034C00
    mov.l   .L_sym_06035C92, r3
    jsr @r3
    mov #0x41, r4
    mov r15, r5
    mov.l   .L_input_proc_buttons_2, r3
    add #0x4, r5
    jsr @r3
    mov #0x40, r4
    mov r0, r14
    tst r14, r14
    bf      .L_06034C38
    mov.l   .L_smpc_cmd_helper_c, r3
    jsr @r3
    nop
    bra     .L_06034C38
    nop
.L_06034C2C:
    mov r15, r5
    mov.l   .L_input_proc_buttons_2, r3
    add #0x4, r5
    jsr @r3
    mov #0x0, r4
    mov r0, r14
.L_06034C38:
    mov r14, r0
    add #0xC, r15
    lds.l @r15+, pr
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14

    .global ai_brake_zone_calc
    .type ai_brake_zone_calc, @function
ai_brake_zone_calc:
    sts.l pr, @-r15
    add #-0x8, r15
    mov.l   .L_input_proc_analog_4, r3
    jsr @r3
    mov r15, r4
    mov r15, r2
    mov #0x5, r3
    mov r15, r5
    mov.b r3, @r2
    mov.l   .L_input_proc_buttons_2, r3
    jsr @r3
    mov #0x0, r4
    add #0x8, r15
    lds.l @r15+, pr
    rts
    mov r0, r4

    .global ai_brake_zone_adjust
    .type ai_brake_zone_adjust, @function
ai_brake_zone_adjust:
    mov.l r14, @-r15
    mov.l r13, @-r15
    sts.l pr, @-r15
    mov r4, r13
    add #-0x8, r15
    tst r13, r13
    bf      .L_06034C8C
    bra     .L_06034C8E
    mov #0x2, r4
    .2byte  0xFFFF
.L_sym_06035C92:
    .4byte  sym_06035C92
.L_input_proc_buttons_2:
    .4byte  input_proc_buttons
.L_smpc_cmd_helper_c:
    .4byte  smpc_cmd_helper_c
.L_input_proc_analog_4:
    .4byte  input_proc_analog
.L_06034C8C:
    mov.w   .L_const_0x082_w, r4
.L_06034C8E:
    mov.l   .L_input_proc_init, r3
    jsr @r3
    mov r15, r5
    mov r0, r14
    mov r13, r0
    cmp/eq #0x1, r0
    bf      .L_06034CAA
    tst r14, r14
    bf      .L_06034CAA
    mov.w @r15, r0
    extu.w r0, r0
    tst #0x80, r0
    bt      .L_06034CAA
    mov #-0x4, r14
.L_06034CAA:
    tst r14, r14
    bt      .L_06034CB4
    mov r15, r4
    bsr     ai_brake_zone_main
    add #0x4, r4
.L_06034CB4:
    mov.l   .L_const_0xfffd, r4
    mov.l   .L_sym_06035C6E, r3
    jsr @r3
    nop
    mov r14, r0
    add #0x8, r15
    lds.l @r15+, pr
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14

    .global ai_brake_zone_main
    .type ai_brake_zone_main, @function
ai_brake_zone_main:
    mov.l r14, @-r15
    mov r4, r14

    .global ai_brake_zone
    .type ai_brake_zone, @function
ai_brake_zone:
    sts.l pr, @-r15
    add #-0x10, r15
    mov r15, r4
    mov.l   .L_input_proc_analog_5, r3
    jsr @r3
    add #0x8, r4
    mov r15, r2
    mov #0x6, r3
    mov r15, r6
    mov r15, r5
    add #0x8, r2
    add #0x8, r5
    mov.b r3, @r2
    mov.l   .L_input_proc_digital_3, r3
    jsr @r3
    mov #0x0, r4
    mov r0, r4
    mov r15, r2
    mov r4, r0
    mov.l @r2, r3
    mov.l   .L_mask_24bit_2, r2
    and r2, r3
    mov.l r3, @r14
    add #0x10, r15
    lds.l @r15+, pr
    rts
    mov.l @r15+, r14
.L_const_0x082_w:
    .2byte  0x0082
.L_input_proc_init:
    .4byte  input_proc_init
.L_const_0xfffd:
    .4byte  0x0000FFFD
.L_sym_06035C6E:
    .4byte  sym_06035C6E
.L_input_proc_analog_5:
    .4byte  input_proc_analog
.L_input_proc_digital_3:
    .4byte  input_proc_digital
.L_mask_24bit_2:
    .4byte  0x00FFFFFF

    .global ai_throttle_modulate
    .type ai_throttle_modulate, @function
ai_throttle_modulate:
    mov.l r14, @-r15
    mov r4, r14

    .global ai_throttle_mod
    .type ai_throttle_mod, @function
ai_throttle_mod:
    sts.l pr, @-r15
    add #-0x8, r15
    mov.l   .L_input_proc_analog_6, r3
    jsr @r3
    mov r15, r4
    mov r15, r2
    mov #0x10, r3
    mov r15, r5
    mov.b r3, @r2
    add #0x1, r5
    bsr     ai_throttle_adjust
    mov r14, r4
    mov r15, r2
    mov #0x10, r0
    mov r15, r5
    mov r14, r4
    mov.b @(r0, r14), r3
    add #0x5, r5
    mov r3, r0
    mov.b r0, @(4, r2)
    bsr     ai_throttle_adjust
    add #0x8, r4
    mov r15, r5
    mov.l   .L_input_proc_buttons_3, r3
    jsr @r3
    mov #0x0, r4
    mov r0, r4
    add #0x8, r15
    lds.l @r15+, pr
    rts
    mov.l @r15+, r14

    .global ai_throttle_adjust
    .type ai_throttle_adjust, @function
ai_throttle_adjust:
    sts.l pr, @-r15
    bra     .L_06034DD0
    mov.l @r4, r0
.L_06034D64:
    mov #0x0, r2
    mov.b r2, @r5
    mov.b @(4, r4), r0
    mov r0, r3
    mov.b r0, @(1, r5)
    mov.b @(5, r4), r0
    mov r0, r3
    bra     .L_06034D9A
    nop
.L_06034D76:
    mov r4, r0
    add #0x4, r0
    mov.b @(1, r0), r0
    extu.b r0, r0
    or #0x80, r0
    extu.b r0, r0
    mov.b r0, @r5
    mov r4, r3
    add #0x4, r3
    mov.b @(2, r3), r0
    mov r0, r2
    mov r2, r0
    mov.b r0, @(1, r5)
    mov r4, r3
    add #0x4, r3
    mov.b @(3, r3), r0
    mov r0, r2
    mov r2, r0
.L_06034D9A:
    bra     .L_06034DE4
    mov.b r0, @(2, r5)
.L_06034D9E:
    mov r5, r1
    mov.l   .L_sym_06059CA7, r2
    mov.l   .L_sym_06035228, r3
    jsr @r3
    mov #0x3, r0
    bra     .L_06034DE4
    nop
.L_06034DAC:
    mov r5, r1
    mov.l   .L_sym_06059CA4, r2
    mov.l   .L_sym_06035228, r3
    jsr @r3
    mov #0x3, r0
    bra     .L_06034DE4
    nop
    .2byte  0xFFFF
.L_input_proc_analog_6:
    .4byte  input_proc_analog
.L_input_proc_buttons_3:
    .4byte  input_proc_buttons
.L_sym_06059CA7:
    .4byte  sym_06059CA7
.L_sym_06035228:
    .4byte  sym_06035228
.L_sym_06059CA4:
    .4byte  sym_06059CA4
.L_06034DD0:
    cmp/eq #0x0, r0
    bt      .L_06034DAC
    cmp/eq #0x1, r0
    bt      .L_06034D76
    cmp/eq #0x2, r0
    bt      .L_06034D64
    cmp/eq #0x3, r0
    bt      .L_06034D9E
    bra     .L_06034DAC
    nop
.L_06034DE4:
    lds.l @r15+, pr
    rts
    nop

    .global ai_recovery_handler
    .type ai_recovery_handler, @function
ai_recovery_handler:
    mov.l r14, @-r15
    sts.l pr, @-r15
    mov r4, r14
    add #-0x8, r15
    mov.l   .L_input_proc_analog_7, r3
    jsr @r3
    mov r15, r4
    mov r15, r2
    mov #0x11, r3
    mov r15, r5
    mov.b r3, @r2
    add #0x1, r5
    bsr     ai_throttle_adjust
    mov r14, r4
    mov r15, r5
    mov.l   .L_input_proc_buttons_4, r3
    jsr @r3
    mov #0x0, r4
    mov r0, r4
    add #0x8, r15
    lds.l @r15+, pr
    rts
    mov.l @r15+, r14
.L_input_proc_analog_7:
    .4byte  input_proc_analog
.L_input_proc_buttons_4:
    .4byte  input_proc_buttons
