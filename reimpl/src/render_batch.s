/* render_batch — Physics dispatch, car sorting, and LOD-based draw scheduling
 * Translation unit: 0x0600D31C–0x0600D780 (3 functions, 1124 bytes)
 *
 * physics_calc_dispatch (0x0600D31C): Three-way physics pipeline —
 *   checks wram_flag, optionally sorts cars by distance, builds linked
 *   list for secondary physics, then falls through to render_batch.
 *
 * vehicle_state_update (0x0600D41C): Reorders car pointer arrays —
 *   finds pivot car (highest distance), partitions before/after pivot,
 *   builds linked-list chain with next/prev offsets for traversal.
 *
 * render_batch (0x0600D50C): Per-car LOD draw scheduler —
 *   clears render flags, computes camera-relative XZ distance per car,
 *   dispatches bitfield render commands at 3 LOD thresholds:
 *     close (0x0C0000, max 3), medium (0x230000, max 7), far (0x780000).
 *   Final pass adjusts render priority based on relative distance delta.
 *
 * Auto-generated by tools/generate_l3_tu.py, then depth-annotated.
 */
...
.L_compare_done:
    ...
    nop
.L_car_stride:
    .2byte  0x0268                          ! sizeof(car) = 616
.L_dist_compare_off:
    .2byte  0x01F4                          ! car.distance offset (500)
.L_rank_offset:
    .2byte  0x0224                          ! car.sort_rank offset (548)
.L_wram_flag_ptr:
    .4byte  sym_0607EBC4                  ! game state flags
.L_wram_low:
...
DAT_0600d406:
    .2byte  0x0224
.L_distance_offset:
    .2byte  0x01EC                          ! car.dist_key offset (492)
    .2byte  0xFFFF
.L_car_ptr_array:
    .4byte  sym_0607E94C                  ! car pointer array (sorted)
.L_car_count:
    .4byte  sym_0607EA98                  ! car count
.L_secondary_array:
    .4byte  sym_0607E9EC                  ! secondary car array
.L_sort_pivot:
    .4byte  sym_060786B4                  ! sort pivot value
...
.L_secondary_array_b:
    .4byte  sym_0607E9EC
.L_swap_buffer:
    .4byte  sym_0607EA8C                  ! swap workspace
.L_car_count_b:
    .4byte  sym_0607EA98
.L_link_loop:
    mov.l @r4, r2
...
render_batch:
    ...
    mov r14, r5
.L_link_next_offset:
    .2byte  0x0238                          ! car.link_next offset (568)

    .global DAT_0600d536
DAT_0600d536:
    .2byte  0x023C
.L_sort_pivot_c:
    .4byte  sym_060786B4                  ! sort pivot (copied from sort)
.L_player_car_ptr:
    .4byte  sym_0607E944                  ! player car pointer
.L_car_count_c:
    .4byte  sym_0607EA98
.L_render_flag:
    .4byte  sym_0607EBDC                  ! render pass counter
.L_car_ptr_array_b:
    .4byte  sym_0607E94C
.L_clear_car_loop:
    mov.l @r6+, r4
...
.L_bf_cmd_06:
    ...
    .2byte  0xFFFF
.L_fn_bitfield_rw:
    .4byte  sym_06034F78                  ! bitfield read/write utility
.L_wram_flag_ptr_b:
    .4byte  sym_0607EBC4
.L_flag_mask:
    .4byte  0x10060000                  ! flag test mask
.L_cam_heading_ptr:
    .4byte  sym_06063EF0                  ! camera heading angle
.L_fn_sincos:
    .4byte  sincos_pair                   ! sin/cos lookup
.L_fp_scale_7:
    .4byte  0x00070000                  ! FP scale 7.0 (16.16)
.L_fn_fpmul:
    .4byte  fpmul                         ! fixed-point multiply
.L_cam_pos_base:
    .4byte  sym_06063DF8                  ! camera position XYZ base
.L_use_car_heading:
    mov r15, r6
    mov r15, r5
    mov.l @r9, r4
...
.L_fn_sincos_b:
    .4byte  sincos_pair
.L_fp_scale_5:
    .4byte  0x00050000                  ! FP scale 5.0 (16.16)
.L_fn_fpmul_b:
    .4byte  fpmul
.L_lod_close:
    .4byte  0x000C0000                  ! LOD: close threshold (12.0)
.L_fn_bitfield_rw_b:
    .4byte  sym_06034F78
.L_lod_medium:
    .4byte  0x00230000                  ! LOD: medium threshold (35.0)
.L_not_medium:
    mov.l   .L_lod_far, r2
    cmp/ge r2, r5
    bt      .L_lod_done
...
.L_car_stride_b:
    ...
    .2byte  0xFFFF
.L_lod_far:
    .4byte  0x00780000                  ! LOD: far threshold (120.0)
.L_fn_bitfield_rw_c:
    .4byte  sym_06034F78
.L_render_obj_base:
    .4byte  sym_06078B68                  ! renderable object array
