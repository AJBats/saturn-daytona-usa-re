 ...
 *
 * 4 functions:
 *   hud_layout_mgr       - Check disable flags, init HUD sprite layout below threshold
 *   hud_elem_render      - Per-frame HUD element state machine (flag-driven sprite dispatch)
 *   race_data_dispatch    - Master race HUD setup: channels, objects, pause, jump table
 *   loc_06015474          - Thunk: zero-extend r4 byte, tail-call target
 */
...
hud_layout_mgr:                             ! r4 = input param (saved to stack)
    mov.l r14, @-r15
    mov.l r13, @-r15
    sts.l pr, @-r15
    add #-0x4, r15
    ...
    tst r0, r0
    bt      .L_flags_clear
    add #0x4, r15                               ! any flag set: tail-call race_data_dispatch
    lds.l @r15+, pr
    mov.l @r15+, r13
...
.L_disable_flag_d:
    .4byte  sym_0607EAE0
.L_flags_clear:                                 ! all disable flags clear
    .byte   0xD2, 0x23    /* mov.l .L_frame_counter, r2 */
    mov.w   .L_threshold_frame_count, r3        ! 0x2A94 (~10900 frames)
    mov.l @r2, r2
    cmp/hs r3, r2                               ! frame_counter >= threshold?
    ...
    jsr @r3
    nop
    mov #0x60, r6                               ! register 4 HUD sprite elements
    ...
    jsr @r3
    extu.b r4, r4
    extu.b r13, r2                              ! init HUD state vars: counter=0, step=0xE, config=0xA, state=0
    ...
    bra     .L_layout_return
    nop
.L_above_threshold:                             ! above threshold: clear display ctrl, tail-call dispatch
...
hud_elem_render:                                ! r9=config_byte, r12=status_flags, r14=anim_state, r11=anim_step
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov #0x1, r10                               ! r10 = constant 1
    mov.l r9, @-r15
    sts.l pr, @-r15
    ...
    mov.w @(2, r12), r0                         ! read status flags word
    mov r0, r3
    extu.w r3, r3
    and r2, r3                                  ! test bit 0x8000
    tst r3, r3
    bt/s    .L_check_flag_4000                  ! not set: try 0x4000
    mov #0x0, r13
    ...
    jsr @r3
    mov #0xC, r4
    mov.b r10, @r14                             ! anim_state = 1
    mov #0x18, r3
    mov.b r3, @r11                              ! anim_step = 0x18
    extu.b r13, r2
    .byte   0xD3, 0x21    /* mov.l .L_hud_counter_r, r3 */
    mov.b r2, @r3                               ! hud_counter = 0
    bra     .L_check_flag_0200
    nop
.L_check_flag_4000:                             ! 0x8000 not set, check 0x4000
    mov.w @(2, r12), r0
    ...
    jsr @r3
    mov #0xC, r4
    mov #0xE, r2                                ! anim_step = 0xE (different from 0x8000 path)
    mov.b r2, @r11
    exts.b r13, r3
    mov.b r3, @r14                              ! anim_state = 0
    extu.b r13, r2
    .byte   0xD3, 0x12    /* mov.l .L_hud_counter_r, r3 */
    mov.b r2, @r3
.L_check_flag_0200:                             ! HUD animation state machine (flag 0x0200)
    mov.w @(2, r12), r0
    ...
    and r3, r2
    tst r2, r2
    bt      .L_check_flag_0100                  ! 0x0200 not set: try 0x0100
    mov.b @r9, r0                               ! config_byte == 0xC?
    extu.b r0, r0
    cmp/eq #0xC, r0
    bf      .L_after_config_inc
    mov.b @r14, r2                              ! yes: advance anim_state by 2
    add #0x2, r2
    mov.b r2, @r14
.L_after_config_inc:
    mov.b @r14, r0                              ! dispatch on anim_state
    cmp/eq #0x1, r0
    bf      .L_state_dispatch                   ! state 1: jump to cleanup
...
.L_hud_counter_r:
    .4byte  sym_06085F8C
.L_state_dispatch:                              ! state 0: enable race data display
    mov.b @r14, r2
    tst r2, r2
    bf      .L_check_state_2
    ...
    bra     .L_state_done
    nop
.L_check_state_2:                               ! state 2: call geom_render_util
    mov.b @r14, r0
    cmp/eq #0x2, r0
    bf      .L_check_state_3
    ...
    bra     .L_state_done
    nop
.L_check_state_3:                               ! state 3: call geom_output_finalize
    mov.b @r14, r0
    cmp/eq #0x3, r0
    bf      .L_state_done
...
.L_state_done:
    ...
    nop
.L_check_flag_0100:                             ! check flag 0x0100: disable display path
    mov.w @(2, r12), r0
...
.L_disable_display:                             ! clear display ctrl + pause flag, reset frame counter
...
.L_render_epilogue:
    ...
    mov.l @r15+, r14
.L_anim_counter_tick:                           ! increment counter by 0x40; wraps at 256
    ...
    extu.b r2, r2
    tst r2, r2
    bf      .L_counter_nonzero                  ! non-zero: use alt sprite (gfx_d)
...
race_data_dispatch:                             ! master race HUD setup
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    sts.l pr, @-r15
    mov.w   .L_channel_id_0x100, r12            ! r12 = channel ID 0x100
    ...
    mov #0x0, r13                               ! r13 = 0 (used throughout)
    mov r13, r5
    jsr @r14
    mov r12, r4
    ...
    or r3, r2
    tst r2, r2
    bt      .L_after_pause_check                ! neither paused: skip
    .byte   0xD3, 0x31    /* mov.l .L_pause_display_flag_d, r3 */  ! paused: check if pause overlay shown
    mov.w @r3, r3
    extu.w r3, r3
    tst r3, r3
    bf      .L_set_pause_mode                   ! already shown: skip course render
    .byte   0xD3, 0x2F    /* mov.l .L_fn_course_config_render, r3 */
    jsr @r3                                     ! first pause frame: render course config
    nop
.L_set_pause_mode:
    mov #0x1A, r2                               ! set render mode = 0x1A (pause)
    .byte   0xD3, 0x28    /* mov.l .L_render_mode_d, r3 */
    mov.l r2, @r3
.L_after_pause_check:                           ! configure display channels
    .byte   0xD3, 0x2D    /* mov.l .L_fn_vdp2_loop_ctrl, r3 */
    jsr @r3                                     ! vdp2_loop_ctrl()
    nop
    mov #0x0, r5                                ! channel_nibble_config(0x100, 0)
    jsr @r14
    mov r12, r4
    mov #0x1, r5                                ! channel_nibble_config(4, 1)
    jsr @r14
    mov #0x4, r4
    mov #0x6, r5                                ! channel_nibble_config(0x10, 6)
    jsr @r14
    mov #0x10, r4
    ...
    mov.l @r4, r0
    or #0x4, r0                                 ! set bit 2 in display flags
    mov.l r0, @r4
    ...
    add r3, r2                                  ! &table[index]
    mov.l @r2, r2                               ! target = table[index]
    lds.l @r15+, pr
    mov.l @r15+, r12
    mov.l @r15+, r13
    jmp @r2
...
loc_06015474:                                   ! thunk: zero-extend r4 byte, tail-call target
    add #-0x4, r15
    mov.b r4, @r15                              ! store r4 byte to stack
    mov.b @r15, r4                              ! reload (sign-extends)
