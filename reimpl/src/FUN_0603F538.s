
    .section .text.FUN_0603F538


    .global FUN_0603F538
    .type FUN_0603F538, @function
FUN_0603F538:
    sts.l pr, @-r15
    mov r4, r14
    add #-0x8, r15
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x4, r5
    mov.b @(4, r15), r0
    mov r0, r4
    extu.b r4, r4
    tst r4, r4
    bf      .L_0603F578
    mov.l @(12, r14), r3
    add #0x1, r3
    mov.l r3, @(12, r14)
    mov r3, r2
    mov.l @(16, r14), r3
    cmp/eq r3, r2
    bt      .L_0603F578
    mov.w   .L_pool_0603F5B8, r7
    mov #0x1, r5
    mov.l   .L_pool_0603F5BC, r3
    mov.l @(4, r14), r6
    jsr @r3
    mov.l @r14, r4
    mov #0x0, r2
    mov r14, r4
    mov #0x1, r6
    mov.l r2, @(8, r14)
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    mov r15, r5
    mov.b @r15, r5
    extu.b r5, r4
.L_0603F578:
    mov r4, r0
    add #0x8, r15
    lds.l @r15+, pr
    rts
    mov.l @r15+, r14


    .global FUN_0603F582
    .type FUN_0603F582, @function
FUN_0603F582:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov r4, r14
    mov.l r12, @-r15
    mov r5, r13
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov.l r9, @-r15
    mov #0x0, r10
    mov.l r8, @-r15
    sts.l pr, @-r15
    add #-0x54, r15
    mov.l @r14, r0
    tst r0, r0
    bf/s    .L_0603F5A6
    mov r10, r8
    bra     .L_0603F838
    mov.b r10, @r13
.L_0603F5A6:
    .reloc ., R_SH_IND12W, FUN_0603F534 - 4
    .2byte 0xB000    /* bsr FUN_0603F534 (linker-resolved) */
    mov r14, r4
    mov.b r0, @r13
    mov.b @r13, r2
    extu.b r2, r2
    tst r2, r2
    bf      .L_0603F5C0
    bra     .L_0603F838
    nop
.L_pool_0603F5B8:
    .4byte  0x0800FFFF
.L_pool_0603F5BC:
    .4byte  FUN_0603B21C
.L_0603F5C0:
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x4C, r5
    .reloc ., R_SH_IND12W, FUN_0603F500 - 4
    .2byte 0xB000    /* bsr FUN_0603F500 (linker-resolved) */
    mov r14, r4
    mov.w   DAT_0603f682, r2
    add r2, r0
    mov.l r0, @(4, r13)
    .reloc ., R_SH_IND12W, FUN_0603F500 - 4
    .2byte 0xB000    /* bsr FUN_0603F500 (linker-resolved) */
    mov r14, r4
    mov.l r0, @(8, r13)
    bra     .L_0603F5EA
    mov r10, r12
.L_0603F5DE:
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x48, r5
    add #0x1, r12
.L_0603F5EA:
    mov #0x7, r2
    cmp/ge r2, r12
    bf      .L_0603F5DE
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x28, r5
    mov #0x28, r0
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    mov.b @(r0, r15), r2
    mov #0x2C, r0
    mov.b r2, @(r0, r15)
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x24, r5
    mov #0x24, r0
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    mov.b @(r0, r15), r0
    mov.b r0, @(14, r13)
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x20, r5
    mov #0x20, r0
    mov.b @(r0, r15), r0
    mov.b r0, @(15, r13)
    .reloc ., R_SH_IND12W, FUN_0603F4E0 - 4
    .2byte 0xB000    /* bsr FUN_0603F4E0 (linker-resolved) */
    mov r14, r4
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x1C, r5
    mov #0x1C, r0
    mov.b @(r0, r15), r9
    extu.b r9, r2
    add #0x21, r2
    mov.l r2, @(4, r15)
    mov r13, r11
    add #0x10, r11
    bra     .L_0603F676
    mov r10, r12
.L_0603F642:
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x18, r5
    mov #0x18, r0
    mov.b @(r0, r15), r2
    mov.b r2, @r15
    mov.b @r15, r0
    extu.b r0, r0
    cmp/eq #0x3B, r0
    bf/s    .L_0603F660
    add #0x1, r12
    bra     .L_0603F68A
    nop
.L_0603F660:
    mov r11, r2
    mov.b @r15, r3
    mov.b r3, @r2
    mov.b @r15, r0
    extu.b r0, r0
    cmp/eq #0x2E, r0
    bf/s    .L_0603F676
    add #0x1, r11
    mov #0x50, r0
    bra     .L_0603F68A
    mov.l r11, @(r0, r15)
.L_0603F676:
    mov #0x8, r4
    extu.b r9, r5
    cmp/gt r4, r5
    bf      .L_0603F684
    bra     .L_0603F686
    mov r4, r2

    .global DAT_0603f682
DAT_0603f682:
    .2byte  0x0096
.L_0603F684:
    mov r5, r2
.L_0603F686:
    cmp/ge r2, r12
    bf      .L_0603F642
.L_0603F68A:
    mov.b @r15, r0
    extu.b r0, r0
    cmp/eq #0x3B, r0
    bf      .L_0603F6AC
    bra     .L_0603F6A2
    nop
.L_0603F696:
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x44, r5
    add #0x1, r12
.L_0603F6A2:
    extu.b r9, r2
    cmp/ge r2, r12
    bf      .L_0603F696
    bra     .L_0603F748
    nop
.L_0603F6AC:
    mov.b @r15, r0
    extu.b r0, r0
    cmp/eq #0x2E, r0
    bt      .L_0603F6E8
    bra     .L_0603F6E2
    nop
.L_0603F6B8:
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x14, r5
    mov #0x14, r0
    mov.b @(r0, r15), r2
    mov.b r2, @r15
    mov.b @r15, r0
    extu.b r0, r0
    cmp/eq #0x2E, r0
    bf/s    .L_0603F6E0
    add #0x1, r12
    mov r11, r2
    add #0x1, r11
    mov.b @r15, r3
    mov.b r3, @r2
    mov #0x50, r0
    bra     .L_0603F6E8
    mov.l r11, @(r0, r15)
.L_0603F6E0:
    mov #0x1, r8
.L_0603F6E2:
    extu.b r9, r2
    cmp/ge r2, r12
    bf      .L_0603F6B8
.L_0603F6E8:
    bra     .L_0603F748
    nop
.L_0603F6EC:
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x10, r5
    mov #0x10, r0
    mov.b @(r0, r15), r2
    mov.b r2, @r15
    mov.b @r15, r0
    extu.b r0, r0
    cmp/eq #0x3B, r0
    bf/s    .L_0603F720
    add #0x1, r12
    bra     .L_0603F716
    nop
.L_0603F70A:
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x40, r5
    add #0x1, r12
.L_0603F716:
    extu.b r9, r2
    cmp/ge r2, r12
    bf      .L_0603F70A
    bra     .L_0603F74E
    nop
.L_0603F720:
    mov #0x50, r0
    mov r11, r3
    mov #0x3, r1
    mov.l @(r0, r15), r2
    sub r2, r3
    cmp/gt r1, r3
    bf      .L_0603F732
    bra     .L_0603F748
    mov #0x1, r8
.L_0603F732:
    mov r13, r2
    mov r11, r3
    add #0x10, r2
    sub r2, r3
    mov #0xC, r2
    cmp/ge r2, r3
    bt      .L_0603F748
    mov r11, r2
    add #0x1, r11
    mov.b @r15, r3
    mov.b r3, @r2
.L_0603F748:
    extu.b r9, r3
    cmp/ge r3, r12
    bf      .L_0603F6EC
.L_0603F74E:
    mov r13, r3
    mov r11, r2
    add #0x10, r3
    sub r3, r2
    mov #0xC, r3
    cmp/ge r3, r2
    bt      .L_0603F760
    extu.b r10, r3
    mov.b r3, @r11
.L_0603F760:
    extu.b r9, r0
    add #0x1, r0
    tst #0x1, r0
    bt      .L_0603F778
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x3C, r5
    mov.l @(4, r15), r2
    add #0x1, r2
    mov.l r2, @(4, r15)
.L_0603F778:
    mov r13, r3
    add #0x10, r3
    mov.b @r3, r2
    extu.b r2, r2
    tst r2, r2
    bf      .L_0603F792
    mov r13, r0
    mov.l   .L_pool_0603F808, r3
    mov.l   .L_pool_0603F804, r1
    jsr @r3
    add #0x10, r0
    bra     .L_0603F7A8
    nop
.L_0603F792:
    mov r13, r0
    add #0x10, r0
    mov.b @r0, r0
    extu.b r0, r0
    cmp/eq #0x1, r0
    bf      .L_0603F7A8
    mov.l   .L_pool_0603F80C, r1
    mov r13, r0
    mov.l   .L_pool_0603F808, r2
    jsr @r2
    add #0x10, r0
.L_0603F7A8:
    mov.b @r13, r3
    mov.l @(4, r15), r2
    extu.b r3, r3
    cmp/ge r3, r2
    bt      .L_0603F810
    mov r14, r4
    mov #0x4, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x38, r5
    mov r14, r4
    mov #0x2, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0xC, r5
    mov.w @(12, r15), r0
    mov r14, r4
    mov #0x2, r6
    mov r15, r5
    mov.w r0, @(12, r13)
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x34, r5
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x8, r5
    mov.b @(8, r15), r0
    mov r0, r4
    mov #0x1D, r0
    mov.b r4, @(r0, r13)
    mov.l @(4, r15), r12
    bra     .L_0603F7F8
    add #0x9, r12
.L_0603F7EC:
    mov r14, r4
    mov #0x1, r6
    mov r15, r5
    .reloc ., R_SH_IND12W, FUN_0603F3F6 - 4
    .2byte 0xB000    /* bsr FUN_0603F3F6 (linker-resolved) */
    add #0x30, r5
    add #0x1, r12
.L_0603F7F8:
    mov.b @r13, r2
    extu.b r2, r2
    cmp/ge r2, r12
    bf      .L_0603F7EC
    bra     .L_0603F82E
    nop
.L_pool_0603F804:
    .4byte  sym_06059CB8
.L_pool_0603F808:
    .4byte  sym_06035FEC
.L_pool_0603F80C:
    .4byte  sym_06059CBC
.L_0603F810:
    mov #0x2C, r0
    mov.b @(r0, r15), r0
    extu.b r0, r0
    tst #0x2, r0
    bt      .L_0603F820
    mov.l   .L_0603F8B4, r2
    bra     .L_0603F822
    nop
.L_0603F820:
    mov #0x0, r2
.L_0603F822:
    extu.w r2, r2
    mov r2, r0
    mov.w r0, @(12, r13)
    extu.b r10, r3
    mov #0x1D, r0
    mov.b r3, @(r0, r13)
.L_0603F82E:
    tst r8, r8
    bt      .L_0603F838
    mov.w @(12, r13), r0
    or #0x80, r0
    mov.w r0, @(12, r13)
.L_0603F838:
    add #0x54, r15
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14
    .global FUN_0603F84C
FUN_0603F84C:
    .4byte  0x5351E01D
    .4byte  0x24325252
    .4byte  0x1421005C
    .4byte  0x804A8556
    .4byte  0xD3156603
    .4byte  0x666D2369
    .4byte  0x23388901
    .4byte  0xA001E310
    .4byte  0xE300921D
    .4byte  0x22692228
    .4byte  0x8901A001
    .4byte  0xE108E100
    .4byte  0x92172269
    .4byte  0x22288D02
    .4byte  0x331CA001
    .4byte  0xE104E100
    .4byte  0x92102269
    .4byte  0x22288D02
    .4byte  0x331CA001
    .4byte  0xE102E100
    .4byte  0x331C603C
    .4byte  0x804B845E
    .4byte  0x8048845F
    .4byte  0x000B8049
    .2byte  0x0800

    .global DAT_0603f8ae
DAT_0603f8ae:
    .2byte  0x1000
    .4byte  0x2000FFFF
.L_0603F8B4:
    .4byte  0x00008000

    .global sym_0603F8B8
sym_0603F8B8:
    mov.l r5, @r4
    mov.l r6, @(4, r4)
    mov.l r7, @(8, r4)
    mov.l @r15, r3
    mov.l r3, @(12, r4)
    mov.l @(4, r15), r3
    mov.l r3, @(16, r4)
    mov.l @(8, r15), r3
    mov.l r3, @(20, r4)
    rts
    mov r4, r0
    .2byte  0x7FFC
    .4byte  0x2F426043
    .4byte  0x50016442
    .4byte  0x402B7F04
    .4byte  0x7FFC2F42
    .4byte  0x60435001
    .4byte  0xE600D505
    .4byte  0x6442402B
    .2byte  0x7F04

    .global sym_0603F8EE
sym_0603F8EE:
    add #-0x4, r15
    mov.l r4, @r15
    mov r4, r0
    mov.l @(8, r0), r0
    mov.l @r4, r4
    jmp @r0
    add #0x4, r15
    .4byte  0x0000FFFF

    .global sym_0603F900
sym_0603F900:
    add #-0x4, r15
    mov.l r4, @r15
    mov r4, r0
    mov.l @(12, r0), r0
    mov.l @r4, r4
    jmp @r0
    add #0x4, r15

    .global sym_0603F90E
sym_0603F90E:
    add #-0x4, r15
    mov.l r4, @r15
    mov r4, r0
    mov.l @(16, r0), r0
    mov.l @r4, r4
    jmp @r0
    add #0x4, r15

    .global sym_0603F91C
sym_0603F91C:
    add #-0x4, r15
    mov.l r4, @r15
    mov r4, r3
    mov.l @(20, r3), r3
    mov.l @r4, r4
    jmp @r3
    add #0x4, r15
    .2byte  0x0000
    .global FUN_0603F92C
FUN_0603F92C:
    .4byte  0x14586362
    .4byte  0x14395261
    .4byte  0x142A5362
    .4byte  0x000B143B
