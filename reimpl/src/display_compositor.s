/* display_compositor -- L3 assembly (SH-2 mnemonics)
 * Translation unit: 0x06005AE8 - 0x06006CDC
 * Auto-generated by tools/generate_l3_tu.py
 */

    .section .text.FUN_06005AE8


    .global display_compositor
    .type display_compositor, @function
display_compositor:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov.l r9, @-r15
    mov.l r8, @-r15
    sts.l pr, @-r15
    sts.l macl, @-r15
    mov.l   .L_pool_06005B90, r8
    mov.l   .L_pool_06005B94, r9
    mov.l   .L_pool_06005B98, r11
    mov.l   .L_pool_06005B9C, r12
    mov.l   .L_pool_06005BA0, r13
    mov.l   .L_pool_06005BA4, r14
    mov.w   DAT_06005b7c, r0
    mov.l   .L_pool_06005BA8, r3
    mov.l @r14, r14
    mov.l @r3, r3
    mov.l @(r0, r14), r7
    add #0x1, r7
    cmp/hi r3, r7
    bt/s    .L_06005B20
    mov #0x8, r10
    mov #0xC, r6
    mov.w   .L_wpool_06005B7E, r5
    jsr @r13
    mov #0x8, r4
.L_06005B20:
    mov.b @r8, r0
    tst r0, r0
    bt      .L_06005B2C
    mov.w   DAT_06005b80, r5
    bra     .L_06005B2E
    nop
.L_06005B2C:
    mov.w   DAT_06005b82, r5
.L_06005B2E:
    mov.l   .L_pool_06005BAC, r0
    mov.l @r0, r0
    tst r0, r0
    bt      .L_06005B3A
    bra     .L_06005B44
    mov.l @(8, r14), r7
.L_06005B3A:
    mov.l @(8, r14), r7
    mov.l   .L_pool_06005BB0, r3
    mul.l r3, r7
    sts macl, r7
    shlr16 r7
.L_06005B44:
    mov #0x24, r6
    jsr @r13
    mov #0x8, r4
    mov.l   .L_pool_06005BB4, r0
    mov.l @r0, r0
    tst r0, r0
    bf      .L_06005B60
    mov.w   DAT_06005b84, r0
    mov #0x30, r6
    mov.w   .L_wpool_06005B86, r5
    mov.l @(r0, r14), r7
    add #0x1, r7
    jsr @r13
    mov #0x8, r4
.L_06005B60:
    mov.b @r8, r0
    tst r0, r0
    bt      .L_06005B6C
    mov.w   DAT_06005b88, r5
    bra     .L_06005B6E
    nop
.L_06005B6C:
    mov.w   DAT_06005b8a, r5
.L_06005B6E:
    mov.l   .L_pool_06005BB8, r0
    mov.l @r0, r0
    tst r0, r0
    bt      .L_06005BBC
    mov.w   .L_wpool_06005B8C, r0
    bra     .L_06005BBE
    nop

    .global DAT_06005b7c
DAT_06005b7c:
    .2byte  0x021C
.L_wpool_06005B7E:
    .2byte  0x0108

    .global DAT_06005b80
DAT_06005b80:
    .2byte  0x0220

    .global DAT_06005b82
DAT_06005b82:
    .2byte  0x0320

    .global DAT_06005b84
DAT_06005b84:
    .2byte  0x0224
.L_wpool_06005B86:
    .2byte  0x0142

    .global DAT_06005b88
DAT_06005b88:
    .2byte  0x0236

    .global DAT_06005b8a
DAT_06005b8a:
    .2byte  0x0336
.L_wpool_06005B8C:
    .2byte  0x00DE
    .2byte  0xFFFF
.L_pool_06005B90:
    .4byte  sym_06085FF4
.L_pool_06005B94:
    .4byte  sym_0607EAAC
.L_pool_06005B98:
    .4byte  sym_06063E10
.L_pool_06005B9C:
    .4byte  sym_06063E0C
.L_pool_06005BA0:
    .4byte  sym_06028430
.L_pool_06005BA4:
    .4byte  sym_0607E944
.L_pool_06005BA8:
    .4byte  sym_06063F28
.L_pool_06005BAC:
    .4byte  sym_06078644
.L_pool_06005BB0:
    .4byte  0x00009F1A
.L_pool_06005BB4:
    .4byte  sym_0607EAE0
.L_pool_06005BB8:
    .4byte  sym_0607EAB8
.L_06005BBC:
    mov.w   DAT_06005c74, r0
.L_06005BBE:
    mov #0x6C, r6
    mov.w @(r0, r14), r7
    add #0x1, r7
    jsr @r13
    mov #0x8, r4
    mov.b @r8, r0
    tst r0, r0
    bf      .L_06005C60
    mov.l   .L_pool_06005C7C, r3
    mov.l   .L_pool_06005C80, r2
    mov.l @r3, r3
    and r2, r3
    tst r3, r3
    bt/s    .L_06005C1E
    mov #0x1, r4
    mov.l @r9, r0
    cmp/eq #0x78, r0
    bf      .L_06005BE8
    mov #0x3C, r2
    mov.l r2, @r12
    mov.l r10, @r11
.L_06005BE8:
    mov.l @r9, r3
    .2byte  0xE278
    .4byte  0x33238916
    .4byte  0x62B272FF
    .4byte  0x22288F12
    .4byte  0x2B22D522
    .4byte  0xD322430B
    .4byte  0xE40062C2
    .4byte  0x72FD2C22
    .4byte  0x632333A3
    .4byte  0x89002CA2
    .4byte  0x63C24321
    .4byte  0x43214321
    .4byte  0x73012B32
    .2byte  0xE400
.L_06005C1E:
    tst r4, r4
    bt      .L_06005C40
    mov.l @r9, r1
    mov.l   .L_pool_06005C8C, r3
    jsr @r3
    mov #0x14, r0
    mov r0, r7
    mov.w   DAT_06005c76, r4
    cmp/gt r4, r7
    bf      .L_06005C34
    mov r4, r7
.L_06005C34:
    mov #0x18, r6
    mov.w   .L_wpool_06005C78, r5
    jsr @r13
    mov #0x8, r4
    bra     .L_06005C60
    nop
.L_06005C40:
    mov.l   .L_pool_06005C90, r7
    mov #0x60, r6
    mov r6, r5
    add #0x46, r5
    mov #0x8, r4
    lds.l @r15+, macl
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    mov.l   .L_pool_06005C94, r3
    jmp @r3
    mov.l @r15+, r14
.L_06005C60:
    lds.l @r15+, macl
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14

    .global DAT_06005c74
DAT_06005c74:
    .2byte  0x00DC

    .global DAT_06005c76
DAT_06005c76:
    .2byte  0x03E7
.L_wpool_06005C78:
    .2byte  0x00A6
    .2byte  0xFFFF
.L_pool_06005C7C:
    .4byte  sym_0607EBC4
.L_pool_06005C80:
    .4byte  0x00020000
    .4byte  0xAE1114FF
    .4byte  sound_cmd_dispatch
.L_pool_06005C8C:
    .4byte  sym_06034FE0
.L_pool_06005C90:
    .4byte  sym_0605ACF3
.L_pool_06005C94:
    .4byte  sym_060284AE
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000
    .4byte  0x00000000

    .global segment_position_track
    .type segment_position_track, @function
segment_position_track:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov.l r9, @-r15
    mov.l r8, @-r15
    sts.l pr, @-r15
    add #-0x1C, r15
    mov.l   .L_pool_06005FC0, r8
    mov.l   .L_pool_06005FC4, r9
    mov.l   .L_pool_06005FC8, r10
    mov.l   .L_pool_06005FCC, r12
    mov.l   .L_pool_06005FD0, r14
    mov.l @r14, r14
    mov.l @(16, r14), r13
    mov.l @(20, r14), r3
    mov.l r3, @r15
    mov.l   .L_pool_06005FD4, r3
    jsr @r3
    mov.l @(24, r14), r11
    mov.l   .L_pool_06005FD8, r3
    jsr @r3
    nop
    mov.l @(32, r14), r4
    mov.l   .L_pool_06005FDC, r2
    mov.l   .L_pool_06005FE0, r3
    mov.l @r2, r2
    jsr @r3
    add r2, r4
    mov.l   .L_pool_06005FE4, r4
    mov.l   .L_pool_06005FE8, r3
    jsr @r3
    mov r9, r5
    mov.l   .L_pool_06005FEC, r4
    mov.l   .L_pool_06005FE8, r3
    jsr @r3
    mov r12, r5
    mov.l   .L_pool_06005FF0, r4
    mov.l   .L_pool_06005FE8, r3
    jsr @r3
    mov r8, r5
    mov.l   .L_pool_06005FF4, r4
    mov.l   .L_pool_06005FE8, r3
    jsr @r3
    mov r10, r5
    mov.l   .L_pool_06005FF8, r5
    mov.l   .L_pool_06005FFC, r4
    mov.l   .L_pool_06005FE8, r3
    jsr @r3
    nop
    mov.l   .L_pool_06006000, r5
    mov.l   .L_pool_06006004, r4
    mov.l   .L_pool_06005FE8, r3
    jsr @r3
    nop
    mov.l   .L_pool_06006008, r5
    mov.l   .L_pool_0600600C, r4
    mov.l   .L_pool_06005FE8, r3
    jsr @r3
    nop
    mov.l   .L_pool_06006010, r5
    mov.l   .L_pool_06006014, r4
    mov.l   .L_pool_06005FE8, r3
    jsr @r3
    nop
    mov.l @r9, r2
    add r13, r2
    mov.l r2, @r9
    mov.l @r15, r3
    mov.l @(4, r9), r2
    add r3, r2
    mov.l r2, @(4, r9)
    mov.l @(8, r9), r3
    add r11, r3
    mov.l r3, @(8, r9)
    mov.l @r12, r2
    add r13, r2
    mov.l r2, @r12
    mov.l @r15, r3
    mov.l @(4, r12), r2
    add r3, r2
    mov.l r2, @(4, r12)
    mov.l @(8, r12), r3
    add r11, r3
    mov.l r3, @(8, r12)
    mov.l @r8, r2
    add r13, r2
    mov.l r2, @r8
    mov.l @r15, r3
    mov.l @(4, r8), r2
    add r3, r2
    mov.l r2, @(4, r8)
    mov.l @(8, r8), r3
    add r11, r3
    mov.l r3, @(8, r8)
    mov.l @r10, r2
    add r13, r2
    mov.l r2, @r10
    mov.l @r15, r3
    mov.l @(4, r10), r2
    add r3, r2
    mov.l r2, @(4, r10)
    mov.l @(8, r10), r3
    add r11, r3
    mov.l r3, @(8, r10)
    mov.l   .L_pool_06005FF8, r4
    mov.l @r4, r2
    add r13, r2
    mov.l r2, @r4
    mov.l @(8, r4), r3
    add r11, r3
    mov.l r3, @(8, r4)
    mov.l   .L_pool_06006000, r4
    mov.l @r4, r2
    add r13, r2
    mov.l r2, @r4
    mov.l @(8, r4), r3
    add r11, r3
    bra     .L_06006018
    nop
    .2byte  0xFFFF
.L_pool_06005FC0:
    .4byte  sym_06063E88
.L_pool_06005FC4:
    .4byte  sym_06063E4C
.L_pool_06005FC8:
    .4byte  sym_06063E74
.L_pool_06005FCC:
    .4byte  sym_06063E60
.L_pool_06005FD0:
    .4byte  sym_0607E944
.L_pool_06005FD4:
    .4byte  sym_06026DBC
.L_pool_06005FD8:
    .4byte  sym_06026E0C
.L_pool_06005FDC:
    .4byte  sym_06063F10
.L_pool_06005FE0:
    .4byte  mat_rot_y
.L_pool_06005FE4:
    .4byte  sym_0604464C
.L_pool_06005FE8:
    .4byte  mat_vec_transform
.L_pool_06005FEC:
    .4byte  sym_06044640
.L_pool_06005FF0:
    .4byte  sym_06044658
.L_pool_06005FF4:
    .4byte  sym_06044664
.L_pool_06005FF8:
    .4byte  sym_06063E9C
.L_pool_06005FFC:
    .4byte  sym_06044688
.L_pool_06006000:
    .4byte  sym_06063EB0
.L_pool_06006004:
    .4byte  sym_0604467C
.L_pool_06006008:
    .4byte  sym_06063ED8
.L_pool_0600600C:
    .4byte  sym_06044694
.L_pool_06006010:
    .4byte  sym_06063EC4
.L_pool_06006014:
    .4byte  sym_060446A0
.L_06006018:
    mov.l r3, @(8, r4)
    mov r9, r6
    mov.l   .L_pool_0600611C, r4
    add #0xE, r6
    mov.l @r4, r2
    add r13, r2
    mov.l r2, @r4
    mov.l @(8, r4), r3
    add r11, r3
    mov.l r3, @(8, r4)
    mov.l   .L_pool_06006120, r4
    mov.l @r4, r2
    add r13, r2
    mov.l r2, @r4
    mov.l @(8, r4), r3
    add r11, r3
    mov.l r3, @(8, r4)
    mov.l r6, @-r15
    mov.l @(8, r9), r5
    mov.l   .L_pool_06006124, r3
    jsr @r3
    mov.l @r9, r4
    mov r0, r4
    mov r9, r5
    mov.l   .L_pool_06006128, r3
    jsr @r3
    mov.l @r15+, r6
    mov r12, r6
    add #0xE, r6
    mov.l r6, @-r15
    mov.l @(8, r12), r5
    mov.l   .L_pool_06006124, r3
    jsr @r3
    mov.l @r12, r4
    mov r0, r4
    mov r12, r5
    mov.l   .L_pool_06006128, r3
    jsr @r3
    mov.l @r15+, r6
    mov r8, r6
    add #0xE, r6
    mov.l r6, @-r15
    mov.l @(8, r8), r5
    mov.l   .L_pool_06006124, r3
    jsr @r3
    mov.l @r8, r4
    mov r0, r4
    mov r8, r5
    mov.l   .L_pool_06006128, r3
    jsr @r3
    mov.l @r15+, r6
    mov r10, r6
    add #0xE, r6
    mov.l r6, @-r15
    mov.l @(8, r10), r5
    mov.l   .L_pool_06006124, r3
    jsr @r3
    mov.l @r10, r4
    mov r0, r4
    mov r10, r5
    mov.l   .L_pool_06006128, r3
    jsr @r3
    mov.l @r15+, r6
    mov r15, r5
    mov.l @(4, r12), r2
    mov.l @(4, r9), r3
    mov.l @(4, r8), r1
    add r3, r2
    add r1, r2
    mov.l @(4, r10), r3
    add r3, r2
    mov r2, r3
    shar r3
    shar r3
    mov.l r3, @r15
    mov.l r3, @(20, r14)
    mov.l   .L_pool_0600612C, r4
    mov.l   .L_pool_06006130, r3
    jsr @r3
    add #0x10, r5
    mov.l @(16, r15), r2
    mov r15, r6
    add r13, r2
    add #0x4, r6
    mov.l r2, @(16, r15)
    mov.l @r15, r3
    mov.l @(20, r15), r2
    add r3, r2
    mov.l r2, @(20, r15)
    mov.l @(24, r15), r2
    add r11, r2
    mov.l r2, @(24, r15)
    mov.l r6, @-r15
    mov r15, r5
    add #0x14, r5
    mov.l r5, @-r15
    mov.l @(32, r15), r5
    mov.l   .L_pool_06006124, r2
    jsr @r2
    mov.l @(24, r15), r4
    mov r0, r4
    mov.l @r15+, r5
    mov.l   .L_pool_06006128, r3
    jsr @r3
    mov.l @r15+, r6
    mov.l @r15, r2
    mov.l @(20, r15), r3
    cmp/eq r3, r2
    bt      .L_06006104
    mov.l   .L_pool_06006134, r0
    mov.l @r0, r0
    cmp/eq #0x1, r0
    bf      .L_06006144
    mov.w   DAT_06006116, r0
    mov.w   .L_wpool_06006118, r2
    mov.l @(r0, r14), r3
    cmp/ge r2, r3
    bf      .L_06006144
.L_06006104:
    mov.l @(4, r9), r5
    mov.l @(4, r12), r2
    mov.l   .L_pool_06006138, r4
    mov.l   .L_pool_0600613C, r3
    jsr @r3
    add r2, r5
    mov.l   .L_pool_06006140, r13
    bra     .L_06006148
    mov r0, r11

    .global DAT_06006116
DAT_06006116:
    .2byte  0x01EC
.L_wpool_06006118:
    .2byte  0x00DC
    .2byte  0xFFFF
.L_pool_0600611C:
    .4byte  sym_06063ED8
.L_pool_06006120:
    .4byte  sym_06063EC4
.L_pool_06006124:
    .4byte  sym_06006838
.L_pool_06006128:
    .4byte  scene_render_alt
.L_pool_0600612C:
    .4byte  sym_060446AC
.L_pool_06006130:
    .4byte  mat_vec_transform
.L_pool_06006134:
    .4byte  sym_0607EAD8
.L_pool_06006138:
    .4byte  0x00008000
.L_pool_0600613C:
    .4byte  fpmul
.L_pool_06006140:
    .4byte  0x0002CCCC
.L_06006144:
    mov.l @(20, r15), r11
    mov.l   .L_pool_060061B0, r13
.L_06006148:
    mov.l @(4, r10), r5
    mov.l @(4, r8), r3
    mov.l   .L_pool_060061B4, r4
    add r3, r5
    mov.l   .L_pool_060061B8, r3
    jsr @r3
    nop
    mov.l r0, @(8, r15)
    mov.l @(4, r10), r5
    mov.l @(4, r9), r3
    mov.l   .L_pool_060061B4, r4
    add r3, r5
    mov.l   .L_pool_060061B8, r3
    jsr @r3
    nop
    mov.l r0, @r15
    mov.l @(4, r8), r5
    mov.l @(4, r12), r3
    mov.l   .L_pool_060061B4, r4
    add r3, r5
    mov.l   .L_pool_060061B8, r3
    jsr @r3
    nop
    mov r11, r4
    mov.l r0, @(12, r15)
    mov.l @(8, r15), r3
    sub r3, r4
    mov.l   .L_pool_060061BC, r3
    jsr @r3
    mov r13, r5
    mov.l r0, @(28, r14)
    mov.l @r15, r4
    mov.l @(12, r15), r2
    mov.l   .L_pool_060061C0, r5
    mov.l   .L_pool_060061BC, r3
    jsr @r3
    sub r2, r4
    mov.l r0, @(36, r14)
    mov.l   .L_pool_060061C4, r4
    mov.l @r4, r2
    add #-0x30, r2
    mov.l r2, @r4
    add #0x1C, r15
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14
.L_pool_060061B0:
    .4byte  0x000D6666
.L_pool_060061B4:
    .4byte  0x00008000
.L_pool_060061B8:
    .4byte  fpmul
.L_pool_060061BC:
    .4byte  atan2
.L_pool_060061C0:
    .4byte  0x00011998
.L_pool_060061C4:
    .4byte  sym_06089EDC

    .global pre_render_transform
    .type pre_render_transform, @function
pre_render_transform:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    sts.l pr, @-r15
    mov.l   .L_pool_06006264, r11
    mov.l   .L_pool_06006268, r3
    mov.l @r11, r11
    mov.l @(16, r11), r14
    mov.l @(20, r11), r13
    jsr @r3
    mov.l @(24, r11), r12
    mov.l   .L_pool_0600626C, r3
    jsr @r3
    nop
    mov.l   .L_pool_06006270, r3
    jsr @r3
    mov.l @(48, r11), r4
    mov.l   .L_pool_06006274, r11
    mov.l   .L_pool_06006278, r5
    mov.l   .L_pool_0600627C, r4
    jsr @r11
    nop
    mov.l   .L_pool_06006280, r5
    mov.l   .L_pool_06006284, r4
    jsr @r11
    nop
    mov.l   .L_pool_06006288, r5
    mov.l   .L_pool_0600628C, r4
    jsr @r11
    nop
    mov.l   .L_pool_06006290, r5
    mov.l   .L_pool_06006294, r4
    jsr @r11
    nop
    mov.l   .L_pool_06006278, r4
    mov.l @r4, r2
    add r14, r2
    mov.l r2, @r4
    mov.l r13, @(4, r4)
    mov.l @(8, r4), r3
    add r12, r3
    mov.l r3, @(8, r4)
    mov.l   .L_pool_06006280, r4
    mov.l @r4, r2
    add r14, r2
    mov.l r2, @r4
    mov.l r13, @(4, r4)
    mov.l @(8, r4), r3
    add r12, r3
    mov.l r3, @(8, r4)
    mov.l   .L_pool_06006288, r4
    mov.l @r4, r2
    add r14, r2
    mov.l r2, @r4
    mov.l r13, @(4, r4)
    mov.l @(8, r4), r3
    add r12, r3
    mov.l r3, @(8, r4)
    mov.l   .L_pool_06006290, r4
    mov.l @r4, r2
    add r14, r2
    mov.l r2, @r4
    mov.l r13, @(4, r4)
    mov.l @(8, r4), r3
    add r12, r3
    mov.l r3, @(8, r4)
    mov.l   .L_pool_06006298, r4
    mov.l @r4, r2
    add #-0x30, r2
    mov.l r2, @r4
    lds.l @r15+, pr
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14
    .2byte  0xFFFF
.L_pool_06006264:
    .4byte  sym_0607E940
.L_pool_06006268:
    .4byte  sym_06026DBC
.L_pool_0600626C:
    .4byte  sym_06026E0C
.L_pool_06006270:
    .4byte  mat_rot_y
.L_pool_06006274:
    .4byte  mat_vec_transform
.L_pool_06006278:
    .4byte  sym_06063E9C
.L_pool_0600627C:
    .4byte  sym_0604464C
.L_pool_06006280:
    .4byte  sym_06063EB0
.L_pool_06006284:
    .4byte  sym_06044640
.L_pool_06006288:
    .4byte  sym_06063ED8
.L_pool_0600628C:
    .4byte  sym_06044658
.L_pool_06006290:
    .4byte  sym_06063EC4
.L_pool_06006294:
    .4byte  sym_06044664
.L_pool_06006298:
    .4byte  sym_06089EDC

    .global car_array_init
    .type car_array_init, @function
car_array_init:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov #0x0, r13
    mov.l r11, @-r15
    extu.w r13, r1
    extu.w r13, r0
    extu.w r13, r3
    mov.l r10, @-r15
    mov.l r9, @-r15
    mov.l r8, @-r15
    sts.l pr, @-r15
    sts.l macl, @-r15
    mov.l   .L_pool_06006364, r8
    mov.w   .L_wpool_0600635E, r9
    mov.l   .L_pool_06006368, r10
    mov.l   .L_pool_0600636C, r12
    mov.l   .L_pool_06006370, r14
    mov.l   .L_pool_06006374, r2
    mov.w r13, @r2
    mov.l   .L_pool_06006378, r2
    mov.w r1, @r2
    mov.l   .L_pool_0600637C, r2
    mov.w r0, @r2
    mov.l   .L_pool_06006380, r2
    mov.w r3, @r2
    mov.l   .L_pool_06006384, r1
    mov.b @r1, r1
    extu.b r1, r1
    tst r1, r1
    bt      .L_0600635A
    mov r8, r3
    add r9, r3
    mov.l r3, @r14
    mov r3, r2
    mov.l r3, @r12
    mov.l @r14, r3
    jsr @r10
    mov.l r13, @(4, r3)
    mov.l @r12, r2
    mov #0x38, r3
    mov #0x74, r0
    mov.l r3, @(r0, r2)
    mov.l @r12, r3
    mov #0x38, r2
    add #0x1C, r0
    mov.l r2, @(r0, r3)
    mov.l   .L_pool_06006388, r4
    add #0xE, r0
    mov.l @r12, r3
    mov.b @r4, r2
    extu.b r2, r2
    mov.w r2, @(r0, r3)
    mov.l @r12, r3
    mov.b @(1, r4), r0
    mov r0, r2
    extu.b r2, r2
    mov #0x7C, r0
    mov.w r2, @(r0, r3)
    mov.l @r12, r3
    mov.b @(2, r4), r0
    mov r0, r2
    extu.b r2, r2
    mov.w   DAT_06006360, r0
    mov.w r2, @(r0, r3)
    mov.l   .L_pool_0600638C, r0
    mov.l @r0, r0
    tst r0, r0
    bt      .L_06006338
    mov.l @r14, r3
    mov.w   DAT_06006362, r0
    mov.w r13, @(r0, r3)
    mov.l @r14, r3
    exts.w r13, r2
    add #0x2, r0
    mov.w r2, @(r0, r3)
    bra     .L_06006348
    nop
.L_06006338:
    mov.l @r14, r2
    mov #0x3, r3
    mov.w   DAT_06006362, r0
    mov.w r3, @(r0, r2)
    mov.l @r14, r3
    mov #0x3, r2
    add #0x2, r0
    mov.w r2, @(r0, r3)
.L_06006348:
    mov r8, r3
    mov.l r8, @r14
    mov r3, r2
    mov.l r3, @r12
    mov.l @r14, r3
    jsr @r10
    mov.l r13, @(4, r3)
    bra     .L_060063A8
    nop
.L_0600635A:
    bra     .L_060063A0
    mov r13, r11
.L_wpool_0600635E:
    .2byte  0x0268

    .global DAT_06006360
DAT_06006360:
    .2byte  0x009C

    .global DAT_06006362
DAT_06006362:
    .2byte  0x00DC
.L_pool_06006364:
    .4byte  sym_06078900
.L_pool_06006368:
    .4byte  car_type_init
.L_pool_0600636C:
    .4byte  sym_0607E944
.L_pool_06006370:
    .4byte  sym_0607E940
.L_pool_06006374:
    .4byte  sym_0605DF4E
.L_pool_06006378:
    .4byte  sym_0605DF50
.L_pool_0600637C:
    .4byte  sym_0605DF52
.L_pool_06006380:
    .4byte  sym_0605DF54
.L_pool_06006384:
    .4byte  sym_06078635
.L_pool_06006388:
    .4byte  sym_0607ED90
.L_pool_0600638C:
    .4byte  sym_0607EAD8
.L_06006390:
    mul.l r9, r11
    sts macl, r2
    add r8, r2
    mov r2, r3
    mov.l r2, @r14
    jsr @r10
    mov.l r11, @(4, r3)
    add #0x1, r11
.L_060063A0:
    mov.l   .L_pool_06006450, r2
    mov.l @r2, r2
    cmp/ge r2, r11
    bf      .L_06006390
.L_060063A8:
    mov.l @r12, r2
    mov #0x38, r3
    mov #0x74, r0
    mov.l r3, @(r0, r2)
    mov.l @r12, r3
    mov #0x38, r2
    add #0x1C, r0
    mov.l r2, @(r0, r3)
    mov.l @r12, r3
    add #0xC, r0
    mov.l   .L_pool_06006454, r2
    mov.l @r2, r2
    exts.w r2, r2
    mov.w r2, @(r0, r3)
    mov.l @r12, r3
    add #0x2, r0
    mov.l   .L_pool_06006458, r2
    mov.w @r2, r2
    mov.w r2, @(r0, r3)
    mov #0x7C, r0
    mov.l @r12, r3
    mov.l   .L_pool_0600645C, r2
    mov.l @r2, r2
    mov.w r2, @(r0, r3)
    mov.l   .L_pool_06006460, r3
    jsr @r3
    nop
    mov.l   .L_pool_06006464, r3
    jsr @r3
    nop
    mov.l   .L_pool_06006468, r0
    mov.l @r0, r0
    cmp/eq #0xE, r0
    bf      .L_060063F4
    mov.l   .L_pool_0600646C, r0
    mov.l @r0, r0
    tst r0, r0
    bt      .L_060063FE
.L_060063F4:
    mov #0x0, r3
    mov.l   .L_pool_06006470, r2
    mov.l r3, @r2
    bra     .L_06006404
    nop
.L_060063FE:
    mov #0x1, r3
    mov.l   .L_pool_06006470, r2
    mov.l r3, @r2
.L_06006404:
    mov.l @r12, r4
    mov.l   .L_pool_06006474, r3
    mov.l @(32, r4), r2
    mov.l r2, @r3
    add #0x4, r3
    mov.l r13, @r3
    mov.l   .L_pool_06006478, r3
    mov.l r13, @r3
    mov.l   .L_pool_0600647C, r3
    mov.l r13, @r3
    mov r3, r2
    mov.l @r2, r2
    mov.l   .L_pool_06006480, r3
    mov.l r2, @r3
    mov r3, r2
    mov.l @r2, r2
    mov.l   .L_pool_06006484, r3
    mov.l r2, @r3
    mov r3, r2
    mov.l @r2, r2
    mov.l   .L_pool_06006488, r3
    mov.l r2, @r3
    mov.l   .L_pool_0600648C, r2
    mov.l   .L_pool_06006490, r3
    mov.l r2, @r3
    mov.l   .L_pool_0600646C, r0
    mov.l @r0, r0
    tst r0, r0
    bt      .L_06006494
    mov.w   .L_wpool_0600644C, r0
    mov.w r13, @(r0, r4)
    exts.w r13, r3
    add #0x2, r0
    mov.w r3, @(r0, r4)
    bra     .L_060064A0
    nop
.L_wpool_0600644C:
    .2byte  0x00DC
    .2byte  0xFFFF
.L_pool_06006450:
    .4byte  sym_0607EA98
.L_pool_06006454:
    .4byte  sym_0607EAB8
.L_pool_06006458:
    .4byte  sym_06063F42
.L_pool_0600645C:
    .4byte  sym_06078868
.L_pool_06006460:
    .4byte  sym_0600D280
.L_pool_06006464:
    .4byte  sym_0602E5E4
.L_pool_06006468:
    .4byte  sym_0607EBC0
.L_pool_0600646C:
    .4byte  sym_0607EAD8
.L_pool_06006470:
    .4byte  sym_0607EAD0
.L_pool_06006474:
    .4byte  sym_06063EF0
.L_pool_06006478:
    .4byte  sym_06063F10
.L_pool_0600647C:
    .4byte  sym_06063E78
.L_pool_06006480:
    .4byte  sym_06063E8C
.L_pool_06006484:
    .4byte  sym_06063E64
.L_pool_06006488:
    .4byte  sym_06063E50
.L_pool_0600648C:
    .4byte  0xFEA00000
.L_pool_06006490:
    .4byte  sym_06063F14
.L_06006494:
    mov #0x3, r2
    mov.w   .L_wpool_0600654C, r0
    mov.w r2, @(r0, r4)
    mov r2, r3
    add #0x2, r0
    mov.w r3, @(r0, r4)
.L_060064A0:
    mov.l   .L_pool_06006550, r3
    mov #0x2, r2
    mov #0x0, r5
    mov.l r13, @r3
    mov.l   .L_pool_06006554, r3
    mov.l r2, @r3
    mov #0x1, r2
    mov.l   .L_pool_06006558, r3
    mov.l r2, @r3
    mov.l   .L_pool_0600655C, r3
    jsr @r3
    mov #0x8, r4
    mov.l   .L_pool_06006560, r2
    mov.l   .L_pool_06006564, r3
    mov.l r2, @r3
    mov.l   .L_pool_06006568, r4
    mov.l   .L_pool_0600656C, r3
    mov.l r4, @r3
    mov.l   .L_pool_06006570, r2
    mov.l   .L_pool_06006574, r3
    mov.l r2, @r3
    mov.l   .L_pool_06006578, r2
    mov.l   .L_pool_0600657C, r3
    mov.l r2, @r3
    mov.l   .L_pool_06006580, r3
    mov.l r13, @r3
    mov.l   .L_pool_06006584, r3
    mov.l r4, @r3
    mov #0x2, r2
    mov.l   .L_pool_06006588, r3
    mov.l r2, @r3
    lds.l @r15+, macl
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    bra     coord_grid_pack
    mov.l @r15+, r14

    .global scene_fallback_render
    .type scene_fallback_render, @function
scene_fallback_render:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    sts.l pr, @-r15
    mov.l   .L_pool_0600658C, r11
    mov.l   .L_pool_06006590, r12
    mov.l   .L_pool_06006554, r13
    mov.l   .L_pool_06006564, r14
    mov.l   .L_pool_06006588, r3
    mov.l   .L_pool_06006594, r2
    mov.l @r3, r3
    shll2 r3
    add r2, r3
    mov.l @r3, r1
    mov.l r1, @r12
    mov.l @r14, r3
    cmp/gt r11, r3
    bt/s    .L_06006532
    mov #0x1, r10
    mov.l   .L_pool_06006598, r0
    mov.b @r0, r0
    extu.b r0, r0
    cmp/eq #0x4, r0
    bt      .L_06006532
    mov #0x5, r5
    mov.l   .L_pool_0600655C, r3
    jsr @r3
    mov #0x8, r4
    bra     .L_0600653A
    nop
.L_06006532:
    mov #0x0, r5
    mov.l   .L_pool_0600655C, r3
    jsr @r3
    mov #0x8, r4
.L_0600653A:
    mov.l @r14, r2
    mov.l @r12, r3
    cmp/eq r3, r2
    bf      .L_0600659C
    mov.l   .L_pool_06006588, r3
    mov.l @r3, r3
    mov.l r3, @r13
    bra     .L_06006780
    nop
.L_wpool_0600654C:
    .2byte  0x00DC
    .2byte  0xFFFF
.L_pool_06006550:
    .4byte  sym_060620D0
.L_pool_06006554:
    .4byte  sym_06063E1C
.L_pool_06006558:
    .4byte  sym_06059F30
.L_pool_0600655C:
    .4byte  channel_nibble_config
.L_pool_06006560:
    .4byte  0x00058000
.L_pool_06006564:
    .4byte  sym_06063E24
.L_pool_06006568:
    .4byte  0x0000F300
.L_pool_0600656C:
    .4byte  sym_06063E34
.L_pool_06006570:
    .4byte  0x006E0000
.L_pool_06006574:
    .4byte  sym_06063E28
.L_pool_06006578:
    .4byte  0x00100000
.L_pool_0600657C:
    .4byte  sym_06063E2C
.L_pool_06006580:
    .4byte  sym_06063E30
.L_pool_06006584:
    .4byte  sym_06063EEC
.L_pool_06006588:
    .4byte  sym_06063E20
.L_pool_0600658C:
    .4byte  0x00008000
.L_pool_06006590:
    .4byte  sym_06063F4C
.L_pool_06006594:
    .4byte  sym_060446B8
.L_pool_06006598:
    .4byte  sym_06078654
.L_0600659C:
    mov.l   .L_pool_0600666C, r0
    mov.l   .L_pool_06006670, r1
    mov.l   .L_pool_06006674, r7
    mov.l   .L_pool_06006678, r6
    mov.l   .L_pool_0600667C, r5
    mov.l   .L_pool_06006680, r4
    mov.l @r12, r2
    mov.l @r14, r3
    cmp/ge r3, r2
    bt      .L_060066A0
    mov.l @r14, r3
    mov.l   .L_pool_06006684, r2
    cmp/gt r2, r3
    bf      .L_060065D6
    mov #0x4, r2
    mov.l r2, @r13
    mov.l @r14, r3
    sub r1, r3
    mov.l r3, @r14
    mov.w   .L_wpool_06006662, r2
    mov.l @r4, r3
    sub r2, r3
    mov.l r3, @r4
    mov.l   .L_pool_06006688, r2
    mov.l @r5, r3
    sub r2, r3
    mov.l r3, @r5
    bra     .L_06006780
    nop
.L_060065D6:
    mov.l @r14, r2
    cmp/gt r0, r2
    bf      .L_060065FA
    mov #0x3, r2
    mov.l r2, @r13
    mov.w   .L_wpool_06006664, r3
    mov.l @r14, r2
    sub r3, r2
    mov.l r2, @r14
    mov #-0x80, r3
    mov.l @r4, r2
    sub r3, r2
    mov.l r2, @r4
    mov.l   .L_pool_0600668C, r3
    mov.l @r5, r2
    sub r3, r2
    bra     .L_0600665E
    mov.l r2, @r5
.L_060065FA:
    mov.l @r14, r2
    cmp/gt r11, r2
    bf      .L_06006622
    mov #0x2, r2
    mov.l r2, @r13
    mov.l   .L_pool_06006690, r3
    mov.l @r14, r2
    sub r3, r2
    mov.l r2, @r14
    mov.l @r4, r3
    add #0x40, r3
    mov.l r3, @r4
    mov.l @r5, r2
    sub r1, r2
    mov.l r2, @r5
    mov.w   DAT_06006666, r3
    mov.l @r7, r2
    sub r3, r2
    bra     .L_0600665E
    mov.l r2, @r7
.L_06006622:
    mov.l @r14, r2
    cmp/pl r2
    bf      .L_0600665A
    mov.l r10, @r13
    mov.w   DAT_06006668, r2
    mov.l @r14, r3
    sub r2, r3
    mov.l r3, @r14
    mov.w   .L_wpool_0600666A, r2
    mov.l @r4, r3
    sub r2, r3
    mov.l r3, @r4
    mov.l @r5, r2
    sub r0, r2
    mov.l r2, @r5
    mov.l   .L_pool_06006694, r3
    mov.l @r6, r2
    sub r3, r2
    mov.l r2, @r6
    mov.l   .L_pool_06006698, r0
    mov.b @r0, r0
    tst r0, r0
    bt      .L_0600665E
    mov.l   .L_pool_0600669C, r3
    mov.l @r6, r2
    sub r3, r2
    bra     .L_0600665E
    mov.l r2, @r6
.L_0600665A:
    mov #0x0, r2
    mov.l r2, @r13
.L_0600665E:
    bra     .L_06006780
    nop
.L_wpool_06006662:
    .2byte  0xFDC0
.L_wpool_06006664:
    .2byte  0x4000

    .global DAT_06006666
DAT_06006666:
    .2byte  0xE000

    .global DAT_06006668
DAT_06006668:
    .2byte  0x1000
.L_wpool_0600666A:
    .2byte  0xFEA0
.L_pool_0600666C:
    .4byte  0x00058000
.L_pool_06006670:
    .4byte  0x00010000
.L_pool_06006674:
    .4byte  sym_06063E30
.L_pool_06006678:
    .4byte  sym_06063E2C
.L_pool_0600667C:
    .4byte  sym_06063E28
.L_pool_06006680:
    .4byte  sym_06063E34
.L_pool_06006684:
    .4byte  0x00078000
.L_pool_06006688:
    .4byte  0x0003C000
.L_pool_0600668C:
    .4byte  0x0001C000
.L_pool_06006690:
    .4byte  0x0000A000
.L_pool_06006694:
    .4byte  0x000D0000
.L_pool_06006698:
    .4byte  sym_06083255
.L_pool_0600669C:
    .4byte  0x000A0000
.L_060066A0:
    mov.l @r12, r2
    mov.l @r14, r3
    cmp/gt r3, r2
    bf      .L_06006780
    mov.l @r14, r3
    cmp/ge r11, r3
    bt      .L_060066E2
    mov #0x0, r3
    mov.l r3, @r13
    mov.w   DAT_06006752, r2
    mov.l @r14, r3
    add r2, r3
    mov.l r3, @r14
    mov.w   .L_wpool_06006754, r2
    mov.l @r4, r3
    add r2, r3
    mov.l r3, @r4
    mov.l @r5, r2
    add r0, r2
    mov.l r2, @r5
    mov.l   .L_pool_0600675C, r3
    mov.l @r6, r2
    add r3, r2
    mov.l r2, @r6
    mov.l   .L_pool_06006760, r0
    mov.b @r0, r0
    tst r0, r0
    bt      .L_06006780
    mov.l   .L_pool_06006764, r3
    mov.l @r6, r2
    add r3, r2
    bra     .L_06006780
    mov.l r2, @r6
.L_060066E2:
    mov.l @r14, r2
    cmp/ge r0, r2
    bt      .L_0600670A
    mov.l r10, @r13
    mov.l   .L_pool_06006768, r2
    mov.l @r14, r3
    add r2, r3
    mov.l r3, @r14
    mov.l @r4, r2
    add #-0x40, r2
    mov.l r2, @r4
    mov.l @r5, r3
    add r1, r3
    mov.l r3, @r5
    mov.w   DAT_06006756, r2
    mov.l @r7, r3
    add r2, r3
    mov.l r3, @r7
    bra     .L_06006780
    nop
.L_0600670A:
    mov.l @r14, r2
    mov.l   .L_pool_0600676C, r3
    cmp/ge r3, r2
    bt      .L_0600672E
    mov #0x2, r3
    mov.l r3, @r13
    mov.w   DAT_06006758, r2
    mov.l @r14, r3
    add r2, r3
    mov.l r3, @r14
    mov.l @r4, r2
    add #-0x80, r2
    mov.l r2, @r4
    mov.l   .L_pool_06006770, r3
    mov.l @r5, r2
    add r3, r2
    bra     .L_06006780
    mov.l r2, @r5
.L_0600672E:
    mov.l @r14, r2
    mov.l   .L_pool_06006774, r3
    cmp/ge r3, r2
    bt      .L_0600677C
    mov #0x3, r3
    mov.l r3, @r13
    mov.l @r14, r2
    add r1, r2
    mov.l r2, @r14
    mov.w   .L_wpool_0600675A, r3
    mov.l @r4, r2
    add r3, r2
    mov.l r2, @r4
    mov.l   .L_pool_06006778, r3
    mov.l @r5, r2
    add r3, r2
    bra     .L_06006780
    mov.l r2, @r5

    .global DAT_06006752
DAT_06006752:
    .2byte  0x1000
.L_wpool_06006754:
    .2byte  0xFEA0

    .global DAT_06006756
DAT_06006756:
    .2byte  0xE000

    .global DAT_06006758
DAT_06006758:
    .2byte  0x4000
.L_wpool_0600675A:
    .2byte  0xFDC0
.L_pool_0600675C:
    .4byte  0x000D0000
.L_pool_06006760:
    .4byte  sym_06083255
.L_pool_06006764:
    .4byte  0x000A0000
.L_pool_06006768:
    .4byte  0x0000A000
.L_pool_0600676C:
    .4byte  0x00078000
.L_pool_06006770:
    .4byte  0x0001C000
.L_pool_06006774:
    .4byte  0x00178000
.L_pool_06006778:
    .4byte  0x0003C000
.L_0600677C:
    mov #0x4, r2
    mov.l r2, @r13
.L_06006780:
    mov.w   DAT_06006802, r5
    mov.l   .L_pool_06006808, r3
    jsr @r3
    mov.l @r14, r4
    mov.l   .L_pool_0600680C, r3
    mov.l r0, @r3
    mov.w   DAT_06006802, r5
    mov.l   .L_pool_06006808, r3
    jsr @r3
    mov.l @r14, r4
    mov.l   .L_pool_06006810, r3
    mov.l r0, @r3
    mov.l   .L_pool_06006814, r4
    mov.l @r13, r1
    tst r1, r1
    bt      .L_060067B0
    mov.l @r13, r0
    cmp/eq #0x1, r0
    bf      .L_060067B8
    mov #0x1, r3
    mov.l   .L_pool_06006818, r2
    mov.l @r2, r2
    cmp/hi r3, r2
    bt      .L_060067B8
.L_060067B0:
    mov #0x0, r3
    mov.l r3, @r4
    bra     .L_060067BA
    nop
.L_060067B8:
    mov.l r10, @r4
.L_060067BA:
    lds.l @r15+, pr
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14

    .global coord_grid_pack
    .type coord_grid_pack, @function
coord_grid_pack:
    sts.l pr, @-r15
    mov.l   .L_pool_0600681C, r3
    jsr @r3
    nop
    extu.w r0, r5
    mov.l   .L_pool_06006820, r4
    mov.l   .L_pool_06006824, r3
    jsr @r3
    mov.b @r4, r4
    mov.l   .L_pool_06006828, r7
    mov.l   .L_pool_0600682C, r6
    mov.w   DAT_06006804, r5
    mov.l   .L_pool_06006830, r4
    mov.l   .L_pool_06006834, r0
    mov.b @r0, r0
    tst r0, r0
    bf      .L_060067F4
    extu.w r5, r5
    mov.w r5, @r6
    extu.w r4, r4
    bra     .L_060067FC
    mov.w r4, @r7
.L_060067F4:
    extu.w r4, r4
    mov.w r4, @r6
    extu.w r5, r5
    mov.w r5, @r7
.L_060067FC:
    lds.l @r15+, pr
    rts
    nop

    .global DAT_06006802
DAT_06006802:
    .2byte  0x1999

    .global DAT_06006804
DAT_06006804:
    .2byte  0x4000
    .2byte  0xFFFF
.L_pool_06006808:
    .4byte  fpmul
.L_pool_0600680C:
    .4byte  sym_06063F04
.L_pool_06006810:
    .4byte  sym_06063F08
.L_pool_06006814:
    .4byte  sym_06059F30
.L_pool_06006818:
    .4byte  sym_06063E20
.L_pool_0600681C:
    .4byte  sym_0601A5F8
.L_pool_06006820:
    .4byte  sym_0605D240
.L_pool_06006824:
    .4byte  sprite_pair_render
.L_pool_06006828:
    .4byte  sym_06063F4A
.L_pool_0600682C:
    .4byte  sym_06063F48
.L_pool_06006830:
    .4byte  0x00008000
.L_pool_06006834:
    .4byte  sym_06078663

    .global sym_06006838
sym_06006838:
    mov.l   .L_pool_06006860, r3
    mov.l   .L_pool_06006864, r2
    sub r5, r3
    add r2, r4
    mov r3, r5
    shlr16 r4
    shlr16 r5
    shlr2 r4
    shlr2 r5
    shlr2 r4
    shlr2 r5
    shlr r4
    shlr r5
    shll2 r5
    shll2 r5
    shll2 r5
    mov r5, r0
    rts
    add r4, r0
    .2byte  0xFFFF
.L_pool_06006860:
    .4byte  0x03FFFFFF
.L_pool_06006864:
    .4byte  0x04000000

    .global scene_post_render
    .type scene_post_render, @function
scene_post_render:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov.l r9, @-r15
    mov.l r8, @-r15
    sts.l pr, @-r15
    add #-0x2C, r15
    mov.l   .L_pool_06006928, r14
    mov.l   .L_pool_0600692C, r4
    mov.l   .L_pool_06006930, r2
    mov.l @r4, r4
    mov.l @(20, r4), r3
    mov.l r3, @r2
    mov.l   .L_pool_06006934, r4
    mov.l @r14, r0
    shll2 r0
    shll r0
    mov.l @(r0, r4), r3
    mov.l r3, @(4, r15)
    mov.l @r14, r0
    shll r0
    add #0x1, r0
    shll2 r0
    mov.l @(r0, r4), r2
    mov.l r2, @(8, r15)
    mov.l   .L_pool_06006938, r4
    mov.l @r14, r0
    shll2 r0
    shll2 r0
    mov.l @(r0, r4), r1
    mov.l r1, @(16, r15)
    mov.l @r14, r5
    shll2 r5
    mov r5, r0
    add #0x1, r0
    shll2 r0
    mov.l @(r0, r4), r3
    mov r5, r0
    mov.l r3, @(28, r15)
    add #0x2, r0
    shll2 r0
    mov.l @(r0, r4), r2
    mov r5, r0
    mov.l r2, @(24, r15)
    add #0x3, r0
    shll2 r0
    mov.l @(r0, r4), r1
    mov.l r1, @(20, r15)
    mov.l   .L_pool_0600693C, r5
    mov.l   .L_pool_06006940, r3
    mov.l @r5, r4
    add r3, r4
    shlr16 r4
    shlr2 r4
    shlr2 r4
    shlr r4
    mov.l @(8, r5), r2
    neg r2, r2
    add #-0x1, r3
    add r3, r2
    mov r2, r5
    shlr16 r5
    mov #0x40, r3
    shlr2 r5
    shlr2 r5
    shlr r5
    mov r5, r2
    mov r5, r0
    shll2 r2
    shll2 r2
    shll2 r2
    add r4, r2
    mov.l r2, @(32, r15)
    mov.l r3, @(36, r15)
    mov #-0x1, r3
    mov.l r3, @(40, r15)
    mov #0x1, r3
    mov.l r3, @(12, r15)
    cmp/eq #0x3F, r0
    bf/s    .L_06006914
    mov #-0x40, r6
    mov #0x0, r2
    bra     .L_0600691A
    mov.l r2, @(36, r15)
.L_06006914:
    tst r5, r5
    bf      .L_0600691A
    mov #0x0, r6
.L_0600691A:
    mov r4, r0
    cmp/eq #0x3F, r0
    bf      .L_06006944
    mov #0x0, r2
    bra     .L_0600694C
    mov.l r2, @(12, r15)
    .2byte  0xFFFF
.L_pool_06006928:
    .4byte  sym_0607EAD8
.L_pool_0600692C:
    .4byte  sym_0607E944
.L_pool_06006930:
    .4byte  sym_06063F54
.L_pool_06006934:
    .4byte  sym_06062248
.L_pool_06006938:
    .4byte  sym_06062260
.L_pool_0600693C:
    .4byte  sym_06063DF8
.L_pool_06006940:
    .4byte  0x04000000
.L_06006944:
    tst r4, r4
    bf      .L_0600694C
    mov #0x0, r3
    mov.l r3, @(40, r15)
.L_0600694C:
    mov.l   .L_pool_060069E4, r10
    bra     .L_06006A14
    mov.l r6, @r15
.L_06006952:
    mov.l @(40, r15), r12
    mov.l @r15, r9
    mov.l @(32, r15), r3
    mov.l @(12, r15), r2
    add r12, r9
    cmp/gt r2, r12
    bt/s    .L_06006A0E
    add r3, r9
.L_06006962:
    mov r9, r11
    mov r9, r4
    mov.l @(16, r15), r7
    mov.l @(24, r15), r3
    shll r4
    add r4, r7
    add r3, r4
    mov.w @r7, r7
    exts.w r7, r2
    cmp/pl r2
    bf/s    .L_060069AE
    mov.w @r4, r13
    mov r11, r0
    mov.l @(28, r15), r2
    mov.l @r14, r8
    mov.l   .L_pool_060069E8, r3
    shll2 r0
    shll2 r8
    mov.l @(r0, r2), r0
    shll r8
    add r3, r8
    mov.l @r8, r8
    add r0, r8
    mov.l @r14, r0
    cmp/eq #0x2, r0
    bt      .L_060069A4
    mov.w @r10, r6
    mov.l @(4, r15), r5
    mov.l   .L_pool_060069EC, r3
    jsr @r3
    mov r8, r4
    bra     .L_060069AE
    nop
.L_060069A4:
    mov.w @r10, r6
    mov.l @(4, r15), r5
    mov.l   .L_pool_060069F0, r3
    jsr @r3
    mov r8, r4
.L_060069AE:
    exts.w r13, r2
    cmp/pl r2
    bf      .L_06006A04
    mov r11, r0
    mov.l @(20, r15), r1
    mov.l @r14, r2
    mov.l   .L_pool_060069E8, r3
    shll2 r0
    shll r2
    add #0x1, r2
    shll2 r2
    add r3, r2
    mov.l @r2, r11
    mov.l @(r0, r1), r2
    mov.l @r14, r0
    cmp/eq #0x2, r0
    bt/s    .L_060069F8
    add r2, r11
    mov r13, r7
    mov.w @r10, r6
    mov.l @(8, r15), r5
    mov.l   .L_pool_060069F4, r3
    jsr @r3
    mov r11, r4
    bra     .L_06006A04
    nop
    .2byte  0xFFFF
.L_pool_060069E4:
    .4byte  sym_06089E96
.L_pool_060069E8:
    .4byte  sym_06062230
.L_pool_060069EC:
    .4byte  sym_06029BF4
.L_pool_060069F0:
    .4byte  sym_0602B328
.L_pool_060069F4:
    .4byte  sym_0602A834
.L_060069F8:
    mov r13, r7
    mov.w @r10, r6
    mov.l @(8, r15), r5
    mov.l   .L_pool_06006A80, r3
    jsr @r3
    mov r11, r4
.L_06006A04:
    add #0x1, r12
    mov.l @(12, r15), r2
    cmp/gt r2, r12
    bf/s    .L_06006962
    add #0x1, r9
.L_06006A0E:
    mov.l @r15, r2
    add #0x40, r2
    mov.l r2, @r15
.L_06006A14:
    mov.l @r15, r3
    mov.l @(36, r15), r2
    cmp/gt r2, r3
    bf      .L_06006952
    mov.l   .L_pool_06006A84, r2
    mov.l   .L_pool_06006A88, r3
    mov.l @r2, r2
    and r3, r2
    tst r2, r2
    bf      .L_06006A6C
    bra     .L_06006A4A
    mov.l @r14, r0
.L_06006A2C:
    mov.l   .L_pool_06006A8C, r3
    jsr @r3
    nop
    bra     .L_06006A56
    nop
.L_06006A36:
    mov.l   .L_pool_06006A90, r3
    jsr @r3
    nop
    bra     .L_06006A56
    nop
.L_06006A40:
    mov.l   .L_pool_06006A94, r3
    jsr @r3
    nop
    bra     .L_06006A56
    nop
.L_06006A4A:
    cmp/eq #0x0, r0
    bt      .L_06006A2C
    cmp/eq #0x1, r0
    bt      .L_06006A36
    cmp/eq #0x2, r0
    bt      .L_06006A40
.L_06006A56:
    add #0x2C, r15
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    mov.l   .L_pool_06006A98, r3
    jmp @r3
    mov.l @r15+, r14
.L_06006A6C:
    add #0x2C, r15
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14
.L_pool_06006A80:
    .4byte  sym_0602AF3C
.L_pool_06006A84:
    .4byte  sym_0607EBC4
.L_pool_06006A88:
    .4byte  0x30000000
.L_pool_06006A8C:
    .4byte  track_geometry_proc
.L_pool_06006A90:
    .4byte  track_state_machine
.L_pool_06006A94:
    .4byte  geom_math_calc
.L_pool_06006A98:
    .4byte  scene_process_stage

    .global scene_path_a
    .type scene_path_a, @function
scene_path_a:
    mov.l r14, @-r15
    mov.l r13, @-r15
    mov.l r12, @-r15
    mov.l r11, @-r15
    mov.l r10, @-r15
    mov.l r9, @-r15
    mov.l r8, @-r15
    sts.l pr, @-r15
    add #-0x30, r15
    mov.l   .L_pool_06006B5C, r8
    mov.l   .L_pool_06006B60, r9
    mov.l   .L_pool_06006B64, r14
    mov.l   .L_pool_06006B68, r4
    mov.l @r14, r0
    shll2 r0
    shll r0
    mov.l @(r0, r4), r3
    mov.l r3, @(12, r15)
    mov.l @r14, r0
    shll r0
    add #0x1, r0
    shll2 r0
    mov.l @(r0, r4), r2
    mov.l r2, @(4, r15)
    mov.l   .L_pool_06006B6C, r4
    mov.l @r14, r0
    shll2 r0
    shll2 r0
    mov.l @(r0, r4), r1
    mov.l r1, @(32, r15)
    mov.l @r14, r5
    shll2 r5
    mov r5, r0
    add #0x1, r0
    shll2 r0
    mov.l @(r0, r4), r3
    mov r5, r0
    mov.l r3, @(36, r15)
    add #0x2, r0
    shll2 r0
    mov.l @(r0, r4), r2
    mov r5, r0
    mov.l r2, @(20, r15)
    add #0x3, r0
    shll2 r0
    mov.l @(r0, r4), r1
    mov.l r1, @(24, r15)
    mov.l   .L_pool_06006B70, r5
    mov.l   .L_pool_06006B74, r3
    mov.l @r5, r4
    add r3, r4
    shlr16 r4
    shlr2 r4
    shlr2 r4
    shlr r4
    mov.l @(8, r5), r2
    neg r2, r2
    add #-0x1, r3
    add r3, r2
    mov r2, r5
    shlr16 r5
    shlr2 r5
    shlr2 r5
    shlr r5
    mov #-0x2, r6
    mov r5, r2
    mov r6, r3
    shll2 r2
    shll2 r2
    shll2 r2
    add r4, r2
    mov.l r2, @(28, r15)
    mov.l r3, @(44, r15)
    mov #0x2, r3
    mov.l r3, @(16, r15)
    mov.l r3, @(40, r15)
    mov #0x3E, r3
    cmp/hs r3, r5
    bf      .L_06006B44
    mov #0x3F, r3
    sub r5, r3
    mov.l r3, @(16, r15)
    bra     .L_06006B4C
    nop
.L_06006B44:
    mov #0x1, r2
    cmp/hi r2, r5
    bt      .L_06006B4C
    neg r5, r6
.L_06006B4C:
    mov #0x3E, r2
    cmp/hs r2, r4
    bf      .L_06006B78
    mov #0x3F, r2
    sub r4, r2
    bra     .L_06006B82
    mov.l r2, @(40, r15)
    .2byte  0xFFFF
.L_pool_06006B5C:
    .4byte  sym_060620D4
.L_pool_06006B60:
    .4byte  sym_06089E44
.L_pool_06006B64:
    .4byte  sym_0607EAD8
.L_pool_06006B68:
    .4byte  sym_06062248
.L_pool_06006B6C:
    .4byte  sym_06062260
.L_pool_06006B70:
    .4byte  sym_06063DF8
.L_pool_06006B74:
    .4byte  0x04000000
.L_06006B78:
    mov #0x1, r2
    cmp/hi r2, r4
    bt      .L_06006B82
    neg r4, r4
    mov.l r4, @(44, r15)
.L_06006B82:
    shll2 r6
    shll2 r6
    shll2 r6
    mov.l @(16, r15), r3
    shll2 r3
    shll2 r3
    shll2 r3
    mov.l r3, @(16, r15)
    bra     .L_06006CAC
    mov r6, r10
.L_06006B96:
    bra     .L_06006CA0
    mov.l @(44, r15), r12
.L_06006B9A:
    mov r10, r0
    cmp/eq #-0x80, r0
    bt      .L_06006BB6
    mov.w   DAT_06006c1e, r3
    cmp/eq r3, r10
    bt      .L_06006BB6
    mov r12, r0
    cmp/eq #-0x2, r0
    bt      .L_06006BB6
    mov r12, r0
    cmp/eq #0x2, r0
    bf      .L_06006C9E
    bra     .L_06006BB6
    nop
.L_06006BB6:
    mov r10, r2
    mov.l @(28, r15), r3
    add r12, r2
    add r3, r2
    mov r2, r4
    mov.l r2, @r15
    shll r4
    mov.l @(32, r15), r13
    mov.l @(20, r15), r3
    add r4, r13
    add r3, r4
    mov.w @r13, r13
    exts.w r13, r2
    cmp/pl r2
    bf/s    .L_06006C3E
    mov.w @r4, r11
    mov.l @r14, r3
    mov.l   .L_pool_06006C24, r2
    mov.l @r15, r0
    mov.l @(36, r15), r1
    shll2 r3
    shll2 r0
    shll r3
    mov.l @(r0, r1), r0
    add r2, r3
    mov.l @r3, r3
    exts.w r13, r2
    add r0, r3
    mov.l r3, @(8, r15)
    mov.l @r8, r0
    add r0, r2
    mov.w   .L_wpool_06006C20, r0
    cmp/hs r0, r2
    bf      .L_06006C02
    mov.w   .L_wpool_06006C20, r13
    mov.l @r8, r3
    sub r3, r13
    exts.w r13, r13
.L_06006C02:
    mov.l @r14, r0
    cmp/eq #0x2, r0
    bt      .L_06006C2C
    exts.w r13, r7
    mov r9, r6
    mov.l @(12, r15), r5
    mov.l   .L_pool_06006C28, r3
    add #0x52, r6
    mov.w @r6, r6
    extu.w r6, r6
    jsr @r3
    mov.l @(8, r15), r4
    bra     .L_06006C3E
    nop

    .global DAT_06006c1e
DAT_06006c1e:
    .2byte  0x0080
.L_wpool_06006C20:
    .2byte  0x0640
    .2byte  0xFFFF
.L_pool_06006C24:
    .4byte  sym_06062230
.L_pool_06006C28:
    .4byte  sym_0602A214
.L_06006C2C:
    exts.w r13, r7
    mov r9, r6
    mov.l @(12, r15), r5
    mov.l   .L_pool_06006CCC, r3
    add #0x52, r6
    mov.w @r6, r6
    extu.w r6, r6
    jsr @r3
    mov.l @(8, r15), r4
.L_06006C3E:
    exts.w r11, r2
    cmp/pl r2
    bf      .L_06006C9E
    exts.w r11, r1
    mov.l @r14, r13
    mov.l   .L_pool_06006CD0, r3
    mov.l @r15, r0
    mov.l @(24, r15), r2
    shll r13
    shll2 r0
    add #0x1, r13
    mov.l @(r0, r2), r0
    shll2 r13
    add r3, r13
    mov.l @r13, r13
    add r0, r13
    mov.l @r8, r0
    add r0, r1
    mov.w   DAT_06006cca, r0
    cmp/hs r0, r1
    bf      .L_06006C70
    mov.w   DAT_06006cca, r11
    mov.l @r8, r3
    sub r3, r11
    exts.w r11, r11
.L_06006C70:
    mov.l @r14, r0
    cmp/eq #0x2, r0
    bt      .L_06006C8C
    exts.w r11, r7
    mov r9, r6
    mov.l @(4, r15), r5
    mov.l   .L_pool_06006CD4, r3
    add #0x52, r6
    mov.w @r6, r6
    extu.w r6, r6
    jsr @r3
    mov r13, r4
    bra     .L_06006C9E
    nop
.L_06006C8C:
    exts.w r11, r7
    mov r9, r6
    mov.l @(4, r15), r5
    mov.l   .L_pool_06006CD8, r3
    add #0x52, r6
    mov.w @r6, r6
    extu.w r6, r6
    jsr @r3
    mov r13, r4
.L_06006C9E:
    add #0x1, r12
.L_06006CA0:
    mov.l @(40, r15), r2
    cmp/ge r2, r12
    bt      .L_06006CAA
    bra     .L_06006B9A
    nop
.L_06006CAA:
    add #0x40, r10
.L_06006CAC:
    mov.l @(16, r15), r2
    cmp/ge r2, r10
    bt      .L_06006CB6
    bra     .L_06006B96
    nop
.L_06006CB6:
    add #0x30, r15
    lds.l @r15+, pr
    mov.l @r15+, r8
    mov.l @r15+, r9
    mov.l @r15+, r10
    mov.l @r15+, r11
    mov.l @r15+, r12
    mov.l @r15+, r13
    rts
    mov.l @r15+, r14

    .global DAT_06006cca
DAT_06006cca:
    .2byte  0x0640
.L_pool_06006CCC:
    .4byte  vdp1_list_finalize
.L_pool_06006CD0:
    .4byte  sym_06062230
.L_pool_06006CD4:
    .4byte  sym_0602ABB8
.L_pool_06006CD8:
    .4byte  sym_0602B9E0
