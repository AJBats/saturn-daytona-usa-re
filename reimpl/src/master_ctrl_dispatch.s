 ...
 *
 * Display element dispatch controller. Manages 4 timed display channels
 * (A/B/C/D) with command IDs 0x1901, 0x1801, 0x1701, 0x1601.
 *
 ...
 *   - set 3D position/scale via sym_060270F2
 *   - apply matrix rotation via mat_rot_xy_b
 *   - set render parameters from indexed tables (z, x, y, w)
 *   - decrement cmd_buf_base by 0x30 per frame
 *   - cycle through 3 animation frames (channels A-D) or 4 (anim dispatch)
 *
 ...
 *
 * Register allocation:
 *   r8  = display element index word ptr (sym_0605DF44)
 *   r9  = 0.5 fixed-point constant (0x00008000)
 *   r10 = command buffer base ptr (sym_0608A52C)
...
master_ctrl_dispatch:
    ...
    sts.l pr, @-r15
    add #-0x4, r15
    mov.l   .L_disp_elem_index, r8       ! r8 = &display_element_index
    mov.l   .L_fp_half, r9               ! r9 = 0.5 (16.16 fixed-point)
    mov.l   .L_cmd_buf_base, r10         ! r10 = &cmd_buffer
...
.L_check_timer_ch_a:                     ! -- channel A timer processing --
    ...
    bf      .L_ch_a_neg_position         ! if either != 1, use neg position
    ...
    mov r9, r5                           ! r5 = 0.5 (delay slot)
    mov.w   DAT_0601c510, r6             ! r6 = 0x6666 (uniform scale)
...
.L_fn_mat_scale:
    .4byte  mat_scale_b
.L_ch_a_neg_position:                    ! -- ch A: flags != 1, use alt position --
    mov.l   .L_fp_neg_one, r6            ! r6 = -1.0 (16.16)
    mov.l   .L_pos_param_ch_a_alt, r4    ! r4 = 0x14872 (same position)
    mov.l   .L_fn_position_set_2, r3
    jsr @r3                              ! position_set(pos, 0.5, -1.0)
    mov r9, r5
.L_ch_a_render_common:                   ! -- ch A: common render path --
    mov.w   DAT_0601c5f8, r4             ! r4 = 0x1000 (rotation angle)
    mov.l   .L_fn_mat_rot_xy, r3
    jsr @r3                              ! mat_rot_xy(0x1000)
    nop
...
.L_check_anim_state:                     ! -- animation state check --
    ...
    mov.w r3, @r8                        ! display_elem_index = 0
...
.L_anim_render_dispatch:                 ! -- animated display element render --
    ...
    jsr @r3                              ! render_setup()
    nop
    mov.l   .L_camera_state_ptr, r14     ! r14 = camera_state struct
    mov.l   .L_z_offset_neg, r3          ! r3 = -0xA000 (z offset)
    mov.w   .L_y_offset_neg, r2          ! r2 = 0xCCCD (y offset, signed neg)
    mov.l @(8, r14), r6                  ! r6 = camera_state.z
    mov.l @(4, r14), r5                  ! r5 = camera_state.y
    neg r6, r6                           ! r6 = -z
    add r2, r5                           ! r5 = y + y_offset
    add r3, r6                           ! r6 = -z + z_offset
    mov.l   .L_fn_position_set_6, r3
    jsr @r3                              ! position_set(x, y+off, -z+off)
    mov.l @r14, r4                       ! r4 = camera_state.x (delay slot)
    mov.l   .L_fn_mat_rot_xy_5, r3
    jsr @r3                              ! mat_rot_xy(0.5)
    mov r9, r4                           ! r4 = 0.5 (delay slot)
    mov.w @r8, r5                        ! r5 = display_elem_index
    ...
    jsr @r3
    mov.l @r4, r4
    mov.l @r10, r2                       ! r2 = cmd_buf ptr
    add #-0x30, r2                       ! advance cmd buffer by -0x30
    mov.l r2, @r10
    mov.w @r8, r3                        ! elem_index++
    add #0x1, r3
    mov.w r3, @r8
    mov.w @r8, r2
    mov #0x4, r3
    cmp/ge r3, r2                        ! index >= 4?
    bf      .L_epilogue                  ! no -- return
    exts.w r13, r3
    mov.w r3, @r8                        ! wrap index to 0
