
    .section .text.FUN_06028384


    .global FUN_06028384
    .type FUN_06028384, @function
FUN_06028384:
    sts.l pr, @-r15
    mov.l   .L_pool_060283AC, r0
    mov.l @(r0, r4), r2
    mov.l @r2, r2
    add r2, r5
    .reloc ., R_SH_IND12W, FUN_060282C0 - 4
    .2byte 0xB000    /* bsr FUN_060282C0 (linker-resolved) */
    mov r7, r4
    mov r0, r1
    mov #0x16, r7
    mov #0x0, r0
    .global FUN_06028398
FUN_06028398:
.L_06028398:
    mov.b @r1+, r3
    add r6, r3
    mov.w r3, @(r0, r5)
    cmp/eq r7, r0
    bf/s    .L_06028398
    add #0x2, r0
    lds.l @r15+, pr
    rts
    nop
    .2byte  0x0000
.L_pool_060283AC:
    .4byte  sym_06028614
    .4byte  0xA003E302
    .4byte  0xA001E306
    .4byte  0xE30ED008
    .4byte  0x024E6222
    .4byte  0x325CE50F
    .4byte  0x61736033
    .4byte  0x21594709
    .4byte  0x316C4709
    .4byte  0x02152338
    .4byte  0x8FF673FE
    .4byte  0x000B0009
    .4byte  sym_06028614

    .global sym_060283E0
sym_060283E0:
    mov.l   .L_pool_060283F8, r0
    mov.l @(r0, r4), r2
    mov.l @r2, r2
    add r5, r2
.L_060283E8:
    mov.b @r7+, r3
    tst r3, r3
    bt      .L_060283FC
    add r6, r3
    mov.w r3, @r2
    bra     .L_060283E8
    add #0x2, r2
    .2byte  0x0000
.L_pool_060283F8:
    .4byte  sym_06028614
.L_060283FC:
    rts
    nop

    .global sym_06028400
sym_06028400:
    mov.l   .L_pool_0602842C, r0
    mov.l @(r0, r4), r4
    mov.l @r4, r4
    add r4, r6
    mov #-0x80, r4
    mov.w @r5+, r3
    mov.w @r5+, r2
    add r3, r4
    add r3, r4
    neg r4, r4
.L_06028414:
    mov r3, r1
.L_06028416:
    mov.w @r5+, r0
    add r7, r0
    mov.w r0, @r6
    dt r1
    bf/s    .L_06028416
    add #0x2, r6
    dt r2
    bf/s    .L_06028414
    add r4, r6
    rts
    nop
.L_pool_0602842C:
    .4byte  sym_06028614

    .global sym_06028430
sym_06028430:
    mov.l r8, @-r15
    mov.l r9, @-r15
    mov.l r11, @-r15
    mov.l r12, @-r15
    mov.l r13, @-r15


    .global FUN_0602843A
    .type FUN_0602843A, @function
FUN_0602843A:
    mov.l r14, @-r15
    sts.l pr, @-r15
    .byte   0xD0, 0x19    /* mov.l .L_pool_060284A4, r0 */
    mov.l @(r0, r4), r4
    mov.l @r4, r4
    add r4, r5
    .reloc ., R_SH_IND12W, FUN_06028306 - 4
    .2byte 0xB000    /* bsr FUN_06028306 (linker-resolved) */
    mov r7, r4
    .byte   0xD1, 0x17    /* mov.l .L_pool_060284A8, r1 */
    add r1, r6
    mov.l @r6+, r1
    mov.w @r6+, r2
    mov.b @r6+, r3
    mov.b @r6+, r4
    mov.b @r6+, r11
    mov.b @r6+, r8
    mov.w @r6+, r12
    mov r8, r13
    add #-0xC, r13
    neg r13, r13
    add r13, r0
    mov r12, r6
    mov r0, r14
.L_06028468:
    mov r8, r12
    mov r14, r0
.L_0602846C:
    mov r3, r13
    mov r1, r9
    mov.b @r0+, r7
    mulu.w r7, r11
    sts macl, r7
    add r7, r9
.L_06028478:
    mov.w @r9, r7
    add r2, r7
    mov.w r7, @r5
    add #0x2, r9
    dt r13
    bf/s    .L_06028478
    add #0x2, r5
    dt r12
    bf      .L_0602846C
    add r3, r1
    add r3, r1
    dt r4
    bf/s    .L_06028468
    add r6, r5
    lds.l @r15+, pr
    mov.l @r15+, r14
    mov.l @r15+, r13
    mov.l @r15+, r12
    mov.l @r15+, r11
    mov.l @r15+, r9
    rts
    mov.l @r15+, r8
.L_pool_060284A4:
    .4byte  sym_06028614
.L_pool_060284A8:
    .4byte  sym_06063690
    .2byte  0x0009

    .global sym_060284AE
sym_060284AE:
    mov.l r8, @-r15
    mov.l r9, @-r15
    mov.l r10, @-r15
    mov.l r11, @-r15
    mov.l r12, @-r15
    mov.l r13, @-r15

    .global FUN_060284BA
    .type FUN_060284BA, @function
FUN_060284BA:
    mov.l r14, @-r15
    sts.l pr, @-r15
    .byte   0xD0, 0x1D    /* mov.l .L_pool_06028534, r0 */
    mov.l @(r0, r4), r4
    mov.l @r4, r4
    add r4, r5
    mov r7, r0
    .byte   0xD1, 0x1B    /* mov.l .L_pool_06028538, r1 */
    add r1, r6
    mov.l @r6+, r1
    mov.w @r6+, r2
    mov.b @r6+, r3
    mov.b @r6+, r4
    mov.b @r6+, r11
    mov.b @r6+, r8
    mov r0, r14
    mov #-0x1, r12
.L_060284DC:
    mov.b @r0+, r7
    tst r7, r7
    bf/s    .L_060284DC
    add #0x1, r12
    mulu.w r12, r3
    mov r12, r10
    sts macl, r12
    add #-0x40, r12
    neg r12, r12
    shll r12
    mov r14, r0
.L_060284F2:
    mov r10, r9
    mov r14, r0
.L_060284F6:
    mov.b @r0+, r7
    mov r3, r13
    sub r8, r7
    mulu.w r7, r11
    mov r1, r6
    sts macl, r7
    add r7, r6
.L_06028504:
    mov.w @r6, r7
    add r2, r7
    mov.w r7, @r5
    add #0x2, r6
    dt r13
    bf/s    .L_06028504
    add #0x2, r5
    dt r9
    bf      .L_060284F6
    add r3, r1
    add r3, r1
    dt r4
    bf/s    .L_060284F2
    add r12, r5
    lds.l @r15+, pr
    mov.l @r15+, r14
    mov.l @r15+, r13
    mov.l @r15+, r12
    mov.l @r15+, r11
    mov.l @r15+, r10
    mov.l @r15+, r9
    rts
    mov.l @r15+, r8
    .2byte  0x0000
.L_pool_06028534:
    .4byte  sym_06028614
.L_pool_06028538:
    .4byte  sym_06063690
    .2byte  0x0009

    .global sym_0602853E
sym_0602853E:
    .byte   0xD0, 0x06    /* mov.l .L_pool_06028558, r0 */
    mov.l @(r0, r4), r2
    .byte   0xD0, 0x06    /* mov.l .L_pool_0602855C, r0 */
    mov.l @(r0, r4), r0
    mov.l @r0, r0
    mov.w   .L_wpool_06028556, r1
.L_0602854A:
    mov.l r2, @r0
    dt r1
    bf/s    .L_0602854A
    add #0x4, r0
    rts
    nop
.L_wpool_06028556:
    .2byte  0x0800
.L_pool_06028558:
    .4byte  sym_06028644
.L_pool_0602855C:
    .4byte  sym_0602862C

    .global sym_06028560
sym_06028560:
    .byte   0xD0, 0x04    /* mov.l .L_pool_06028574, r0 */
    mov.w   .L_wpool_06028572, r1
    .byte   0xD2, 0x04    /* mov.l .L_pool_06028578, r2 */
.L_06028566:
    mov.l r2, @r0
    dt r1
    bf/s    .L_06028566
    add #0x4, r0
    rts
    nop
.L_wpool_06028572:
    .2byte  0x0380
.L_pool_06028574:
    .4byte  sym_060612C4
.L_pool_06028578:
    .4byte  0x00200020
    .4byte  0x00002800
