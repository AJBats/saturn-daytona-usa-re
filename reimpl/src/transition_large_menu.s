 ...
 *
 * Large menu transition setup — initializes audio, loads palette/tile data
 * for the main menu screens, and configures display layers for the menu
 * overlay (mode select, car select, course select).
 *
 * Initialization sequence:
 *   1. sound_cmd_dispatch(0, 0xAE0003FF) — trigger menu music
 *   2. display_channel_config(0xC) — configure display channels
 *   3. memcpy_word_idx(palette_src, VDP2_CRAM+0x460, 0x80) — load palette
 *   4. scroll_display_setup() — configure scroll/display parameters
 *   5. Read game status word[+6] to select tile layout:
 *      If nonzero: tile offsets = 0x28/0x27 (extended layout)
 *      Else: tile offsets = 0x25/0x23 (standard layout)
 *   6. Configure 3 display layers via tile_writer() calls
 *   7. sound_cmd_dispatch(0, 0xAB1104FF) — secondary sound command
 *   8. Set game_mode_byte = 9, clear transition state vars, set active = 1
 *   9. If 2P flag set: configure additional split-screen display layer
 *  10. If 2P flag clear: ui_element_setup(1) + anim_ui_transition(1)
 *  11. graphics_mode_setup() → tail-call dual_hud_dispatch
 *
 * Persistent registers:
 *   r11 = tile table base (sym_06063750)
 *   r12 = tile_writer function (sym_06028400)
 *   r13 = tile index B (mode-dependent)
 *   r14 = tile index A (mode-dependent)
 */
...
transition_large_menu:
    ...
    mov.l r11, @-r15
    sts.l pr, @-r15
    add #-0x4, r15                 ! allocate 4 bytes stack space
    ...
    jsr @r3                        ! display_channel_config(0xC)
    mov #0xC, r4
    mov.w   .L_palette_size, r6    ! r6 = 0x80 (128 bytes = 64 colors)
    ...
    jsr @r3                        ! memcpy_word_idx(CRAM+0x460, palette, 0x80)
    nop
    .byte   0xD3, 0x12    /* mov.l .L_fn_scroll_setup, r3 */
    jsr @r3                        ! scroll_display_setup()
    nop
    .byte   0xD2, 0x12    /* mov.l .L_status_word, r2 — game status array */
    mov.w @(6, r2), r0             ! read status[+6] = display mode
    mov r0, r3
    extu.w r3, r3
    tst r3, r3                     ! zero = standard, nonzero = extended
    bf      .L_extended_layout
    mov #0x25, r14                 ! standard: tile index A = 0x25
    bra     .L_setup_display_layers
    mov #0x23, r13                 ! standard: tile index B = 0x23
.L_palette_size:
...
.L_setup_display_layers:                   /* === Configure 3 display layers === */
    .byte   0xDB, 0x35    /* mov.l .L_tile_table, r11 — tile table base */
    mov r14, r7                    ! --- layer A setup ---
    .byte   0xDC, 0x35    /* mov.l .L_fn_tile_writer, r12 */
    shll2 r7                       ! index * 4
    shll r7                        ! index * 8
    add r11, r7                    ! &tile_table[index_A]
    mov.l r7, @r15                 ! save table entry ptr on stack
    mov.l @(4, r7), r7             ! tile_table[A][+4] = layer data ptr
    .byte   0xD3, 0x33    /* mov.l .L_tile_data_offset, r3 — 0x9000 */
    mov.w   .L_tile_size_a, r6    ! r6 = 0x0B04 (tile size A)
    mov.l @r15, r5                 ! r5 = table entry base
    add r3, r7                     ! apply +0x9000 offset to data ptr
    mov.l @r5, r5                  ! r5 = tile_table[A][0] = source data
    jsr @r12                       ! tile_writer(0xC, src, tile_size, data+offset)
    mov #0xC, r4
    mov r13, r7                    ! --- layer B setup ---
    shll2 r7
    shll r7                        ! index * 8
    add r11, r7                    ! &tile_table[index_B]
    mov.l r7, @r15
    mov.l @(4, r7), r7             ! tile_table[B][+4]
    .byte   0xD3, 0x2D    /* mov.l .L_tile_data_offset, r3 — 0x9000 */
    mov.w   .L_tile_size_b, r6    ! r6 = 0x0B38 (tile size B)
    mov.l @r15, r5
    add r3, r7
    mov.l @r5, r5
    jsr @r12                       ! tile_writer(0xC, src, tile_size, data+offset)
    mov #0xC, r4
    mov.w   .L_tile_index_c, r7   ! --- layer C setup --- r7 = 0x0170
    add r11, r7                    ! &tile_table[0x2E] (0x170/8)
    mov.l r7, @r15
    mov.l @(4, r7), r7             ! tile_table[C][+4]
    mov.w   .L_tile_offset_c, r3  ! r3 = 0x4000 (tile offset C)
    mov.w   .L_tile_size_c, r6    ! r6 = 0x0C26 (tile size C)
    mov.l @r15, r5
    add r3, r7
    mov.l @r5, r5
    jsr @r12                       ! tile_writer(0xC, src, tile_size, data+offset)
...
.L_finalize:                               /* === Common finalization === */
    ...
    jmp @r3                        ! tail-call dual_hud_dispatch
