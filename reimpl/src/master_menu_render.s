 ...
 *
 * Menu/scene rendering pipeline for Daytona USA primary SH-2.
 * Manages display list construction, SMPC peripheral input polling,
 * VDP1 sprite command generation, and multi-phase animation state.
 *
 * Functions:
 ...
 *   data_table_copy        - Byte-by-byte copy from source cursor to dest,
 *                            with special-case for 0x20 (space) character
 *   game_logic_main        - Button state decode: clears direction bits
 *                            (up/down/left/right/A/B/C/start) from packed
 *                            input byte, writes render table entries
 *   display_channel_iter   - Iterates display list: stores char + draws
 *                            VDP1 sprite per entry until count reached
 *   vdp1_sprite_draw_main  - Mode-switched VDP1 command builder: writes
 *                            sprite coords/size per display mode (00-FF)
 ...
 *   0x30 (8-byte), 0xE1 (3-byte), 0xE2 (6-byte), 0xFF (per-element fill)
 */
