 ...
 *
 * Mixed TU containing Stack A vec-transform functions and Stack B matrix
 * stack management / identity / accumulate-translate functions.
 *
 * Stack A (camera/world transform) addresses in local pool:
 ...
 *
 * Stack B (object/model transform) addresses in far pool (TU mat_vec_transform_b):
 *   sym_0608A52C = matrix stack B pointer (current top)  [at .L_pool_0602732C]
 *   sym_0608A530 = matrix stack B alternate base         [at .L_pool_06027330]
 *   sym_0608A6B0 = vector scratch buffer B0              [at .L_pool_06027334]
 *   0x00010000   = 1.0 in 16.16 fixed-point              [at .L_pool_06027340]
 *
 ...
 *
 * Contains:
 *   mat_vec_transform      - Stack A: M * vec + T (rotation + translation)
 *   mat_vec_notrans        - Stack A: M * vec     (rotation only, raw bytes)
 *   sym_06027080           - Stack B: matrix stack push (duplicate top, advance by 0x30)
 *   mat_stack_pop          - Stack B: matrix stack pop (rewind by 0x30, raw bytes)
 *   sym_060270C6           - Stack B: set identity at explicit base address
 *   sym_060270D0           - Stack B: set identity at current stack top
 *   sym_060270F2           - Stack B: transform vec3 inplace into translation vector
 */
...
/* sym_06027080 -- Matrix stack push (Stack B)
 * Address: 0x06027080
 * Sawyer name: mat_push_B
 *
 * Duplicates the current 48-byte (0x30) matrix entry at the top of
 * Stack B, then advances the stack pointer by 0x30 so the copy becomes
 * the new top. This preserves the parent transform for later restoration.
 *
 * Input:  none (reads matrix stack B pointer from sym_0608A52C via far pool)
 * Output: stack pointer advanced by 0x30; top entry duplicated
 *
 * Uses far pool in TU mat_vec_transform_b:
 *   .L_pool_0602732C = &mat_stack_b_ptr (sym_0608A52C)
 *
 * Algorithm:
 *   r3 = &mat_stack_b_ptr       (via .L_pool_0602732C in distant TU)
 *   r0 = *r3 = old_top
...
sym_06027080:
    ...
    mov.l @r3, r0                          ! r0 = old stack top (current matrix base, stack B)
    mov #0x30, r1                          ! r1 = 0x30 (matrix entry size: 36B rotation + 12B translation)
    add r0, r1                             ! r1 = old_top + 0x30 = new stack top
    mov.l r1, @r3                          ! advance stack B pointer to new top
...
/* mat_stack_pop -- Matrix stack pop (Stack B)
 * Address: 0x060270B6
 * Sawyer name: mat_pop_B
 *
 * Rewinds the matrix stack B pointer by 0x30 (one entry), effectively
 ...
 * (.L_pool_0602732C in TU mat_vec_transform_b).
 *
 * Uses far pool:
 *   .L_pool_0602732C = &mat_stack_b_ptr (sym_0608A52C)
 *
 * Input:  none
 * Output: stack B pointer decremented by 0x30
 *
 * Instruction sequence (decoded):
 *   mov.l .L_pool_0602732C, r1   ! r1 = &mat_stack_b_ptr
 *   mov.l @r1, r0                ! r0 = current stack top
 *   add #-0x30, r0               ! r0 = top - 0x30 (rewind one entry)
 *   rts                          ! return
 *   mov.l r0, @r1                ! *mat_stack_b_ptr = new top (delay slot)
 */
    .4byte  0xD19B6012
    .4byte  0x70D0000B
    .2byte  0x2102

/* sym_060270C6 -- Set identity matrix at explicit base (Stack B)
 * Address: 0x060270C6
 * Sawyer name: mat_identity_B2
 *
 * Resets the matrix stack B pointer to a specified initial base address
 * (sym_0608A530), then writes a 3x3 identity matrix + zero translation
 * at that location.
 *
 * Uses far pool entries (in TU mat_vec_transform_b):
 *   .L_pool_06027330 = initial base address for stack B (sym_0608A530)
 *   .L_pool_0602732C = &mat_stack_b_ptr (sym_0608A52C)
 *   .L_pool_06027340 = 0x00010000 (1.0 in 16.16 fixed-point)
 *
 * Input:  none
 * Output: mat_stack_b_ptr = initial base; identity matrix written there
 *
 * Algorithm:
 *   r0 = initial_base_addr   (from .L_pool_06027330 = sym_0608A530)
 *   r1 = &mat_stack_b_ptr    (from .L_pool_0602732C = sym_0608A52C)
...
sym_060270C6:
    ...
    mov.l r0, @r1                          ! *mat_stack_b_ptr = initial base (delay slot)

/* sym_060270D0 -- Set identity matrix at current stack top (Stack B)
 * Address: 0x060270D0
 * Sawyer name: mat_identity_B
 *
 * Writes a 3x3 identity matrix + zero translation at the current
 * matrix stack B top without changing the stack pointer.
 *
 * Uses far pool entries (in TU mat_vec_transform_b):
 *   .L_pool_0602732C = &mat_stack_b_ptr (sym_0608A52C)
 *   .L_pool_06027340 = 0x00010000 (1.0 in 16.16 fixed-point)
 *
 * Input:  none
 * Output: identity matrix written at current stack B top
 *
...
sym_060270D0:
    ...
    mov #0x0, r2                           ! r2 = 0 (zero for off-diagonal and translation)
    mov.l @r0, r0                          ! r0 = current stack B top (matrix base address)
.L_write_identity:
...
/* sym_060270F2 -- Transform vec3 inplace into translation vector (Stack B)
 * Address: 0x060270F2
 * Sawyer name: mat_xform_trans_B
 *
 * Transforms a 3-component vector by the current Stack B matrix's
...
sym_060270F2:
    ...
    mov.l @r1, r4                          ! r4 = current matrix base (stack B top)
