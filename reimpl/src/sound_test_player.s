 ...
 *
 * Sound test player and related scene/menu rendering functions.
 * Contains the debug sound test menu accessible from game options,
 * plus shared utilities for 3D scene setup, color matrix management,
 * and controller configuration.
 *
 * Functions:
 *   scene_invalidate_a     - Compute sine of 16.16 fixed-point angle (quadrant reduction)
 *   scene_invalidate_b     - Compute cosine of 16.16 fixed-point angle (quadrant reduction)
 *   controller_config      - Initialize controller type, display mode, and 3D view matrices
 *   scene_color_matrix     - Update per-viewport color/rotation matrices and dispatch rendering
 *   sound_test_player      - Main sound test rendering: build rotation matrix, compute palette
 *   options_render          - Options menu rendering: same structure as sound_test_player
 *   display_channel_handler - Rebuild rotation matrix for a single display channel
 *   hud_update_main        - Accumulate rotation angle and rebuild HUD view matrix
 */
