...
def generate_output(image, code_addrs, globals_map, fourbyte_map,
    ...
    Emits a single .section for the contiguous TU, with symbolic labels for
    branches and pool refs within the TU, raw bytes for cross-TU references.
    """
    lines = []
    ...
    def _in_tu(addr):
        """Check if an address is within the TU bounds."""
        return TU_START <= addr < TU_END

    # Emit single section header
    ...
    while pc < TU_END:
        ...
        if is_code:
            ...
            if hi == 0xD and pool_target is not None:
                ...
                if TU_START % 4 != 0 or not _in_tu(pool_target):
            ...
            if base in ('bra', 'bsr', 'bf', 'bt', 'bf/s', 'bt/s'):
                ...
                if target is not None:
                    if _in_tu(target):
                        # TU-internal branch — use symbolic label
                        if target in globals_map:
                        ...
                        # Cross-TU branch — keep as .byte pairs
                        b0 = image[off]
                        b1 = image[off + 1]
                        lines.append(f"    .byte   0x{b0:02X}, 0x{b1:02X}"
                                     f"    /* {mnemonic} (external) */")
