...
def generate_linker_script(fun_syms, provide_syms):
    ...
    lines.append(" * All referenced addresses provided at original locations.")
    lines.append(" */")
    lines.append("")
    ...
    # PROVIDE declarations — grouped by type
    fun_provide = {}
    dat_provide = {}
    syn_provide = {}
    ...
    if syn_provide:
        lines.append(f"/* Synthetic symbols from disassembly ({len(syn_provide)}) */")
...
def main():
    ...
    # DAT_ and synthetic symbols need PROVIDE (they have no .s file definition)
    provide_syms = dict(all_symbols)
    generate_ld = generate_linker_script(fun_syms, provide_syms)
